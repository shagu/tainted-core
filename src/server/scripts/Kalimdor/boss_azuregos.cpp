...
public:
      ...
      if (Teleport_Timer <= diff) {
        ...
        for (i = threatlist.begin(); i != threatlist.end(); ++i) {
          ...
          if (pUnit && (pUnit->GetTypeId() == TYPEID_PLAYER))
            DoTeleportPlayer(pUnit, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ() + 3,
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_azuregosAI(pCreature); }
};

void AddSC_boss_azuregos() { new boss_azuregos(); }
