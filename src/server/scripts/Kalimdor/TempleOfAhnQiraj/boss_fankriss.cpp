...
public:
        ...
        if (SpawnHatchlings_Timer <= diff) {
          ...
          if (pTarget && pTarget->GetTypeId() == TYPEID_PLAYER) {
            ...
            case 0:
              ...
              Hatchling = me->SummonCreature(15962, pTarget->GetPositionX() - 3, pTarget->GetPositionY() - 3,
              ...
              if (Hatchling)
                ((CreatureAI *)Hatchling->AI())->AttackStart(pTarget);
              Hatchling = me->SummonCreature(15962, pTarget->GetPositionX() - 5, pTarget->GetPositionY() - 5,
                                             pTarget->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 15000);
              if (Hatchling)
                ((CreatureAI *)Hatchling->AI())->AttackStart(pTarget);
              Hatchling = me->SummonCreature(15962, pTarget->GetPositionX() - 5, pTarget->GetPositionY() + 5,
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_fankrissAI(pCreature); }
};

void AddSC_boss_fankriss() { new boss_fankriss(); }
