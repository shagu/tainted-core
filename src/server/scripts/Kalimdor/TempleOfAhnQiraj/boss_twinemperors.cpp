...
struct boss_twinemperorsAI : public ScriptedAI {
  ...
  boss_twinemperorsAI(Creature *c) : ScriptedAI(c) { pInstance = (ScriptedInstance *)c->GetInstanceData(); }
...
#define HEAL_BROTHER_AMOUNT 30000.0f
  ...
  void TeleportToMyBrother() {
    ...
    if (pOtherBoss) {
      ...
      thismap->CreatureRelocation(pOtherBoss, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(),
                                  me->GetOrientation());
  ...
  Creature *RespawnNearbyBugsAndGetOne() {
    ...
    TypeContainerVisitor<Oregon::CreatureListSearcher<AnyBugCheck>, GridTypeMapContainer> grid_creature_searcher(
        searcher);
...
public:
  BugAura(SpellEntry *spell, uint32 eff, int32 *bp, Unit *pTarget, Unit *caster)
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_veknilashAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_veklorAI(pCreature); }
};
