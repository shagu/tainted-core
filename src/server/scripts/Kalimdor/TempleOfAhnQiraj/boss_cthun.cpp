...
public:
  ...
  struct eye_of_cthunAI : public Scripted_NoMovementAI {
    ...
    void SpawnEyeTentacle(float x, float y) {
      Creature *Spawned;
      Spawned = (Creature *)me->SummonCreature(MOB_EYE_TENTACLE, me->GetPositionX() + x, me->GetPositionY() + y,
                                               me->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 500);
      if (Spawned) {
        Unit *pTarget;
      ...
      case 0: {
        ...
        if (ClawTentacleTimer <= diff) {
          ...
          if (pTarget) {
            ...
            Spawned =
                (Creature *)me->SummonCreature(MOB_CLAW_TENTACLE, pTarget->GetPositionX(), pTarget->GetPositionY(),
                                               pTarget->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 500);
  ...
  // GetAIs
  CreatureAI *GetAI(Creature *pCreature) const { return new eye_of_cthunAI(pCreature); }
};
...
public:
  ...
  struct mob_giant_flesh_tentacleAI : public Scripted_NoMovementAI {
    ...
    void SpawnedByCthun(uint64 p) { Parent = p; }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new mob_giant_flesh_tentacleAI(pCreature); }
};
...
public:
  ...
  struct cthunAI : public Scripted_NoMovementAI {
    ...
    void UpdateAI(const uint32 diff) {
      ...
      case 2: {
        ...
        if (PhaseTimer <= diff) {
          ...
          Spawned = (Creature *)me->SummonCreature(MOB_FLESH_TENTACLE, TENTACLE_POS1_X, TENTACLE_POS1_Y,
                                                   TENTACLE_POS1_Z, TENTACLE_POS1_O, TEMPSUMMON_CORPSE_DESPAWN, 0);
          ...
          // Spawn flesh tentacle
          Spawned = (Creature *)me->SummonCreature(MOB_FLESH_TENTACLE, TENTACLE_POS2_X, TENTACLE_POS2_Y,
                                                   TENTACLE_POS2_Z, TENTACLE_POS2_O, TEMPSUMMON_CORPSE_DESPAWN, 0);
      ...
      case 3: {
        ...
        if (FleshTentaclesKilled > 1) {
          ...
          while (i != Stomach_Map.end()) {
            ...
            if (pUnit && i->second == true) {
              ...
              DoTeleportPlayer(pUnit, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ() + 10,
                               float(rand() % 6));
        ...
        if (GiantClawTentacleTimer <= diff) {
          ...
          if (pTarget) {
            ...
            // Spawn claw tentacle on the random target
            Spawned = (Creature *)me->SummonCreature(MOB_GIANT_CLAW_TENTACLE, pTarget->GetPositionX(),
                                                     pTarget->GetPositionY(), pTarget->GetPositionZ(), 0,
        ...
        if (GiantEyeTentacleTimer <= diff) {
          ...
          if (pTarget) {
            ...
            // Spawn claw tentacle on the random target
            Spawned =
                (Creature *)me->SummonCreature(MOB_GIANT_EYE_TENTACLE, pTarget->GetPositionX(), pTarget->GetPositionY(),
                                               pTarget->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 500);
    ...
    void FleshTentcleKilled() { FleshTentaclesKilled++; }
  };

  CreatureAI *GetAI(Creature *pCreature) const { return new cthunAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new eye_tentacleAI(pCreature); }
};
...
public:
      ...
      if (!me->IsWithinMeleeRange(me->GetVictim())) {
        if (EvadeTimer <= diff) {
          ...
          if (!pTarget->HasAura(SPELL_DIGESTIVE_ACID, 0)) {
            me->GetMap()->CreatureRelocation(me, pTarget->GetPositionX(), pTarget->GetPositionY(),
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new claw_tentacleAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new giant_claw_tentacleAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new giant_eye_tentacleAI(pCreature); }
};
