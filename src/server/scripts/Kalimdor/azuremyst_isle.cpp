...
SD%Complete: 75
SDComment: Quest support: 9283, 9537, 9582, 9554, 9531, 9303(special flight path, proper model for mount missing).
Injured Draenei cosmetic only, 9582. SDCategory: Azuremyst Isle EndScriptData */
...
public:
  ...
  struct npc_draenei_survivorAI : public ScriptedAI {
    ...
    void MoveInLineOfSight(Unit *who) // MoveInLineOfSight is called if creature could see you, updated all 100 ms
    {
      ...
      if (who->GetTypeId() == TYPEID_PLAYER && me->IsFriendlyTo(who) && me->IsWithinDistInMap(who, 15) && say &&
          !isRun) {
    ...
    void
    SpellHit(Unit *Hitter,
             const SpellEntry *Spellkind) // Called if you cast a spell and do some things if Specified spell is true!
    {
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_draenei_survivorAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *creature) const { return new npc_tavaraAI(creature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_engineer_spark_overgrindAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_injured_draeneiAI(pCreature); }
};
...
public:
  ...
  struct npc_magwinAI : public npc_escortAI {
    ...
    void EnterCombat(Unit *who) { DoScriptText(SAY_AGGRO, me, who); }

    void Reset() {}
  };

  CreatureAI *GetAI(Creature *pCreature) const { return new npc_magwinAI(pCreature); }
};
...
public:
  ...
  struct npc_geezleAI : public ScriptedAI {
    ...
    void StartEvent() {
      Step = 0;
      EventStarted = true;
      if (Creature *Spark = me->SummonCreature(NPC_SPARK, SparkPos[0], SparkPos[1], SparkPos[2], 0,
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_geezleAI(pCreature); }
};
...
public:
  ...
  struct mob_nestlewood_owlkinAI : public ScriptedAI {
    ...
    void UpdateAI(const uint32 diff) {
      if (ChannelTimer <= diff && !Channeled && Hitted) {
        ...
        me->RemoveCorpse();
        me->SummonCreature(INOCULATED_OWLKIN, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), 0.0f,
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new mob_nestlewood_owlkinAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_death_ravagerAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_stillpine_capitiveAI(pCreature); }
};
