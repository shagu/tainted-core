...
enum misc { SAY_START = -1921941, SAY_END = -1921942, QUEST_THERYLUNE_ESCAPE = 945 };
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_theryluneAI(pCreature); }
...
public:
  ...
  struct npc_kerlonianAI : public FollowerAI {
    ...
    void Reset() { m_uiFallAsleepTimer = urand(10000, 45000); }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_kerlonianAI(pCreature); }
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_prospector_remtravelAI(pCreature); }
...
public:
  ...
  struct npc_threshwackonatorAI : public FollowerAI {
    ...
    void JustDied() { SetFollowComplete(); }
  };

  CreatureAI *GetAI(Creature *pCreature) const { return new npc_threshwackonatorAI(pCreature); }

  bool OnGossipHello(Player *pPlayer, Creature *pCreature) override {
    if (pPlayer->GetQuestStatus(QUEST_GYROMAST_REV) == QUEST_STATUS_INCOMPLETE)
      pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_ITEM_INSERT_KEY, GOSSIP_SENDER_MAIN,
...
public:
  ...
  struct npc_rabid_bearAI : public ScriptedAI {
    ...
    void Reset() { rabiesTimer = urand(12000, 18000); }
        ...
        if (checkTimer <= diff) {
          ...
          if (GameObject *trap = GetClosestGameObjectWithEntry(me, GO_NIGHT_ELVEN_BEAR_TRAP, 0.5f)) {
            ...
            if (trapOwner && trapOwner->GetTypeId() == TYPEID_PLAYER &&
              ...
              ((Player *)trapOwner)
                  ->CastedCreatureOrGO(NPC_CAPTURED_RABID_THISTLE_BEAR, me->GetGUID(), SPELL_PLACE_TRAP);
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_rabid_bearAI(pCreature); }
};
