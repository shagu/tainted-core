...
SD%Complete: 85
SDComment: Doomfires not completely offlike due to core limitations for random moving. Tyrande and second phase not
fully implemented. SDCategory: Caverns of Time, Mount Hyjal EndScriptData */
...
#define SPELL_DOOMFIRE_VISUAL                                                                                          \
  42344 // This is actually a Zul'Aman spell, but the proper Doomfire spell sometimes freezes the server if a player
        // stands in it for too long
...
public:
  ...
  struct mob_doomfireAI : public ScriptedAI {
    ...
    void MoveInLineOfSight(Unit *who) {
      // Do not do anything if who does not exist, or we are refreshing our timer, or who is Doomfire, Archimonde or
      // Doomfire targetting
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new mob_doomfireAI(pCreature); }
};
...
public:
  ...
  struct mob_doomfire_targettingAI : public ScriptedAI {
    ...
    uint32 SummonTimer; // This timer will serve as both a summon timer for the doomfire that does damage as well as to
                        // check on Archionde
    ...
    void MoveInLineOfSight(Unit *who) {
      ...
      if (!who || who == me || who->GetEntry() == CREATURE_ARCHIMONDE || who->GetEntry() == CREATURE_DOOMFIRE ||
          who->GetEntry() == CREATURE_DOOMFIRE_TARGETING || !who->isTargetableForAttack())
        return;
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new mob_doomfire_targettingAI(pCreature); }
};
...
public:
  ...
  struct mob_ancient_wispAI : public ScriptedAI {
    mob_ancient_wispAI(Creature *c) : ScriptedAI(c) { pInstance = (ScriptedInstance *)c->GetInstanceData(); }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new mob_ancient_wispAI(pCreature); }
};
...
public:
  ...
  struct boss_archimondeAI : public hyjal_trashAI {
    boss_archimondeAI(Creature *c) : hyjal_trashAI(c) { pInstance = (ScriptedInstance *)c->GetInstanceData(); }
    ...
    void Reset() {
      ...
      CheckDistanceTimer = 30000; // This checks if he's too close to the World Tree (75 yards from a point on the
                                  // tree), if true then he will enrage
      SummonWispTimer = 0;
    ...
    void SummonDoomfire(Unit *pTarget) {
      Creature *Doomfire = DoSpawnCreature(CREATURE_DOOMFIRE_TARGETING, float(rand() % 30), float(rand() % 30), 0, 0,
      ...
      if (Doomfire) {
        ...
        for (itr = me->getThreatManager().getThreatList().begin(); itr != me->getThreatManager().getThreatList().end();
             ++itr)
    ...
    void UpdateAI(const uint32 diff) {
      if (!me->IsInCombat()) {
        ...
        if (DrainNordrassilTimer <= diff) {
          if (!IsChanneling) {
            Creature *Nordrassil = me->SummonCreature(CREATURE_CHANNEL_TARGET, NORDRASSIL_X, NORDRASSIL_Y, NORDRASSIL_Z,
                                                      0, TEMPSUMMON_TIMED_DESPAWN, 1200000);
            if (Nordrassil) {
          ...
          Creature *Nordrassil = me->SummonCreature(CREATURE_CHANNEL_TARGET, NORDRASSIL_X, NORDRASSIL_Y, NORDRASSIL_Z,
                                                    0, TEMPSUMMON_TIMED_DESPAWN, 5000);
          if (Nordrassil) {
            Nordrassil->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE);
            Nordrassil->SetDisplayId(11686);
      ...
      if (!Enraged) {
        ...
        if (CheckDistanceTimer <= diff) {
          // To simplify the check, we simply summon a creature in the location and then check how far we are from the
          // creature
          Creature *Check = me->SummonCreature(CREATURE_CHANNEL_TARGET, NORDRASSIL_X, NORDRASSIL_Y, NORDRASSIL_Z, 0,
      ...
      if (BelowTenPercent) {
        ...
        if (SummonWispTimer <= diff) {
          Creature *Wisp = DoSpawnCreature(CREATURE_ANCIENT_WISP, rand() % 40, rand() % 40, 0, 0,
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_archimondeAI(pCreature); }
};
