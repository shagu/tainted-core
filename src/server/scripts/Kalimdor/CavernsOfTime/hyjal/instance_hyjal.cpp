...
*/
#define YELL_EFFORTS                                                                                                   \
  "All of your efforts have been in vain, for the draining of the World Tree has already begun. Soon the heart of "    \
  "your world will beat no more."
...
public:
  ...
  struct instance_hyjalAI : public ScriptedInstance {
    instance_hyjalAI(Map *pMap) : ScriptedInstance(pMap) { Initialize(); };
    ...
    void SetData(uint32 type, uint32 data) {
      ...
      case DATA_AZGALOREVENT: {
        ...
        if (data == DONE) {
          ...
          if (pCreature) {
            Creature *pUnit = pCreature->SummonCreature(21987, pCreature->GetPositionX(), pCreature->GetPositionY(),
                                                        pCreature->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN, 10000);
            ...
            if (pMap->IsDungeon() && pUnit) {
              ...
              for (Map::PlayerList::const_iterator i = PlayerList.begin(); i != PlayerList.end(); ++i) {
                if (i->GetSource()) {
                  ...
                  pUnit->BuildMonsterChat(&data, CHAT_MSG_MONSTER_YELL, YELL_EFFORTS, 0, YELL_EFFORTS_NAME,
                                          i->GetSource()->GetGUID());
    ...
    void UpdateWorldState(uint32 id, uint32 state) {
      ...
      if (!players.isEmpty()) {
        ...
        sLog.outDebug("OSCR: Instance Hyjal: UpdateWorldState, but PlayerList is empty!");
    }
    ...
    std::string GetSaveData() {
      ...
      stream << Encounters[0] << " " << Encounters[1] << " " << Encounters[2] << " " << Encounters[3] << " "
             << Encounters[4] << " " << allianceRetreat << " " << hordeRetreat << " " << RaidDamage;
    ...
    void Load(const char *in) {
      ...
      loadStream >> Encounters[0] >> Encounters[1] >> Encounters[2] >> Encounters[3] >> Encounters[4] >>
          allianceRetreat >> hordeRetreat >> RaidDamage;
      for (uint8 i = 0; i < ENCOUNTERS; ++i)
  ...
  InstanceData *GetInstanceScript(InstanceMap *pMap) const override { return new instance_hyjalAI(pMap); }
};
