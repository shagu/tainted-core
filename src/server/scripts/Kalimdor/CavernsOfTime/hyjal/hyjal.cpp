...
public:
  ...
  bool OnGossipHello(Player *pPlayer, Creature *pCreature) override {
    ...
    if (RageEncounter == NOT_STARTED)
      pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_ITEM_BEGIN_ALLY, GOSSIP_SENDER_MAIN,
  ...
  bool OnGossipHello(Player *pPlayer, Creature *pCreature) override {
    ...
    if (AnetheronEvent == DONE && ai->GetInstanceData(DATA_ALLIANCE_RETREAT)) {
      ...
      if (KazrogalEvent == NOT_STARTED)
        pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_ITEM_BEGIN_HORDE, GOSSIP_SENDER_MAIN,
                                 GOSSIP_ACTION_INFO_DEF + 1);
