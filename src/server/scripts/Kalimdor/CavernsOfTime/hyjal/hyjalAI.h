...
// Spells for Jaina
#define SPELL_BRILLIANCE_AURA                                                                                          \
  31260 // The database must handle this spell via creature_addon(it should, but is removed in evade..)
...
struct Wave {
  ...
  bool
      IsBoss; // Simply used to inform the wave summoner that the next wave contains a boss to halt all waves after that
};
...
struct hyjalAI : public npc_escortAI {
  ...
  void Reset(); // Generically used to reset our variables. Do *not* call in EnterEvadeMode as this may make problems if
                // the raid is still in combat
  ...
  uint32 GetInstanceData(uint32 Event); // Gets instance data for this instance, used to check if raid has gotten past a
                                        // certain point and can access the next phase

  void Talk(uint32 id); // Searches for the appropriate yell and sound and uses it to inform the raid of various things

  void
  UpdateWorldState(uint32 field,
                   uint32 value); // NYI: Requires core support. Updates the world state counter at the top of the UI.
public:
