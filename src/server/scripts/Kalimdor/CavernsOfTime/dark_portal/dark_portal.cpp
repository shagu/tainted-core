...
static Wave PortalWaves[] = {
    {{C_ASSAS, C_WHELP, C_CHRON, 0}}, {{C_EXECU, C_CHRON, C_WHELP, C_ASSAS}}, {{C_EXECU, C_VANQU, C_CHRON, C_ASSAS}}};
...
public:
  ...
  struct npc_medivh_bmAI : public ScriptedAI {
    npc_medivh_bmAI(Creature *c) : ScriptedAI(c) { pInstance = (ScriptedInstance *)c->GetInstanceData(); }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_medivh_bmAI(pCreature); }
};
...
public:
  ...
  struct npc_time_riftAI : public ScriptedAI {
    npc_time_riftAI(Creature *c) : ScriptedAI(c) { pInstance = (ScriptedInstance *)c->GetInstanceData(); }
    ...
    void DoSummonAtRift(uint32 creature_entry) {
      ...
      pos.m_positionZ = std::max(me->GetMap()->GetHeight(pos.m_positionX, pos.m_positionY, MAX_HEIGHT),
                                 me->GetMap()->GetWaterLevel(pos.m_positionX, pos.m_positionY));
    ...
    void DoSelectSummon() {
      ...
      sLog.outDebug("OSCR: npc_time_rift: summoning wave creature (Wave %u, Entry %u).", mRiftWaveCount, entry);
    ...
    void UpdateAI(const uint32 diff) {
      ...
      sLog.outDebug("OSCR: npc_time_rift: not casting anylonger, i need to die.");
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_time_riftAI(pCreature); }
};
...
public:
  ...
  bool OnGossipHello(Player *player, Creature *pCreature) override {
    ...
    if (player->GetQuestStatus(QUEST_OPENING_PORTAL) == QUEST_STATUS_INCOMPLETE &&
        !player->HasItemCount(ITEM_CHRONO_BEACON, 1)) {
