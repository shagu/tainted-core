...
static Wave RiftWaves[] = {{RIFT_BOSS, 0},    {C_DEJA, 0},         {RIFT_BOSS, 120000},
                           {C_TEMPO, 140000}, {RIFT_BOSS, 120000}, {C_AEONUS, 0}};
...
public:
  ...
  struct instance_dark_portalAI : public ScriptedInstance {
    instance_dark_portalAI(Map *map) : ScriptedInstance(map) { Initialize(); };
    ...
    void UpdateBMWorldState(uint32 id, uint32 state) {
      ...
      if (!players.isEmpty()) {
        ...
        sLog.outDebug("OSCR: Instance Black Portal: UpdateBMWorldState, but PlayerList is empty!");
    }
        ...
        if (data == SPECIAL && Encounter[0] == IN_PROGRESS) {
          ...
          if (!mShieldPercent) {
            if (Unit *medivh = Unit::GetUnit(*player, MedivhGUID)) {
              if (medivh->IsAlive()) {
                medivh->DealDamage(medivh, medivh->GetHealth(), NULL, DIRECT_DAMAGE, SPELL_SCHOOL_MASK_NORMAL, NULL,
                                   false);
                Encounter[0] = FAIL;
    ...
    Unit *SummonedPortalBoss(Unit *source) {
      ...
      pos.m_positionZ = std::max(source->GetMap()->GetHeight(pos.m_positionX, pos.m_positionY, MAX_HEIGHT),
                                 source->GetMap()->GetWaterLevel(pos.m_positionX, pos.m_positionY));
      ...
      if (Unit *medivh = Unit::GetUnit(*player, MedivhGUID)) {
        for (uint8 i = 0; i < 4; i++) {
          ...
          if (tmp != CurrentRiftId) {
            sLog.outDebug("OSCR: Instance Dark Portal: Creating Time Rift at locationId %i (old locationId was %u).",
                          tmp, CurrentRiftId);

            CurrentRiftId = tmp;

            Unit *temp =
                medivh->SummonCreature(C_TIME_RIFT, PortalLocation[tmp][0], PortalLocation[tmp][1],
                                       PortalLocation[tmp][2], PortalLocation[tmp][3], TEMPSUMMON_CORPSE_DESPAWN, 0);
            if (temp) {
  ...
  InstanceData *GetInstanceScript(InstanceMap *pMap) const override { return new instance_dark_portalAI(pMap); }
};
void AddSC_instance_dark_portal() { new instance_dark_portal(); }
