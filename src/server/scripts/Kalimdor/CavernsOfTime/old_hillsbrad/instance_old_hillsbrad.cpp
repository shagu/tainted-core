...
static const float OrcLoc[][4] = {{2104.51f, 91.96f, 53.14f, 0}, {2192.58f, 238.44f, 52.44f, 0}};

enum Summon { NOT_SUMMONED = 0, WAIT_FOR_SUMMON = 1, SUMMONED = 2 };
...
public:
    ...
    void SetData(uint32 type, uint32 data) {
      ...
      case TYPE_BARREL_DIVERSION: {
        if (data == IN_PROGRESS) {
          ...
          if (BarrelCount == 5) {
            if (Creature *drake = player->SummonCreature(DRAKE_ENTRY, 2128.43f, 71.01f, 64.42f, 1.74f,
                                                         TEMPSUMMON_DEAD_DESPAWN, 15000))
            ...
            for (std::list<GameObject *>::iterator itr = RoaringFlamesList.begin(); itr != RoaringFlamesList.end();
                 ++itr) {
      ...
      case TYPE_THRALL_EVENT: {
        if (data == FAIL) {
          if (ThrallEventCount <= 20) {
            ...
            Encounter[7] = DONE;
            Encounter[8] = DONE;
            sLog.outDebug("TSCR: Instance Old Hillsbrad: Thrall event failed %u times. This is the end.",
                          ThrallEventCount);
          }
        } else
          Encounter[1] = data;
  ...
  InstanceData *GetInstanceScript(InstanceMap *pMap) const override { return new instance_old_hillsbradAI(pMap); }
};
