...
const Position LorgusPosition[4] = {{-458.500610f, -38.343079f, -33.474445f, 0},
...
public:
  ...
  struct instance_blackfathom_deepsAI : public ScriptedInstance {
    instance_blackfathom_deepsAI(Map *pMap) : ScriptedInstance(pMap) { Initialize(); };
    ...
    void SetData(uint32 uiType, uint32 uiData) {
      ...
      case DATA_FIRE:
        ...
        case 2:
          if (GameObject *pGO = instance->GetGameObject(m_uiShrine1GUID)) {
            for (uint8 i = 0; i < 2; ++i) {
              pGO->SummonCreature(NPC_MURKSHALLOW_SOFTSHELL, SpawnsLocation[0], TEMPSUMMON_CORPSE_TIMED_DESPAWN,
                                  300000);
              pGO->SummonCreature(NPC_MURKSHALLOW_SOFTSHELL, SpawnsLocation[1], TEMPSUMMON_CORPSE_TIMED_DESPAWN,
                                  300000);
              pGO->SummonCreature(NPC_MURKSHALLOW_SOFTSHELL, SpawnsLocation[2], TEMPSUMMON_CORPSE_TIMED_DESPAWN,
                                  300000);
              pGO->SummonCreature(NPC_MURKSHALLOW_SOFTSHELL, SpawnsLocation[3], TEMPSUMMON_CORPSE_TIMED_DESPAWN,
                                  300000);
            }
          }
          break;
  ...
  InstanceData *GetInstanceScript(InstanceMap *pMap) const override { return new instance_blackfathom_deepsAI(pMap); }
};
