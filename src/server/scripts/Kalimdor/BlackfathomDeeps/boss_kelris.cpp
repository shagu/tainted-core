...
// Id's from ACID
enum Yells { SAY_AGGRO = -1048002, SAY_SLEEP = -1048001, SAY_DEATH = -1048000 };
...
public:
  ...
  struct boss_kelrisAI : public ScriptedAI {
    boss_kelrisAI(Creature *c) : ScriptedAI(c) { pInstance = (ScriptedInstance *)c->GetInstanceData(); }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_kelrisAI(pCreature); }
};

void AddSC_boss_kelris() { new boss_kelris(); }
