...
enum eDyingKodo {
  ...
  SPELL_KODO_KOMBO_PLAYER_BUFF =
      18172, // spells here have unclear function, but using them at least for visual parts and checks
...
static float m_afAmbushSpawn[4][3] = {{-1388.37f, 2427.81f, 88.8286f},
...
public:
  ...
  struct npc_aged_dying_ancient_kodoAI : public ScriptedAI {
    ...
    void UpdateAI(const uint32 diff) {
      // timer should always be == 0 unless we already updated entry of creature. Then not expect this updated to ever
      // be in combat.
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_aged_dying_ancient_kodoAI(pCreature); }
  ...
  bool OnDummyEffect(Unit *pCaster, uint32 spellId, uint32 effIndex, Creature *pCreatureTarget) override {
    ...
    if (spellId == SPELL_KODO_KOMBO_ITEM && effIndex == 0) {
      ...
      if (pCaster->HasAura(SPELL_KODO_KOMBO_PLAYER_BUFF, 0) ||
          pCreatureTarget->HasAura(SPELL_KODO_KOMBO_DESPAWN_BUFF, 0))
        return true;

      if (pCreatureTarget->GetEntry() == NPC_AGED_KODO || pCreatureTarget->GetEntry() == NPC_DYING_KODO ||
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_dalindaAI(pCreature); }
...
public:
    ...
    void WaypointReached(uint32 uiPointId) {
      if (Player *pPlayer = GetPlayerForEscort()) {
        ...
        case 1:
          ...
          break;
        case 7:
          me->SummonCreature(NPC_MARAUDINE_SCOUT, m_afAmbushSpawn[0][0], m_afAmbushSpawn[0][1], m_afAmbushSpawn[0][2],
                             1.6f, TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 600000);
          me->SummonCreature(NPC_MARAUDINE_SCOUT, m_afAmbushSpawn[1][0], m_afAmbushSpawn[1][1], m_afAmbushSpawn[1][2],
                             1.6f, TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 600000);
          me->SummonCreature(NPC_MARAUDINE_SCOUT, m_afAmbushSpawn[2][0], m_afAmbushSpawn[2][1], m_afAmbushSpawn[2][2],
                             1.6f, TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 600000);
          me->SummonCreature(NPC_MARAUDINE_BONEPAW, m_afAmbushSpawn[3][0], m_afAmbushSpawn[3][1], m_afAmbushSpawn[3][2],
                             1.6f, TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 600000);
          break;
        case 10:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_melizza_brimbuzzleAI(pCreature); }
...
public:
  ...
  bool OnGossipHello(Player *player, GameObject *go) override {
    if (player->GetQuestStatus(QUEST_PORTAL_OF_THE_LEGION) == QUEST_STATUS_INCOMPLETE) {
      if (Creature *guardian = player->SummonCreature(NPC_DEMON_GUARDIAN, go->GetPositionX(), go->GetPositionY(),
                                                      go->GetPositionZ(), 0.0f, TEMPSUMMON_DEAD_DESPAWN, 0))
