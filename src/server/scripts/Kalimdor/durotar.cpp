...
public:
  ...
  struct npc_lazy_peonAI : public ScriptedAI {
    ...
    void SpellHit(Unit *caster, const SpellEntry *spell) {
      if (spell->Id == SPELL_AWAKEN_PEON && caster->GetTypeId() == TYPEID_PLAYER &&
        ...
        me->RemoveAllAuras();
        if (GameObject *Lumberpile = me->FindNearestGameObject(GO_LUMBERPILE, 20))
          me->GetMotionMaster()->MovePoint(1, Lumberpile->GetPositionX() - 1, Lumberpile->GetPositionY(),
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_lazy_peonAI(pCreature); }
};

void AddSC_durotar() { new npc_lazy_peon(); }
