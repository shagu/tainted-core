...
EventLocation m_afEventMoveTo[] = {{-2943.92f, -3319.41f, 29.8336f}, {-2933.01f, -3321.05f, 29.5781f}

};

enum eZelfrax { SAY_ZELFRAX = -1000605, SAY_ZELFRAX_2 = -1000606 };

enum { QUEST_NATS_BARGAIN = 11209, SPELL_FISH_PASTE = 42644, NPC_LURKING_SHARK = 23928 };
...
#define GOSSIP_ITEM_THERAMORE_GUARD                                                                                    \
  "You look like an intelligent person. Why don't you read one of these leaflets and give it some thought?"
#define GOSSIP_ITEM_JAINA                                                                                              \
  "I know this is rather silly but i have a young ward who is a bit shy and would like your autograph."
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new mobs_risen_husk_spiritAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_deserter_agitatorAI(pCreature); }
};
...
public:
  ...
  struct npc_gavis_greyshieldAI : public ScriptedAI {
    npc_gavis_greyshieldAI(Creature *pCreature) : ScriptedAI(pCreature) { Reset(); }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_gavis_greyshieldAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_theramore_guardAI(pCreature); }
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_jaina_proudmooreAI(pCreature); }
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_morokkAI(pCreature); }
...
public:
          ...
          if (m_uiGlobalTimer < uiDiff) {
            ...
            case PHASE_INTRO: {
              ...
              case 5:
                ...
                me->SummonCreature(NPC_CALDWELL, m_afSpawn[0], m_afSpawn[1], m_afSpawn[2], 0.0f,
                                   TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 300000);
                me->SummonCreature(NPC_HALLAN, m_afSpawn[0], m_afSpawn[1], m_afSpawn[2], 0.0f,
                                   TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 300000);
                me->SummonCreature(NPC_SKIRMISHER, m_afSpawn[0], m_afSpawn[1], m_afSpawn[2], 0.0f,
                                   TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 300000);
                me->SummonCreature(NPC_SKIRMISHER, m_afSpawn[0], m_afSpawn[1], m_afSpawn[2], 0.0f,
                                   TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 300000);
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_ogronAI(pCreature); }
...
public:
  ...
  struct npc_private_hendelAI : public ScriptedAI {
    ...
    void DoAttackPlayer() {
      ...
      TypeContainerVisitor<Oregon::CreatureListSearcher<Oregon::AllCreaturesOfEntryInRange>, GridTypeMapContainer>
          cSearcher(searcher);
        ...
        if (!lCreatureList.empty()) {
          ...
          for (std::list<Creature *>::iterator itr = lCreatureList.begin(); itr != lCreatureList.end(); ++itr) {
            if ((*itr)->IsAlive()) {
              ...
              EnterEvadeMode();
              (*itr)->SetWalk(false);
              (*itr)->GetMotionMaster()->MovePoint(0, m_afEventMoveTo[N].m_fX, m_afEventMoveTo[N].m_fY,
                                                   m_afEventMoveTo[N].m_fZ);
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_private_hendelAI(pCreature); }
...
public:
  ...
  struct npc_zelfraxAI : public ScriptedAI {
    npc_zelfraxAI(Creature *pCreature) : ScriptedAI(pCreature) { MoveToDock(); }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_zelfraxAI(pCreature); }
};
...
public:
  ...
  bool OnTrigger(Player *pPlayer, const AreaTriggerEntry * /*pAt*/) override {
    if (pPlayer->GetQuestStatus(QUEST_NATS_BARGAIN) == QUEST_STATUS_INCOMPLETE &&
        pPlayer->HasAura(SPELL_FISH_PASTE, 0)) {
      Creature *pShark = GetClosestCreatureWithEntry(pPlayer, NPC_LURKING_SHARK, 20.0f);

      if (!pShark)
        pShark = pPlayer->SummonCreature(NPC_LURKING_SHARK, -4246.243f, -3922.356f, -7.488f, 5.0f,
...
public:
  ...
  struct npc_stinkyAI : public npc_escortAI {
    ...
    void EnterCombat(Unit *pWho) { DoScriptText(SAY_ATTACKED_1, me, pWho); }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_stinkyAI(pCreature); }
...
public:
  ...
  CreatureAI *GetAI(Creature *creature) const { return new npc_mottled_drywallow_crocoliskAI(creature); }
};
