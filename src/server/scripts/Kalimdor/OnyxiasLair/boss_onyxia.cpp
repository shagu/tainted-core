...
static float MovementLocations[4][3] = {{-64.0523f, -213.0619f, -68.2985f},
...
public:
  ...
  struct boss_onyxiaAI : public ScriptedAI {
      ...
      if (Phase == 2) {
        if (InitialSpawn) {
          ...
          for (uint32 i = 0; i < 10; ++i) {
            ...
            Creature *Whelp =
                me->SummonCreature(CREATURE_WHELP, SpawnLocations[random][0], SpawnLocations[random][1],
                                   SpawnLocations[random][2], 0, TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, 30000);
            if (Whelp)
    ...
    void ChangePosition() {
      uint32 random = rand() % 4;
      if (random < 4)
        me->GetMotionMaster()->MovePoint(0, MovementLocations[random][0], MovementLocations[random][1],
                                         MovementLocations[random][2]);
    }
    ...
    void SummonWhelps(uint32 Phase) {
      if (Phase == 2) {
        ...
        for (uint32 i = 0; i < max; ++i) {
          ...
          Creature *Whelp = me->SummonCreature(CREATURE_WHELP, SpawnLocations[random][0], SpawnLocations[random][1],
                                               SpawnLocations[random][2], 0, TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, 30000);
          if (Whelp)
            Whelp->AI()->AttackStart(SelectUnit(SELECT_TARGET_RANDOM, 0));
        }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_onyxiaAI(pCreature); }
};

void AddSC_boss_onyxia() { new boss_onyxia(); }
