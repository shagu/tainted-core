...
public:
  ...
  struct mob_matis_the_cruelAI : public ScriptedAI {
    ...
    void Reset() { me->SetFaction(FACTION_ORGINAL); }
      ...
      for (auto itr = PlayerList.begin(); itr != PlayerList.end(); ++itr) {
        if (Player *player = itr->GetSource())
          if (me->IsWithinDistInMap(player, 20.0f) &&
              (player->GetQuestStatus(QUEST_MATIS_THE_CRUEL) == QUEST_STATUS_INCOMPLETE))
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new mob_matis_the_cruelAI(pCreature); }
};
...
const uint32 possibleSpawns[32] = {17322, 17661, 17496, 17522, 17340, 17352, 17333, 17524, 17654, 17348, 17339,
                                   17345, 17359, 17353, 17336, 17550, 17330, 17701, 17321, 17680, 17325, 17320,
                                   17683, 17342, 17715, 17334, 17341, 17338, 17337, 17346, 17344, 17327};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new mob_webbed_creatureAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_young_furbolg_shamanAI(pCreature); };
};
