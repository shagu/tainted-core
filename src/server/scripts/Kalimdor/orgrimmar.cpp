...
enum eShenthul { QUEST_SHATTERED_SALUTE = 2460 };
...
public:
        ...
        if (Reset_Timer <= diff) {
          if (Player *pPlayer = Unit::GetPlayer(*me, PlayerGUID)) {
            if (pPlayer->GetTypeId() == TYPEID_PLAYER &&
                pPlayer->GetQuestStatus(QUEST_SHATTERED_SALUTE) == QUEST_STATUS_INCOMPLETE)
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_shenthulAI(pCreature); }
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_thrall_warchiefAI(pCreature); }
