...
public:
  ...
  struct instance_razorfen_downsAI : public ScriptedInstance {
    instance_razorfen_downsAI(Map *pMap) : ScriptedInstance(pMap) { Initialize(); };
      ...
      if (uiType == DATA_GONG_WAVES) {
        ...
        case 16: {
          ...
          if (Creature *pCreature =
                  pGo->SummonCreature(uiCreature, 2502.635f, 844.140f, 46.896f, 0.633f, TEMPSUMMON_MANUAL_DESPAWN, 0)) {
            if (uiGongWaves == 10 || uiGongWaves == 1) {
              for (uint8 i = 0; i < uiSummonTimes; ++i) {
                if (Creature *pSummon =
                        pGo->SummonCreature(uiCreature, 2502.635f + float(irand(-5, 5)), 844.140f + float(irand(-5, 5)),
                                            46.896f, 0.633f, TEMPSUMMON_MANUAL_DESPAWN, 0))
                  pSummon->GetMotionMaster()->MovePoint(0, 2533.479f + float(irand(-5, 5)),
                                                        870.020f + float(irand(-5, 5)), 47.678f);
              }
            }
            pCreature->GetMotionMaster()->MovePoint(0, 2533.479 + float(irand(-5, 5)), 870.020 + float(irand(-5, 5)),
                                                    47.678);
          }
          break;
        }
  ...
  InstanceData *GetInstanceScript(InstanceMap *pMap) const override { return new instance_razorfen_downsAI(pMap); }
};
