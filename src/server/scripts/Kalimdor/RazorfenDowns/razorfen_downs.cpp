...
enum eTombCreature { SPELL_WEB = 745 };
...
public:
  ...
  struct npc_tomb_creatureAI : public ScriptedAI {
    ...
    void Reset() { uiWebTimer = urand(5000, 8000); }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_tomb_creatureAI(pCreature); }
};
