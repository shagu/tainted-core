...
public:
  ...
  bool OnGossipHello(Player *pPlayer, Creature *pCreature) {
    ...
    if (pPlayer->GetQuestStatus(8304) == QUEST_STATUS_INCOMPLETE && pCreature->GetEntry() == 15170 &&
    ...
    if (pPlayer->GetQuestStatus(8304) == QUEST_STATUS_INCOMPLETE && pCreature->GetEntry() == 15171 &&
        pPlayer->GetReqKillOrCastCurrentCount(8304, TRIGGER_RUTGAR))
...
TO DO: Dragons should use the HandleEmoteCommand(EMOTE_ONESHOT_LIFTOFF) after transformation,but for some unknown reason
it doesnt work. EndContentData */
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new mob_qiraj_war_spawnAI(pCreature); }
};
...
public:
  ...
  struct npc_anachronos_quest_triggerAI : public ScriptedAI {
    ...
    void SummonNextWave() {
      ...
      for (uint8 i = 0; i < (sizeof(SpawnLocation) / sizeof(*SpawnLocation)); ++i) {
        ...
        Spawn = me->SummonCreature(WavesInfo[WaveCount].CreatureId, X, Y, Z, O, TEMPSUMMON_TIMED_OR_DEAD_DESPAWN,
                                   desptimer);
      ...
      if (Group *EventGroup = pPlayer->GetGroup()) {
        ...
        for (Group::member_citerator itr = members.begin(); itr != members.end(); ++itr) {
          ...
          if (!GroupMember->IsWithinDistInMap(me, EVENT_AREA_RADIUS) &&
              GroupMember->GetQuestStatus(QUEST_A_PAWN_ON_THE_ETERNAL_BOARD) == QUEST_STATUS_INCOMPLETE) {
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_anachronos_quest_triggerAI(pCreature); }
};
  ...
  bool OnQuestAccept(Player *plr, GameObject *go, Quest const *quest) {
    if (quest->GetQuestId() == QUEST_A_PAWN_ON_THE_ETERNAL_BOARD) {
      ...
      if (Unit *Anachronos_Quest_Trigger = go->FindNearestCreature(15454, 100, plr)) {
        ...
        Unit *Merithra = Anachronos_Quest_Trigger->SummonCreature(15378, -8034.535f, 1535.14f, 2.61f, 0,
                                                                  TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 220000);
        Unit *Caelestrasz = Anachronos_Quest_Trigger->SummonCreature(15379, -8032.767f, 1533.148f, 2.61f, 1.5f,
                                                                     TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 220000);
        Unit *Arygos = Anachronos_Quest_Trigger->SummonCreature(15380, -8034.52f, 1537.843f, 2.61f, 5.7f,
                                                                TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 220000);
        /* Unit* Fandral = */ Anachronos_Quest_Trigger->SummonCreature(15382, -8028.462f, 1535.843f, 2.61f, 3.141592f,
                                                                       TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 220000);
        Creature *Anachronos = Anachronos_Quest_Trigger->SummonCreature(15381, -8028.75f, 1538.795f, 2.61f, 4,
                                                                        TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 220000);
        ...
        if (Anachronos) {
          CAST_AI(npc_anachronos_the_ancient::npc_anachronos_the_ancientAI, Anachronos->AI())->PlayerGUID =
              plr->GetGUID();
          CAST_AI(npc_anachronos_quest_trigger::npc_anachronos_quest_triggerAI,
                  CAST_CRE(Anachronos_Quest_Trigger)->AI())
              ->Failed = false;
          CAST_AI(npc_anachronos_quest_trigger::npc_anachronos_quest_triggerAI,
                  CAST_CRE(Anachronos_Quest_Trigger)->AI())
