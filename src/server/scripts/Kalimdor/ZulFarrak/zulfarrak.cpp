...
SDTodo: Make goblin run away. Spell for Barrel explosion needs proper implementation. Make all eventually hearth/fade
out. Bly needs proper gossip. All cages should open on opening one. This is handled by event scripts currently but it
doesn't seem to execute with a C++ script. EndScriptData */
...
enum blySays { SAY_1 = -1209002, SAY_2 = -1209003 };
...
enum { ZOMBIE = 7286, DEAD_HERO = 7276, ZOMBIE_CHANCE = 65, DEAD_HERO_CHANCE = 10 };

/*######
## at_zumrah
######*/

enum { ZUMRAH_ID = 7271, ZUMRAH_HOSTILE_FACTION = 37 };

/*######
## at_antusul
######*/

enum { ANTUSUL_ID = 8127 };
...
public:
  ...
  struct npc_sergeant_blyAI : public ScriptedAI {
    ...
    uint32 Revenge_Timer; // this is wrong, spell should never be used unless me->GetVictim() dodge, parry or block
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_sergeant_blyAI(pCreature); }
...
public:
  ...
  struct npc_weegli_blastfuseAI : public ScriptedAI {
    ...
    void DoAction(const int32 /*param*/) { DestroyDoor(); }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_weegli_blastfuseAI(pCreature); }
  ...
  bool OnGossipHello(Player * /*pPlayer*/, GameObject *pGo) override {
    ...
    if (pGo->GetUseCount() == 0) {
      ...
      if (randomchance < ZOMBIE_CHANCE)
        pGo->SummonCreature(ZOMBIE, pGo->GetPositionX(), pGo->GetPositionY(), pGo->GetPositionZ(), 0,
                            TEMPSUMMON_CORPSE_TIMED_DESPAWN, 30000);
      else if ((randomchance - ZOMBIE_CHANCE) < DEAD_HERO_CHANCE)
        pGo->SummonCreature(DEAD_HERO, pGo->GetPositionX(), pGo->GetPositionY(), pGo->GetPositionZ(), 0,
                            TEMPSUMMON_CORPSE_TIMED_DESPAWN, 30000);
    }
    pGo->AddUse();
    return false;
