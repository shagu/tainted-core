...
public:
  ...
  std::vector<ChatCommand> GetCommands() const override {
    ...
    static std::vector<ChatCommand> commandTable = {{"account", SEC_PLAYER, false, nullptr, "", accountCommandTable}};
    return commandTable;
  };
  ...
  static bool HandleAccountSet2FACommand(ChatHandler *handler, const char *args) {
    ...
    if (secret == "off") {
      LoginDatabase.PQuery("UPDATE `account` SET `token_key` = '', `security_flag` = '0' WHERE `id` = '%u';",
                           targetAccountId);
    ...
    case 6: // Pin
    {
      ...
      LoginDatabase.PQuery("UPDATE `account` SET `token_key` = '%u', `security_flag` = '1' WHERE `id` = '%u';",
                           atoi(secret.c_str()), targetAccountId);
      handler->PSendSysMessage("Account %s has been successfully updated with [PIN] \nYour pin is %u",
                               accountName.c_str(), atoi(secret.c_str()));
      break;
    }
    case 16: // TOTP
    {
      ...
      LoginDatabase.PQuery("UPDATE `account` SET `token_key` = '%s', `security_flag` = '4' WHERE `id` = '%u';",
                           secret.c_str(), targetAccountId);
  ...
  static bool HandleAccountOnlineListCommand(ChatHandler *handler, const char * /*args*/) {
    ...
    do {
      ...
      if (resultLogin) {
        ...
        handler->PSendSysMessage("|%15s| %20s | %15s |%4d|%5d|", fieldsLogin[0].GetString(), name.c_str(),
                                 fieldsLogin[1].GetString(), fieldsLogin[2].GetUInt32(), fieldsLogin[3].GetUInt32());
      } else
  ...
  static bool HandleAccountSetGmLevelCommand(ChatHandler *handler, const char *args) {
    ...
    if (handler->getSelectedPlayer() && arg1 && !arg3) {
      ...
      if (gmRealmID == uint32(-1)) {
        ...
        LoginDatabase.PExecute("DELETE FROM account_access WHERE id = '%u' AND RealmID = '%d'", targetAccountId,
                               realmID);
