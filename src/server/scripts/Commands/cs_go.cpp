...
public:
  ...
  std::vector<ChatCommand> GetCommands() const override {
    ...
    static std::vector<ChatCommand> commandTable = {{"go", SEC_MODERATOR, false, nullptr, "", goCommandTable}};
    return commandTable;
  }
  ...
  static bool HandleGoCreatureCommand(ChatHandler *handler, char const *args) {
    ...
    QueryResult *result = WorldDatabase.PQuery(
        "SELECT position_x,position_y,position_z,orientation,map FROM creature %s", whereClause.str().c_str());
    if (!result) {
  ...
  static bool HandleGoZoneXYCommand(ChatHandler *handler, char const *args) {
    ...
    char *cAreaId = handler->extractKeyFromLink(
        tail, "Harea"); // string or [name] Shift-click form |color|Harea:area_id|h[name]|h|r
    ...
    if (map->Instanceable()) {
      handler->PSendSysMessage(LANG_INVALID_ZONE_MAP, areaEntry->ID,
                               areaEntry->area_name[handler->GetSession()->GetSessionDbcLocale()], map->GetId(),
                               map->GetMapName());
      handler->SetSentErrorMessage(true);
      return false;
    }
