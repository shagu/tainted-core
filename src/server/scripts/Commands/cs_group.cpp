...
public:
  ...
  std::vector<ChatCommand> GetCommands() const override {
    ...
    static std::vector<ChatCommand> commandTable = {{"gorup", SEC_MODERATOR, true, nullptr, "", groupCommandTable}};
  ...
  static bool HandleGROUPSUMMONCommand(ChatHandler *handler, char const *args) {
    ...
    if (to_instance && (!handler->GetSession()->GetPlayer()->GetGroup() ||
                        (grp->GetLeaderGUID() != handler->GetSession()->GetPlayer()->GetGUID()) ||
