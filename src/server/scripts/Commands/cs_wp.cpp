...
public:
  ...
  std::vector<ChatCommand> GetCommands() const override {
    static std::vector<ChatCommand> wpCommandTable = {{"add", SEC_ADMINISTRATOR, false, &HandleWpAddCommand, ""},
    ...
    static std::vector<ChatCommand> commandTable = {{"wp", SEC_ADMINISTRATOR, false, nullptr, "", wpCommandTable}};
    return commandTable;
  }
  ...
  static bool HandleWpAddCommand(ChatHandler *handler, const char *args) {
    ...
    handler->PSendSysMessage("%s%s%u%s%u%s|r", "|cff00ff00", "PathID: |r|cff00ffff", pathid,
                             "|r|cff00ff00: Waypoint |r|cff00ffff", point + 1, "|r|cff00ff00 created. ");
    return true;
  } // HandleWpAddCommand
  ...
  static bool HandleWpEventCommand(ChatHandler *handler, const char *args) {
    ...
    if (show == "listid") {
      ...
      do {
        ...
        handler->PSendSysMessage(
            "|cffff33ffid:|r|cff00ffff %u|r|cff00ff00, guid: |r|cff00ffff%u|r|cff00ff00, delay: "
            "|r|cff00ffff%u|r|cff00ff00, command: |r|cff00ffff%u|r|cff00ff00, datalong: |r|cff00ffff%u|r|cff00ff00, "
            "datalong2: |r|cff00ffff%u|r|cff00ff00, datatext: |r|cff00ffff%s|r|cff00ff00, posx: "
            "|r|cff00ffff%f|r|cff00ff00, posy: |r|cff00ffff%f|r|cff00ff00, posz: |r|cff00ffff%f|r|cff00ff00, "
            "orientation: |r|cff00ffff%f|r",
    ...
    if (show == "mod") {
      ...
      if ((arg_string != "setid") && (arg_string != "delay") && (arg_string != "command") &&
          (arg_string != "datalong") && (arg_string != "datalong2") && (arg_string != "dataint") &&
          (arg_string != "posx") && (arg_string != "posy") && (arg_string != "posz") && (arg_string != "orientation")) {
      ...
      if (arg_str_2 == "setid") {
        ...
        handler->PSendSysMessage("%s%s|r|cff00ffff%u|r|cff00ff00%s|r|cff00ffff%u|r", "|cff00ff00",
                                 "Wp Event: Wypoint scipt guid: ", newid, " id changed: ", id);
        WorldDatabase.PExecuteLog("UPDATE waypoint_scripts SET id='%u' WHERE guid='%u'", newid, id);
        return true;
      } else {
        QueryResult *result = WorldDatabase.PQuery("SELECT id FROM waypoint_scripts WHERE guid='%u'", id);
        ...
        if (arg_str_2 == "posx") {
          coord = atof(arg_3);
          WorldDatabase.PExecuteLog("UPDATE waypoint_scripts SET x='%f' WHERE guid='%u'", coord, id);
          handler->PSendSysMessage("|cff00ff00Waypoint script:|r|cff00ffff %u|r|cff00ff00 position_x updated.|r", id);
          return true;
        } else if (arg_str_2 == "posy") {
          coord = atof(arg_3);
          WorldDatabase.PExecuteLog("UPDATE waypoint_scripts SET y='%f' WHERE guid='%u'", coord, id);
          handler->PSendSysMessage("|cff00ff00Waypoint script: %u position_y updated.|r", id);
          return true;
        } else if (arg_str_2 == "posz") {
          coord = atof(arg_3);
          WorldDatabase.PExecuteLog("UPDATE waypoint_scripts SET z='%f' WHERE guid='%u'", coord, id);
          handler->PSendSysMessage("|cff00ff00Waypoint script: |r|cff00ffff%u|r|cff00ff00 position_z updated.|r", id);
          return true;
        } else if (arg_str_2 == "orientation") {
          coord = atof(arg_3);
          WorldDatabase.PExecuteLog("UPDATE waypoint_scripts SET o='%f' WHERE guid='%u'", coord, id);
          handler->PSendSysMessage("|cff00ff00Waypoint script: |r|cff00ffff%u|r|cff00ff00 orientation updated.|r", id);
          return true;
        } else if (arg_str_2 == "dataint") {
          WorldDatabase.PExecuteLog("UPDATE waypoint_scripts SET %s='%u' WHERE guid='%u'", arg_2, atoi(arg_3), id);
          ...
          WorldDatabase.PExecuteLog("UPDATE waypoint_scripts SET %s='%s' WHERE guid='%u'", arg_2, arg_str_3.c_str(),
                                    id);
        }
      }
      handler->PSendSysMessage("%s%s|r|cff00ffff%u:|r|cff00ff00 %s %s|r", "|cff00ff00", "Waypoint script:", id, arg_2,
                               "updated.");
    }
    return true;
  }
  ...
  static bool HandleWpModifyCommand(ChatHandler *handler, const char *args) {
    ...
    if ((show != "delay") && (show != "action") && (show != "action_chance") && (show != "move_type") &&
    ...
    if (wpGuid)
      wpCreature = handler->GetSession()->GetPlayer()->GetMap()->GetCreature(
    ...
    if (wpCreature->GetEntry() == VISUAL_WAYPOINT) {
      ...
      if (!result) {
        ...
        result = WorldDatabase.PQuery("SELECT id, point FROM waypoint_data WHERE (abs(position_x - %f) <= %s) and "
                                      "(abs(position_y - %f) <= %s) and (abs(position_z - %f) <= %s)",
                                      wpCreature->GetPositionX(), maxDIFF, wpCreature->GetPositionY(), maxDIFF,
    ...
    if (show == "del" && target) {
      ...
      WorldDatabase.PExecuteLog("DELETE FROM waypoint_data WHERE id='%u' AND point='%u'", pathid, point);
      WorldDatabase.PExecuteLog("UPDATE waypoint_data SET point=point-1 WHERE id='%u' AND point>'%u'", pathid, point);
    ...
    if (show == "move" && target) {
      ...
      Map *map = chr->GetMap();
      {
        ...
        if (wpGuid != 0) {
          ...
          if (!wpCreature2->Create(sObjectMgr.GenerateLowGuid(HIGHGUID_UNIT), map, PHASEMASK_NORMAL, VISUAL_WAYPOINT, 0,
        ...
        WorldDatabase.PExecuteLog("UPDATE waypoint_data SET position_x = '%f',position_y = '%f',position_z = '%f' "
                                  "where id = '%u' AND point='%u'",
    ...
    if (text == 0) {
      ...
      WorldDatabase.PExecuteLog("UPDATE waypoint_data SET %s=NULL WHERE id='%u' AND point='%u'", show_str, pathid,
                                point);
    } else {
      // show_str check for present in list of correct values, no sql injection possible
      std::string text2 = text;
      WorldDatabase.escape_string(text2);
      WorldDatabase.PExecuteLog("UPDATE waypoint_data SET %s='%s' WHERE id='%u' AND point='%u'", show_str,
                                text2.c_str(), pathid, point);
    }
  ...
  static bool HandleWpShowCommand(ChatHandler *handler, const char *args) {
    ...
    if (show == "info") {
      ...
      QueryResult *result = WorldDatabase.PQuery(
          "SELECT id, point, delay, move_type, action, action_chance FROM waypoint_data WHERE wpguid = %u",
          target->GetGUIDLow());
      ...
      do {
        ...
        handler->PSendSysMessage(
            "|cff00ff00Show info: for current point: |r|cff00ffff%u|r|cff00ff00, Path ID: |r|cff00ffff%u|r", point,
            pathid);
    ...
    if (show == "on") {
      QueryResult *result = WorldDatabase.PQuery(
          "SELECT point, position_x,position_y,position_z FROM waypoint_data WHERE id = '%u'", pathid);
      ...
      QueryResult *result2 =
          WorldDatabase.PQuery("SELECT wpguid FROM waypoint_data WHERE id = '%u' and wpguid <> 0", pathid);
      ...
      if (result2) {
        ...
        do {
          ...
          Creature *pCreature = handler->GetSession()->GetPlayer()->GetMap()->GetCreature(
              MAKE_NEW_GUID(wpguid, VISUAL_WAYPOINT, HIGHGUID_UNIT));
      ...
      do {
        ...
        if (!wpCreature->Create(sObjectMgr.GenerateLowGuid(HIGHGUID_UNIT), map, chr->GetPhaseMaskForSpawn(), id, 0, x,
                                y, z, o)) {
        ...
        WorldDatabase.PExecuteLog("UPDATE waypoint_data SET wpguid = '%u' WHERE id = '%u' and point = '%u'",
    ...
    if (show == "first") {
      ...
      QueryResult *result = WorldDatabase.PQuery(
          "SELECT position_x,position_y,position_z,orientation FROM waypoint_data WHERE point='1' AND id = '%u'",
          pathid);
      if (!result) {
      ...
      if (!creature->Create(sObjectMgr.GenerateLowGuid(HIGHGUID_UNIT), map, chr->GetPhaseMaskForSpawn(), id, 0, x, y, z,
                            o)) {
        handler->PSendSysMessage(LANG_WAYPOINT_VP_NOTCREATED, id);
        delete creature;
        return false;
    ...
    if (show == "last") {
      ...
      result = WorldDatabase.PQuery(
          "SELECT position_x,position_y,position_z FROM waypoint_data WHERE point ='%u' AND id = '%u'", Maxpoint,
          pathid);
      if (!result) {
    ...
    if (show == "off") {
      ...
      do {
        ...
        Creature *creature = handler->GetSession()->GetPlayer()->GetMap()->GetCreature(
