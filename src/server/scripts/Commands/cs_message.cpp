...
public:
  ...
  static bool HandleWhispersCommand(ChatHandler *handler, const char *args) {
    if (!*args) {
      handler->PSendSysMessage(LANG_COMMAND_WHISPERACCEPTING, handler->GetSession()->GetPlayer()->isAcceptWhispers()
                                                                  ? handler->GetOregonString(LANG_ON)
                                                                  : handler->GetOregonString(LANG_OFF));
      return true;
    }
