...
public:
  ...
  std::vector<ChatCommand> GetCommands() const override {
    ...
    static std::vector<ChatCommand> commandTable = {{"lookup", SEC_MODERATOR, true, nullptr, "", lookupCommandTable}};
    return commandTable;
  }
  ...
  static bool HandleLookupPlayerAccountCommand(ChatHandler *handler, const char *args) {
    ...
    QueryResult *result =
        LoginDatabase.PQuery("SELECT id,username FROM account WHERE username = '%s'", account.c_str());
  ...
  static bool HandleLookupAreaCommand(ChatHandler *handler, const char *args) {
    ...
    for (uint32 areaflag = 0; areaflag < sAreaStore.GetNumRows(); ++areaflag) {
      ...
      if (areaEntry) {
        int loc =
            handler->GetSession() ? int(handler->GetSession()->GetSessionDbcLocale()) : sWorld.GetDefaultDbcLocale();
        ...
        if (loc < MAX_LOCALE) {
          ...
          if (handler->GetSession())
            ss << areaEntry->ID << " - |cffffffff|Harea:" << areaEntry->ID << "|h[" << name << " " << localeNames[loc]
               << "]|h|r";
          else
  ...
  static bool HandleLookupCreatureCommand(ChatHandler *handler, const char *args) {
    ...
    for (uint32 id = 0; id < sCreatureStorage.GetMaxEntry(); ++id) {
      ...
      int loc_idx =
          handler->GetSession() ? handler->GetSession()->GetSessionDbLocaleIndex() : sObjectMgr.GetDBCLocaleIndex();
      if (loc_idx >= 0) {
  ...
  static bool HandleLookupFactionCommand(ChatHandler *handler, const char *args) {
    ...
    for (uint32 id = 0; id < sFactionStore.GetNumRows(); ++id) {
      ...
      if (factionEntry) {
        ...
        if (loc < MAX_LOCALE) {
          // send faction in "id - [faction] rank reputation [visible] [at war] [own team] [unknown] [invisible]
          // [inactive]" format or              "id - [faction] [no reputation]" format
          std::ostringstream ss;
          if (handler->GetSession())
        ...
        if (il) {
          if (il->Title.size() > uint32(loc_idx) && !il->Title[loc_idx].empty()) {
            ...
            if (Utf8FitTo(title, wnamepart)) {
              ...
              if (handler->GetSession())
                handler->PSendSysMessage(LANG_QUEST_LIST_CHAT, qinfo->GetQuestId(), qinfo->GetQuestId(), title.c_str(),
                                         statusStr);
              else
      ...
      if (titleInfo) {
        ...
        if (loc < MAX_LOCALE) {
          ...
          if (handler->GetSession())
            ...
            handler->PSendSysMessage(LANG_TITLE_LIST_CONSOLE, id, titleInfo->bit_index, titleNameStr, localeNames[loc],
                                     knownStr, activeStr);
