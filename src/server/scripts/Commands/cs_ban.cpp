...
public:
  ...
  std::vector<ChatCommand> GetCommands() const override {
    ...
    static std::vector<ChatCommand> banCommandTable = {{"account", SEC_GAMEMASTER, true, &HandleBanAccountCommand, ""},
                                                       {"ip", SEC_GAMEMASTER, true, &HandleBanIPCommand, ""}};

    static std::vector<ChatCommand> commandTable = {{"ban", SEC_GAMEMASTER, true, nullptr, "", banCommandTable},
  ...
  static bool HandleBanListCharacterCommand(ChatHandler *handler, const char *args) {
    ...
    QueryResult *result = CharacterDatabase.PQuery(
        "SELECT account FROM characters WHERE name " _LIKE_ " " _CONCAT3_("'%%'", "'%s'", "'%%'"), filter.c_str());
    if (!result) {
      ...
      do {
        ...
        if (fields[1].GetUInt64() == fields[2].GetUInt64()) {
          handler->PSendSysMessage("|%-15.15s|%02d-%02d-%02d %02d:%02d|   permanent  |%-15.15s|%-15.15s|",
                                   fields[0].GetString(), aTm_ban->tm_year % 100, aTm_ban->tm_mon + 1, aTm_ban->tm_mday,
                                   aTm_ban->tm_hour, aTm_ban->tm_min, fields[3].GetString(), fields[4].GetString());
        } else {
          time_t t_unban = fields[2].GetUInt64();
          tm *aTm_unban = localtime(&t_unban);
          handler->PSendSysMessage("|%-15.15s|%02d-%02d-%02d %02d:%02d|%02d-%02d-%02d %02d:%02d|%-15.15s|%-15.15s|",
                                   fields[0].GetString(), aTm_ban->tm_year % 100, aTm_ban->tm_mon + 1, aTm_ban->tm_mday,
                                   aTm_ban->tm_hour, aTm_ban->tm_min, aTm_unban->tm_year % 100, aTm_unban->tm_mon + 1,
                                   aTm_unban->tm_mday, aTm_unban->tm_hour, aTm_unban->tm_min, fields[3].GetString(),
                                   fields[4].GetString());
        }
      } while (result->NextRow());
      handler->SendSysMessage(" ===============================================================================");
  ...
  static bool HandleBanListHelper(ChatHandler *handler, QueryResult *result) {
    ...
    if (handler->GetSession()) {
      do {
        ...
        QueryResult *banresult = LoginDatabase.PQuery("SELECT account.username FROM account,account_banned WHERE "
    ...
    // Console wide output
      ...
      do {
        ...
        QueryResult *banInfo = LoginDatabase.PQuery(
        ...
        if (banInfo) {
          ...
          do {
            ...
            if (fields2[0].GetUInt64() == fields2[1].GetUInt64()) {
              handler->PSendSysMessage("|%-15.15s|%02d-%02d-%02d %02d:%02d|   permanent  |%-15.15s|%-15.15s|",
                                       account_name.c_str(), aTm_ban->tm_year % 100, aTm_ban->tm_mon + 1,
                                       aTm_ban->tm_mday, aTm_ban->tm_hour, aTm_ban->tm_min, fields2[2].GetString(),
                                       fields2[3].GetString());
            } else {
              ...
              handler->PSendSysMessage("|%-15.15s|%02d-%02d-%02d %02d:%02d|%02d-%02d-%02d %02d:%02d|%-15.15s|%-15.15s|",
                                       account_name.c_str(), aTm_ban->tm_year % 100, aTm_ban->tm_mon + 1,
                                       aTm_ban->tm_mday, aTm_ban->tm_hour, aTm_ban->tm_min, aTm_unban->tm_year % 100,
                                       aTm_unban->tm_mon + 1, aTm_unban->tm_mday, aTm_unban->tm_hour, aTm_unban->tm_min,
  ...
  static bool HandleBanInfoIPCommand(ChatHandler *handler, const char *args) {
    ...
    QueryResult *result =
        LoginDatabase.PQuery("SELECT ip, FROM_UNIXTIME(bandate), FROM_UNIXTIME(unbandate), unbandate-UNIX_TIMESTAMP(), "
    ...
    handler->PSendSysMessage(LANG_BANINFO_IPENTRY, fields[0].GetString(), fields[1].GetString(),
                             permanent ? handler->GetOregonString(LANG_BANINFO_NEVER) : fields[2].GetString(),
                             permanent ? handler->GetOregonString(LANG_BANINFO_INFINITE)
  ...
  static bool HandleBanInfoHelper(ChatHandler *handler, uint32 accountid, char const *accountname) {
    QueryResult *result =
        LoginDatabase.PQuery("SELECT FROM_UNIXTIME(bandate), unbandate-bandate, active, unbandate,banreason,bannedby "
                             "FROM account_banned WHERE id = '%u' ORDER BY bandate ASC",
                             accountid);
    if (!result) {
    ...
    do {
      ...
      std::string bantime =
          permanent ? handler->GetOregonString(LANG_BANINFO_INFINITE) : secsToTimeString(fields[1].GetUInt64(), true);
      handler->PSendSysMessage(LANG_BANINFO_HISTORYENTRY, fields[0].GetString(), bantime.c_str(),
                               active ? handler->GetOregonString(LANG_BANINFO_YES)
                                      : handler->GetOregonString(LANG_BANINFO_NO),
                               fields[4].GetString(), fields[5].GetString());
    } while (result->NextRow());
  ...
  static bool HandleBanHelper(ChatHandler *handler, BanMode mode, const char *args) {
    ...
    switch (sWorld.BanAccount(mode, nameOrIP, duration, reason,
                              handler->GetSession() ? handler->GetSession()->GetPlayerName() : "")) {
    case BAN_SUCCESS:
      if (atoi(duration) > 0)
        handler->PSendSysMessage(LANG_BAN_YOUBANNED, nameOrIP.c_str(),
                                 secsToTimeString(TimeStringToSecs(duration), true).c_str(), reason);
      else
