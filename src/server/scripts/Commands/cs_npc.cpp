...
public:
  ...
  std::vector<ChatCommand> GetCommands() const override {
    ...
    static std::vector<ChatCommand> commandTable = {{"npc", SEC_MODERATOR, false, nullptr, "", npcCommandTable}};
    return commandTable;
  }
  ...
  static bool HandleNpcDeleteCommand(ChatHandler *handler, const char *args) {
    ...
    if (*args) {
      ...
      if (CreatureData const *cr_data = sObjectMgr.GetCreatureData(lowguid))
        unit = handler->GetSession()->GetPlayer()->GetMap()->GetCreature(
  ...
  static bool HandleNpcSetFactionPermanentCommand(ChatHandler *handler, const char *args) {
    ...
    // and DB
    WorldDatabase.PExecuteLog("UPDATE creature_template SET faction = '%u' WHERE entry = '%u'", factionId,
                              pCreature->GetEntry());
  ...
  static bool HandleNpcSetFlagCommand(ChatHandler *handler, const char *args) {
    ...
    WorldDatabase.PExecuteLog("UPDATE creature_template SET npcflag = '%u' WHERE entry = '%u'", npcFlags,
                              pCreature->GetEntry());
  ...
  static bool HandleNpcSetLinkCommand(ChatHandler *handler, const char *args) {
    ...
    handler->PSendSysMessage("LinkGUID '%u' added to creature with DBTableGUID: '%u'", linkguid,
  ...
  static bool HandleNpcAddFormationCommand(ChatHandler *handler, const char *args) {
    ...
    group_member->follow_dist = sqrtf(pow(chr->GetPositionX() - creature->GetPositionX(), int(2)) +
    ...
    WorldDatabase.PExecuteLog("INSERT INTO creature_formations(leaderGUID, memberGUID, dist, angle, groupAI) "
                              "VALUES('%u' , '%u' , '%f' , '%f' , '%u' )",
                              leaderGUID, lowguid, group_member->follow_dist, group_member->follow_angle,
                              uint32(group_member->groupAI));
  ...
  static bool HandleNpcInfoCommand(ChatHandler *handler, const char * /*args*/) {
    ...
    handler->PSendSysMessage(LANG_NPCINFO_CHAR, target->GetDBTableGUIDLow(), faction, npcflags, Entry, displayid,
                             nativeid);
    ...
    handler->PSendSysMessage(LANG_NPCINFO_FLAGS, target->GetUInt32Value(UNIT_FIELD_FLAGS),
                             target->GetUInt32Value(UNIT_DYNAMIC_FLAGS), target->GetFaction());
    handler->PSendSysMessage(LANG_COMMAND_RAWPAWNTIMES, defRespawnDelayStr.c_str(), curRespawnDelayStr.c_str());
    handler->PSendSysMessage(LANG_NPCINFO_LOOT, cInfo->lootid, cInfo->pickpocketLootId, cInfo->SkinLootId);
    handler->PSendSysMessage(LANG_NPCINFO_DUNGEON_ID, target->GetInstanceId());
    handler->PSendSysMessage(LANG_NPCINFO_POSITION, float(target->GetPositionX()), float(target->GetPositionY()),
    ...
    if (const CreatureData *const linked = target->GetLinkedRespawnCreatureData())
      if (CreatureInfo const *master = GetCreatureTemplate(linked->id))
        handler->PSendSysMessage(LANG_NPCINFO_LINKGUID, sObjectMgr.GetLinkedRespawnGuid(target->GetDBTableGUIDLow()),
                                 linked->id, master->Name);
  ...
  static bool HandleNpcNearCommand(ChatHandler *handler, char const *args) {
    ...
    QueryResult *result = WorldDatabase.PQuery(
        "SELECT guid, id, position_x, position_y, position_z, map, (POW(position_x - '%f', 2) + POW(position_y - '%f', "
        "2) + POW(position_z - '%f', 2))"
        "AS order_ FROM creature WHERE map = '%u' AND (POW(position_x - '%f', 2) + POW(position_y - '%f', 2) + "
        "POW(position_z - '%f', 2)) <= '%f' ORDER BY order_",
        player->GetPositionX(), player->GetPositionY(), player->GetPositionZ(), player->GetMapId(),
        player->GetPositionX(), player->GetPositionY(), player->GetPositionZ(), distance * distance);
  ...
  static bool HandleNpcMoveCommand(ChatHandler *handler, const char *args) {
    ...
    WorldDatabase.PExecuteLog("UPDATE creature SET position_x = '%f', position_y = '%f', position_z = '%f', "
                              "orientation = '%f' WHERE guid = '%u'",
                              x, y, z, o, lowguid);
  ...
  static bool HandleNpcAddItemCommand(ChatHandler *handler, char const *args) {
    ...
    if (!sObjectMgr.IsVendorItemValid(vendor_entry, itemId, maxcount, incrtime, extendedcost,
