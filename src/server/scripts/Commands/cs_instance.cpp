  ...
  static bool HandleInstanceListBindsCommand(ChatHandler *handler, const char * /*args*/) {
    ...
    for (uint8 i = 0; i < TOTAL_DIFFICULTIES; ++i) {
      ...
      for (Player::BoundInstancesMap::iterator itr = binds.begin(); itr != binds.end(); ++itr) {
        ...
        handler->PSendSysMessage("map: %d inst: %d perm: %s diff: %s canReset: %s TTR: %s", itr->first,
                                 save->GetInstanceId(), itr->second.perm ? "yes" : "no",
                                 save->GetDifficulty() == DIFFICULTY_NORMAL ? "normal" : "heroic",
      ...
      for (uint8 i = 0; i < TOTAL_DIFFICULTIES; i++) {
        ...
        for (Player::BoundInstancesMap::iterator itr = binds.begin(); itr != binds.end();) {
          if (itr->first != player->GetMapId()) {
            InstanceSave *save = itr->second.save;
            std::string timeleft = handler->GetTimeString(save->GetResetTime() - time(NULL));
            handler->PSendSysMessage("unbinding map: %d inst: %d perm: %s diff: %s canReset: %s TTR: %s", itr->first,
                                     save->GetInstanceId(), itr->second.perm ? "yes" : "no",
                                     save->GetDifficulty() == DIFFICULTY_NORMAL ? "normal" : "heroic",
                                     save->CanReset() ? "yes" : "no", timeleft.c_str());
