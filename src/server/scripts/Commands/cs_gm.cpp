...
public:
  ...
  std::vector<ChatCommand> GetCommands() const override {
    static std::vector<ChatCommand> gmCommandTable = {{"chat", SEC_MODERATOR, false, &HandleGMChatCommand, ""},
    ...
    static std::vector<ChatCommand> commandTable = {{"gm", SEC_MODERATOR, false, nullptr, "", gmCommandTable}};
    return commandTable;
  }
  ...
  static bool HandleGMListFullCommand(ChatHandler *handler, char const * /*args*/) {
    ...
    QueryResult *result = LoginDatabase.Query(
        "SELECT a.username,aa.gmlevel FROM account a, account_access aa WHERE a.id=aa.id AND aa.gmlevel > 0");
    if (result) {
  ...
  static bool HandleVisibleCommand(ChatHandler *handler, char const *args) {
    ...
    if (!*args) {
      handler->PSendSysMessage(LANG_YOU_ARE, _player->isGMVisible() ? handler->GetOregonString(LANG_VISIBLE)
                                                                    : handler->GetOregonString(LANG_INVISIBLE));
      return true;
    }
