...
public:
  ...
  std::vector<ChatCommand> GetCommands() const override {
    ...
    static std::vector<ChatCommand> commandTable = {{"modify", SEC_MODERATOR, false, nullptr, "", modifyCommandTable}};
    return commandTable;
  };
  ...
  static bool HandleMODReputationCommand(ChatHandler *handler, char const *args) {
    ...
    if (factionEntry->reputationListID < 0) {
      handler->PSendSysMessage(LANG_COMMAND_FACTION_NOREP_ERROR,
                               factionEntry->name[handler->GetSession()->GetSessionDbcLocale()], factionId);
      handler->SetSentErrorMessage(true);
      return false;
    }

    target->GetReputationMgr().SetReputation(factionEntry, amount);
    handler->PSendSysMessage(LANG_COMMAND_MODIFY_REP, factionEntry->name[handler->GetSession()->GetSessionDbcLocale()],
                             factionId, target->GetName(), target->GetReputationMgr().GetReputation(factionEntry));
    return true;
  }
