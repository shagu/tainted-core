...
public:
  ...
  std::vector<ChatCommand> GetCommands() const override {
    ...
    static std::vector<ChatCommand> commandTable = {{"list", SEC_MODERATOR, true, nullptr, "", listCommandTable}};
    return commandTable;
  }
  ...
  static bool HandleListCreatureCommand(ChatHandler *handler, const char *args) {
    ...
    if (handler->GetSession()) {
      ...
      result = WorldDatabase.PQuery(
          "SELECT guid, position_x, position_y, position_z, map, (POW(position_x - '%f', 2) + POW(position_y - '%f', "
          "2) + POW(position_z - '%f', 2)) AS order_ FROM creature WHERE id = '%u' ORDER BY order_ ASC LIMIT %u",
          pl->GetPositionX(), pl->GetPositionY(), pl->GetPositionZ(), cr_id, uint32(count));
    } else
      result = WorldDatabase.PQuery(
          "SELECT guid, position_x, position_y, position_z, map FROM creature WHERE id = '%u' LIMIT %u", cr_id,
          uint32(count));
  ...
  static bool HandleListItemCommand(ChatHandler *handler, const char *args) {
    ...
    if (result) {
      do {
        ...
        handler->PSendSysMessage(LANG_ITEMLIST_SLOT, item_guid, owner_name.c_str(), owner_guid, owner_acc, item_pos);
      } while (result->NextRow());
    ...
    if (count > 0) {
      result = CharacterDatabase.PQuery(
          //          0                     1            2              3               4            5               6
          "SELECT mail_items.item_guid, mail.sender, mail.receiver, char_s.account, char_s.name, char_r.account, "
          "char_r.name "
          "FROM mail,mail_items,characters as char_s,characters as char_r "
          "WHERE mail_items.item_template='%u' AND char_s.guid = mail.sender AND char_r.guid = mail.receiver AND "
          "mail.id=mail_items.mail_id LIMIT %u",
          item_id, uint32(count));
    } else
      result = nullptr;
    ...
    if (result) {
      do {
        ...
        handler->PSendSysMessage(LANG_ITEMLIST_MAIL, item_guid, item_s_name.c_str(), item_s, item_s_acc,
                                 item_r_name.c_str(), item_r, item_r_acc, item_pos);
      } while (result->NextRow());
    ...
    uint32 auc_count = 0;
    result =
        CharacterDatabase.PQuery("SELECT COUNT(item_template) FROM auctionhouse WHERE item_template='%u'", item_id);
    if (result)
    ...
    if (count > 0) {
      result = CharacterDatabase.PQuery(
          ...
          "FROM auctionhouse,characters WHERE auctionhouse.item_template='%u' AND characters.guid = "
          "auctionhouse.itemowner LIMIT %u",
          item_id, uint32(count));
    } else
      result = nullptr;
    ...
    handler->PSendSysMessage(LANG_COMMAND_LISTITEMMESSAGE, item_id, inv_count + mail_count + auc_count + guild_count,
  ...
  static bool HandleListObjectCommand(ChatHandler *handler, const char *args) {
    ...
    if (handler->GetSession()) {
      Player *pl = handler->GetSession()->GetPlayer();
      result = WorldDatabase.PQuery("SELECT guid, position_x, position_y, position_z, map, id, (POW(position_x - '%f', "
                                    "2) + POW(position_y - '%f', 2) + POW(position_z - '%f', 2)) AS order_ FROM "
                                    "gameobject WHERE id = '%u' ORDER BY order_ ASC LIMIT %u",
                                    pl->GetPositionX(), pl->GetPositionY(), pl->GetPositionZ(), go_id, uint32(count));
    } else
      result = WorldDatabase.PQuery(
          "SELECT guid, position_x, position_y, position_z, map, id FROM gameobject WHERE id = '%u' LIMIT %u", go_id,
          uint32(count));
  ...
  static bool HandleListAurasCommand(ChatHandler *handler, const char * /*args*/) {
    ...
    for (Unit::AuraMap::const_iterator itr = uAuras.begin(); itr != uAuras.end(); ++itr) {
      ...
      handler->PSendSysMessage(LANG_COMMAND_TARGET_AURADETAIL, itr->second->GetId(), itr->second->GetEffIndex(),
                               ...
                               IS_PLAYER_GUID(itr->second->GetCasterGUID()) ? "player" : "creature",
                               GUID_LOPART(itr->second->GetCasterGUID()));
    }
    for (uint16 i = 0; i < TOTAL_AURAS; ++i) {
      ...
      for (Unit::AuraList::const_iterator itr = uAuraList.begin(); itr != uAuraList.end(); ++itr) {
        ...
        handler->PSendSysMessage(LANG_COMMAND_TARGET_AURASIMPLE, (*itr)->GetId(), (*itr)->GetEffIndex(),
                                 (*itr)->GetSpellProto()->SpellName[handler->GetSession()->GetSessionDbcLocale()],
                                 ((*itr)->IsPassive() ? passiveStr : ""), (talent ? talentStr : ""),
                                 IS_PLAYER_GUID((*itr)->GetCasterGUID()) ? "player" : "creature",
                                 GUID_LOPART((*itr)->GetCasterGUID()));
      }
    }
    return true;
  }
