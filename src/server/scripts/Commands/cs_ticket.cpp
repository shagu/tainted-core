...
public:
  ...
  std::vector<ChatCommand> GetCommands() const override {
    ...
    static std::vector<ChatCommand> commandTable = {{"ticket", SEC_MODERATOR, false, nullptr, "", ticketCommandTable}};
    return commandTable;
  };
  ...
  static bool HandleGMTicketListCommand(ChatHandler *handler, char const * /*args*/) {
    ...
    for (GmTicketList::iterator itr = ticketmgr.GM_TicketList.begin(); itr != ticketmgr.GM_TicketList.end(); ++itr) {
      ...
      ss << PGetParseString(handler, LANG_COMMAND_TICKETLISTAGECREATE,
                            (secsToTimeString(time(NULL) - (*itr)->createtime, true, false)).c_str());
      ss << PGetParseString(handler, LANG_COMMAND_TICKETLISTAGE,
                            (secsToTimeString(time(NULL) - (*itr)->timestamp, true, false)).c_str());
  ...
  static bool HandleGMTicketGetByNameCommand(ChatHandler *handler, char const *args) {
    ...
    ss << PGetParseString(handler, LANG_COMMAND_TICKETLISTAGECREATE,
                          (secsToTimeString(time(NULL) - ticket->createtime, true, false)).c_str());
    ss << PGetParseString(handler, LANG_COMMAND_TICKETLISTAGE,
                          (secsToTimeString(time(NULL) - ticket->timestamp, true, false)).c_str());
