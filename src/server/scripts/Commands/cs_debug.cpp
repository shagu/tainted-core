...
public:
  ...
  std::vector<ChatCommand> GetCommands() const override {
    ...
    static std::vector<ChatCommand> commandTable = {{"debug", SEC_GAMEMASTER, false, nullptr, "", debugCommandTable}};
    return commandTable;
  }
  ...
  static bool HandleSetPoiCommand(ChatHandler *handler, const char *args) {
    ...
    pPlayer->PlayerTalkClass->SendPointOfInterest(target->GetPositionX(), target->GetPositionY(), Poi_Icon(icon), flags,
                                                  30, "Test POI");
    return true;
  }
  ...
  static bool HandleSendChatMsgCommand(ChatHandler *handler, const char *args) {
    ...
    WorldPacket data;
    ChatHandler::FillMessageData(&data, handler->GetSession(), type, 0, "chan",
                                 handler->GetSession()->GetPlayer()->GetGUID(), msg,
  ...
  static bool HandleGetItemState(ChatHandler *handler, const char *args) {
      ...
      for (uint8 i = PLAYER_SLOT_START; i < PLAYER_SLOT_END; i++) {
        ...
        if (!item->IsBag()) {
          if (item->GetState() == state)
            handler->PSendSysMessage("bag: 255 slot: %d guid: %d owner: %d", item->GetSlot(), item->GetGUIDLow(),
    ...
    if (list_queue) {
      ...
      for (size_t i = 0; i < updateQueue.size(); i++) {
        ...
        handler->PSendSysMessage("bag: %d slot: %d guid: %d - state: %s", bag_slot, item->GetSlot(), item->GetGUIDLow(),
                                 st.c_str());
      }
    ...
    if (check_all) {
      ...
      for (uint8 i = PLAYER_SLOT_START; i < PLAYER_SLOT_END; i++) {
        ...
        if (item->GetSlot() != i) {
          handler->PSendSysMessage("item at slot %d, guid %d has an incorrect slot value: %d", i, item->GetGUIDLow(),
                                   item->GetSlot());
          error = true;
          continue;
        }
        ...
        if (item->GetOwnerGUID() != player->GetGUID()) {
          handler->PSendSysMessage(
              ...
              item->GetSlot(), item->GetGUIDLow(), GUID_LOPART(item->GetOwnerGUID()), player->GetGUIDLow());
          error = true;
          continue;
        }

        if (Bag *container = item->GetContainer()) {
          handler->PSendSysMessage("item at slot: %d guid: %d has a container (slot: %d, guid: %d) but shouldnt!",
                                   item->GetSlot(), item->GetGUIDLow(), container->GetSlot(), container->GetGUIDLow());
          error = true;
          continue;
        }
        ...
        if (item->IsInUpdateQueue()) {
          ...
          if (qp > updateQueue.size()) {
            handler->PSendSysMessage(
                "item at slot: %d guid: %d has a queuepos (%d) larger than the update queue size! ", item->GetSlot(),
                item->GetGUIDLow(), qp);
            error = true;
            continue;
          }
          ...
          if (updateQueue[qp] == NULL) {
            handler->PSendSysMessage("item at slot: %d guid: %d has a queuepos (%d) that points to NULL in the queue!",
          ...
          if (updateQueue[qp] != item) {
            handler->PSendSysMessage("item at slot: %d guid: %d has has a queuepos (%d) that points to another item in "
                                     "the queue (bag: %d, slot: %d, guid: %d)",
                                     item->GetSlot(), item->GetGUIDLow(), qp, updateQueue[qp]->GetBagSlot(),
                                     updateQueue[qp]->GetSlot(), updateQueue[qp]->GetGUIDLow());
            error = true;
            continue;
          }
        } else if (item->GetState() != ITEM_UNCHANGED) {
          handler->PSendSysMessage("item at slot: %d guid: %d is not in queue but should be (state: %d)!",
                                   item->GetSlot(), item->GetGUIDLow(), item->GetState());
          error = true;
          continue;
        }
        ...
        if (item->IsBag()) {
          ...
          for (uint8 j = 0; j < bag->GetBagSize(); ++j) {
            ...
            if (item->GetSlot() != j) {
              handler->PSendSysMessage("the item in bag %d slot %d, guid %d has an incorrect slot value: %d",
                                       bag->GetSlot(), j, item->GetGUIDLow(), item->GetSlot());
              error = true;
              continue;
            }

            if (item->GetOwnerGUID() != player->GetGUID()) {
              handler->PSendSysMessage("for the item in bag %d at slot %d and itemguid %d, owner's guid (%d) and "
                                       "player's guid (%d) don't match!",
                                       bag->GetSlot(), item->GetSlot(), item->GetGUIDLow(),
                                       GUID_LOPART(item->GetOwnerGUID()), player->GetGUIDLow());
              error = true;
              continue;
            }
            ...
            Bag *container = item->GetContainer();
            if (!container) {
              handler->PSendSysMessage("the item in bag %d at slot %d with guid %d has no container!", bag->GetSlot(),
            ...
            if (container != bag) {
              handler->PSendSysMessage(
                  ...
                  bag->GetSlot(), item->GetSlot(), item->GetGUIDLow(), container->GetSlot(), container->GetGUIDLow());
              error = true;
              continue;
            }
            ...
            if (item->IsInUpdateQueue()) {
              ...
              if (qp > updateQueue.size()) {
                handler->PSendSysMessage(
                    ...
                    bag->GetSlot(), item->GetSlot(), item->GetGUIDLow(), qp);
                error = true;
                continue;
              }
              ...
              if (updateQueue[qp] != item) {
                handler->PSendSysMessage("item in bag: %d at slot: %d guid: %d has has a queuepos (%d) that points to "
                                         "another item in the queue (bag: %d, slot: %d, guid: %d)",
                                         bag->GetSlot(), item->GetSlot(), item->GetGUIDLow(), qp,
                                         updateQueue[qp]->GetBagSlot(), updateQueue[qp]->GetSlot(),
              ...
              handler->PSendSysMessage(
                  "item in bag: %d at slot: %d guid: %d is not in queue but should be (state: %d)!", bag->GetSlot(),
                  item->GetSlot(), item->GetGUIDLow(), item->GetState());
              error = true;
              continue;
            }
      ...
      for (size_t i = 0; i < updateQueue.size(); i++) {
        ...
        if (item->GetOwnerGUID() != player->GetGUID()) {
          handler->PSendSysMessage(
              "queue(%lu): for the an item (guid %d), the owner's guid (%d) and player's guid (%d) don't match!", i,
              item->GetGUIDLow(), GUID_LOPART(item->GetOwnerGUID()), player->GetGUIDLow());
          error = true;
          continue;
        }
        ...
        if (item->GetQueuePos() != i) {
          handler->PSendSysMessage(
              "queue(%lu): for the an item (guid %d), the queuepos doesn't match it's position in the queue!", i,
              item->GetGUIDLow());
          error = true;
          continue;
        }
        ...
        if (test == NULL) {
          handler->PSendSysMessage("queue(%lu): the bag(%d) and slot(%d) values for the item with guid %d are "
                                   "incorrect, the player doesn't have an item at that position!",
                                   i, item->GetBagSlot(), item->GetSlot(), item->GetGUIDLow());
          error = true;
          continue;
        }
        ...
        if (test != item) {
          handler->PSendSysMessage("queue(%lu): the bag(%d) and slot(%d) values for the item with guid %d are "
                                   "incorrect, the item with guid %d is there instead!",
                                   i, item->GetBagSlot(), item->GetSlot(), item->GetGUIDLow(), test->GetGUIDLow());
          error = true;
          continue;
        }
      }
  ...
  static bool HandleMod32Value(ChatHandler *handler, const char *args) {
    ...
    if (Opcode >= handler->GetSession()->GetPlayer()->GetValuesCount()) {
      handler->PSendSysMessage(LANG_TOO_BIG_INDEX, Opcode, handler->GetSession()->GetPlayer()->GetGUIDLow(),
  ...
  static bool HandleGetLootRecipient(ChatHandler *handler, const char * /*args*/) {
    ...
    handler->PSendSysMessage("loot recipient: %s",
  ...
  static bool HandleDebugThreatList(ChatHandler *handler, const char * /*args*/) {
    ...
    for (itr = tlist.begin(); itr != tlist.end(); ++itr) {
      ...
      if (!unit)
        continue;
      ++cnt;
      handler->PSendSysMessage("   %u.   %s   (guid %u)  - threat %f", cnt, unit->GetName(), unit->GetGUIDLow(),
                               (*itr)->getThreat());
    }
  ...
  static bool HandleSpellCrashTestCommand(ChatHandler *handler, const char * /*args*/) {
    ...
    for (uint32 i = 31000; i <= 53085; ++i) {
      if (SpellEntry const *spellInfo = sSpellStore.LookupEntry(i)) {
        ...
        Creature *trigger = player->SummonTrigger(player->GetPositionX(), player->GetPositionY(),
                                                  player->GetPositionZ(), player->GetOrientation(), 600, NULL);
  ...
  static bool HandleDebugAnimationCommand(ChatHandler *handler, const char *args) {
    ...
    data << (handler->GetSession()->GetPlayer()->GetSelection() ? handler->GetSession()->GetPlayer()->GetSelection()
    ...
    if (CinematicSequencesEntry const *cineSeq = sCinematicSequencesStore.LookupEntry(dwId)) {
      ...
      if (itr != sFlyByCameraStore.end()) {
        ...
        for (FlyByCamera cam : itr->second) {
          handler->PSendSysMessage("%02u - %7ums [%f, %f, %f] Facing %f (%f degrees)", count, cam.timeStamp,
