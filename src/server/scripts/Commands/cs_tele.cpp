...
public:
  ...
  std::vector<ChatCommand> GetCommands() const override {
    ...
    static std::vector<ChatCommand> commandTable = {{"tele", SEC_MODERATOR, false, nullptr, "", teleCommandTable}};
    return commandTable;
  }
  ...
  static bool HandleNameTeleCommand(ChatHandler *handler, const char *args) {
    ...
    if (target) {
      ...
      handler->PSendSysMessage(LANG_TELEPORTING_TO, name.c_str(), handler->GetOregonString(LANG_OFFLINE),
                               tele->name.c_str());
