...
static const float Pos_X[4] = {112.8827f, 107.8827f, 122.8827f, 127.8827f};
...
struct boss_hexlord_addAI : public ScriptedAI {
  ...
  boss_hexlord_addAI(Creature *c) : ScriptedAI(c) { pInstance = (ScriptedInstance *)c->GetInstanceData(); }
...
public:
      ...
      if (DrainPower_Timer <= diff) {
        ...
        for (Map::PlayerList::const_iterator i = PlayerList.begin(); i != PlayerList.end(); ++i) {
          if (Player *i_pl = i->GetSource())
            if (i_pl->IsAlive())
              me->AddAura(44132, me); //+1% Damage for each active player on boss (+ActivePlayer_Stack)
        }
        // me->AddAura(44132, me);
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_hex_lord_malacrassAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_thurgAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_gazakrothAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_lord_raadanAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_darkheartAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_slitherAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_fenstalkerAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_koraggAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_alyson_antilleAI(pCreature); }
};
