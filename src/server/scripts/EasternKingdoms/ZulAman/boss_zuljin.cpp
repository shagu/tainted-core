...
static SpiritInfoStruct SpiritInfo[] = {{23878, 147.87f, 706.51f, 45.11f, 3.04f},
...
static TransformStruct Transform[] = {
    ...
    {SOUND_TRANSFORM_TO_DRAGONHAWK, YELL_TRANSFORM_TO_DRAGONHAWK, SPELL_SHAPE_OF_THE_DRAGONHAWK,
...
public:
  ...
  struct boss_zuljinAI : public ScriptedAI {
    boss_zuljinAI(Creature *c) : ScriptedAI(c), Summons(me) { pInstance = (ScriptedInstance *)c->GetInstanceData(); }
    ...
    void SpawnAdds() {
      ...
      for (uint8 i = 0; i < 4; ++i) {
        pCreature = me->SummonCreature(SpiritInfo[i].entry, SpiritInfo[i].x, SpiritInfo[i].y, SpiritInfo[i].z,
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_zuljinAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new feather_vortexAI(pCreature); }
};
