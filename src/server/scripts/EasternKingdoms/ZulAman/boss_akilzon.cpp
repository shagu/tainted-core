...
public:
  ...
  struct boss_akilzonAI : public ScriptedAI {
    ...
    void HandleStormSequence(Unit *Cloud) // 1: begin, 2-9: tick, 10: end
    {
      if (StormCount < 10 && StormCount > 1) {
...

        {
          ...
          TypeContainerVisitor<Oregon::UnitListSearcher<Oregon::AnyAoETargetUnitInObjectRangeCheck>,
    ...
    void UpdateAI(const uint32 diff) {
      ...
      if (StaticDisruption_Timer <= diff) {
        ...
        /*if (float dist = me->IsWithinDist3d(pTarget->GetPositionX(), pTarget->GetPositionY(),
        pTarget->GetPositionZ(), 5.0f) dist = 5.0f; SDisruptAOEVisual_Timer = 1000 + floor(dist / 30 * 1000.0f);*/
      } else
      ...
      if (CallLighting_Timer <= diff) {
        ...
        CallLighting_Timer =
            (diff - CallLighting_Timer) + (12 + rand() % 5) * 1000; // totaly random timer. can't find any info on this
      } else
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_akilzonAI(pCreature); }
};
...
public:
  ...
  // Soaring Eagle
  CreatureAI *GetAI(Creature *pCreature) const { return new mob_soaring_eagleAI(pCreature); }
};
