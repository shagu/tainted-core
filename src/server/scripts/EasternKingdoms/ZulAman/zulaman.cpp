...
static char SAY_HARRISON_UNK0[] =
    "Puzzling. They're clearly harnessing great power from their sacrifices. But for what?";
static char YELL_HARRISON_FOLLOWME[] =
    "Suit yourself. At least five of you must assist me if we're to get inside. Follow me...";
static char YELL_HARRISON_UNK1[] = "According to my calculations, if enough of us bang the gong at once the seal on "
                                   "these doors will break and we can enter.";
static char YELL_HARRISON_UNK2[] = "I've researched this site extensively and I won't allow any dim-witted treasure "
                                   "hunters to swoop in and steal what belongs in a museum. I'll lead this charge.";
...
public:
  ...
  struct npc_forest_frogAI : public ScriptedAI {
    npc_forest_frogAI(Creature *c) : ScriptedAI(c) { pInstance = (ScriptedInstance *)c->GetInstanceData(); }
    ...
    void DoSpawnRandom() {
      if (pInstance) {
        ...
        if (cEntry == 24408)
          pInstance->SetData(ENCOUNTER_RAND_VENDOR_1, DONE);
        if (cEntry == 24409)
          pInstance->SetData(ENCOUNTER_RAND_VENDOR_2, DONE);
      }
    }

    void SpellHit(Unit *caster, const SpellEntry *spell) {
      if (spell->Id == SPELL_REMOVE_AMANI_CURSE && caster->GetTypeId() == TYPEID_PLAYER &&
          me->GetEntry() == ENTRY_FOREST_FROG) {
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_forest_frogAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_zulaman_hostageAI(pCreature); }
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_harrison_jonesAI(pCreature); }
