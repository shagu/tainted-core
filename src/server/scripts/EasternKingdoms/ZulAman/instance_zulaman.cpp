...
public:
  ...
  struct instance_zulamanAI : public ScriptedInstance {
    instance_zulamanAI(Map *pMap) : ScriptedInstance(pMap) { Initialize(); };
    ...
    void SummonHostage(uint8 num) {
      ...
      if (Player *i_pl = i->GetSource()) {
        if (Unit *Hostage = i_pl->SummonCreature(HostageInfo[num].npc, HostageInfo[num].x, HostageInfo[num].y,
                                                 HostageInfo[num].z, HostageInfo[num].o, TEMPSUMMON_DEAD_DESPAWN, 0)) {
  ...
  InstanceData *GetInstanceScript(InstanceMap *pMap) const override { return new instance_zulamanAI(pMap); }
};
