...
public:
        ...
        if (Phase == PHASE_HUMAN) {
          if (CheckTimer <= diff) {
            if (((me->GetHealth() * 100) / me->GetMaxHealth() <= 20) /*me->GetHealth() * 10 < me->GetMaxHealth()*/)
              ...
              if (Lynx && ((Lynx->GetHealth() * 100) / Lynx->GetMaxHealth() <=
                           20) /*Lynx->GetHealth() * 10 < Lynx->GetMaxHealth()*/)
                EnterPhase(PHASE_MERGE);
            }
            CheckTimer = 1000;
          } else
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_halazziAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_spiritlynxAI(pCreature); }
};
