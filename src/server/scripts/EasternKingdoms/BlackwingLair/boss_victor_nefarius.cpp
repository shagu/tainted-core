...
public:
  ...
  struct boss_victor_nefariusAI : public ScriptedAI {
    ...
    void Reset() {
      ...
      FearTimer = 8000;
      ResetTimer = 900000; // On official it takes him 15 minutes(900 seconds) to reset. We are only doing 1 minute to
                           // make testing easier
      NefarianGUID = 0;
      NefCheckTime = 2000;
    ...
    void BeginEvent(Player *pTarget) {
      ...
      for (i = MapManager::Instance().GetMap(me->GetMapId(), me)->GetPlayers().begin(); i !=
      MapManager::Instance().GetMap(me->GetMapId(), me)->GetPlayers().end(); ++i)
      {
      AttackStart((*i));
      }
      */
    ...
    void UpdateAI(const uint32 diff) {
      ...
      if (SpawnedAdds < 42) {
        ...
        if (AddSpawnTimer <= diff) {
          ...
          Spawned = me->SummonCreature(CreatureID, ADD_X1, ADD_Y1, ADD_Z1, 5.000f,
          ...
          Spawned = me->SummonCreature(CreatureID, ADD_X2, ADD_Y2, ADD_Z2, 5.000f,
                                       TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 5000);
          pTarget = SelectTarget(SELECT_TARGET_RANDOM, 0, 100, true);
          if (pTarget && Spawned) {
            Spawned->AI()->AttackStart(pTarget);
          ...
          if (SpawnedAdds >= 42) {
            ...
            Creature *Nefarian = me->SummonCreature(CREATURE_NEFARIAN, NEF_X, NEF_Y, NEF_Z, 0,
            ...
            if (pTarget && Nefarian) {
              ...
              sLog.outError("OSCR: Blackwing Lair: Unable to spawn nefarian properly.");
          }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_victor_nefariusAI(pCreature); }
...
void AddSC_boss_victor_nefarius() { new boss_victor_nefarius(); }
