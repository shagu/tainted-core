...
enum Emotes { EMOTE_FRENZY = -1469002, EMOTE_SHIMMER = -1469003 };
...
enum Spells {
  ...
  SPELL_FROSTBURN = 23187,     // Frost burn 23187, 23189
                           // Brood Affliction 23173 - Scripted Spell that cycles through all targets within 100 yards
                           // and has a chance to cast one of the afflictions on them Since Scripted spells arn't coded
...
public:
  ...
  struct boss_chromaggusAI : public ScriptedAI {
    ...
    void UpdateAI(const uint32 diff) {
      ...
      if (Shimmer_Timer <= diff) {
        ...
        uint32 spell = RAND(SPELL_FIRE_VULNERABILITY, SPELL_FROST_VULNERABILITY, SPELL_SHADOW_VULNERABILITY,
        ...
        for (ThreatContainer::StorageType::const_iterator i = threatlist.begin(); i != threatlist.end(); ++i) {
          ...
          if ((*i) && (*i)->GetSource()) {
            ...
            if (pUnit) {
              // Cast affliction
              DoCast(pUnit,
                     RAND(SPELL_BROODAF_BLUE, SPELL_BROODAF_BLACK, SPELL_BROODAF_RED, SPELL_BROODAF_BRONZE,
                          SPELL_BROODAF_GREEN),
                     true);

              // Chromatic mutation if target is effected by all afflictions
              if (pUnit->HasAura(SPELL_BROODAF_BLUE, 0) && pUnit->HasAura(SPELL_BROODAF_BLACK, 0) &&
                  pUnit->HasAura(SPELL_BROODAF_RED, 0) && pUnit->HasAura(SPELL_BROODAF_BRONZE, 0) &&
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_chromaggusAI(pCreature); }
};

void AddSC_boss_chromaggus() { new boss_chromaggus(); }
