...
public:
  ...
  struct npc_astor_hadrenAI : public ScriptedAI {
    ...
    void Reset() { me->SetFaction(68); }

    void EnterCombat(Unit * /*who*/) {}

    void JustDied(Unit * /*who*/) { me->SetFaction(68); }
  };

  CreatureAI *GetAI(Creature *pCreature) const { return new npc_astor_hadrenAI(pCreature); }
...
public:
  ...
  struct npc_deathstalker_erlandAI : public npc_escortAI {
    ...
    void EnterCombat(Unit *who) { DoScriptText(RAND(SAY_AGGRO_1, SAY_AGGRO_2, SAY_AGGRO_3), me, who); }
  };
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_deathstalker_erlandAI(pCreature); }
};
...
public:
  ...
  struct pyrewood_ambushAI : public ScriptedAI {
    pyrewood_ambushAI(Creature *c) : ScriptedAI(c), Summons(me) { QuestInProgress = false; }
    ...
    void SummonCreatureWithRandomTarget(uint32 creatureId, int position) {
      if (Creature *pSummoned =
              me->SummonCreature(creatureId, PyrewoodSpawnPoints[position][0], PyrewoodSpawnPoints[position][1],
                                 PyrewoodSpawnPoints[position][2], PyrewoodSpawnPoints[position][3],
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new pyrewood_ambushAI(pCreature); }

  bool OnQuestAccept(Player *pPlayer, Creature *pCreature, const Quest *pQuest) override {
    if (pQuest->GetQuestId() == QUEST_PYREWOOD_AMBUSH &&
        !CAST_AI(pyrewood_ambushAI, pCreature->AI())->QuestInProgress) {
