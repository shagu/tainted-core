  ...
  struct boss_garrAI : public ScriptedAI {
    ...
    void UpdateAI(const uint32 diff) {
      ...
      if (magmaShacklesTimer <= diff) {
        ...
        magmaShacklesTimer -= diff;

      DoMeleeAttackIfReady();
    }
  };
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_garrAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new mob_fireswornAI(pCreature); }
};
