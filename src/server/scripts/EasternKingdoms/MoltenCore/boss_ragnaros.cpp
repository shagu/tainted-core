...
public:
          ...
          for (uint8 i = 0; i < 9; ++i) {
            if (Unit *pTarget = SelectUnit(SELECT_TARGET_RANDOM, 0)) {
              if (Creature *pSummoned =
                      me->SummonCreature(12143, pTarget->GetPositionX(), pTarget->GetPositionY(),
                                         pTarget->GetPositionZ(), 0.0f, TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, 900000))
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_ragnarosAI(pCreature); }
};

void AddSC_boss_ragnaros() { new boss_ragnaros(); }
