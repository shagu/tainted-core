...
enum Factions { FACTION_ALLIANCE = 1690, FACTION_HORDE = 1689 };
...
enum Spells {
  ...
  SPELL_MOVE_2 = 37388,       // spell which selects AI move square (for long range pieces)
                        // SPELL_MOVE_PAWN = 37146,                 // individual move spells (used only by controlled
                        // npcs) SPELL_MOVE_KNIGHT = 37144, SPELL_MOVE_QUEEN = 37148, SPELL_MOVE_ROCK = 37151,
...
public:
            ...
            if (spellid && !Targets.empty()) {
              for (std::list<Unit *>::iterator itr = Targets.begin(); itr != Targets.end(); itr++) {
                if (spellid == SPELL_FURY_OF_MEDIVH_HORDE) {
                  Unit *summon =
                      me->SummonCreature(NPC_FURY_MEDIVH_VISUAL, (*itr)->GetPositionX(), (*itr)->GetPositionY(),
                                         (*itr)->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN, 60000);
                  summon->SetFaction(GetPlayerControlledFaction() == FACTION_ALLIANCE ? FACTION_HORDE
                                                                                      : FACTION_ALLIANCE);
  ...
  bool OnGossipSelect(Player *player, Creature *_Creature, uint32 sender, uint32 action) override {
    if (action == GOSSIP_ACTION_INFO_DEF + 1) {
      ...
      if (pInstance) {
        ...
        npc_echo_of_medivh::npc_echo_of_medivhAI *medivhAI =
            (npc_echo_of_medivh::npc_echo_of_medivhAI *)GetMedivhAI(pInstance, _Creature);
        if (medivhAI)
  ...
  CreatureAI *GetAI(Creature *_Creature) const { return GetInstanceAI<npc_echo_of_medivhAI>(_Creature); }
};
...
public:
  ...
  bool OnGossipHello(Player *player, Creature *_Creature) override {
    ...
    if (pInstance->GetData(TYPE_CHESS) != IN_PROGRESS && _Creature->GetEntry() != NPC_KING_H &&
        _Creature->GetEntry() != NPC_KING_A)
      CanControl = false;
    ...
    if (action == GOSSIP_ACTION_INFO_DEF + 1) {
      ...
      if (pInstance && _Creature) {
        ...
        if ((_Creature->GetEntry() == NPC_KING_A || _Creature->GetEntry() == NPC_KING_H) &&
            pInstance->GetData(TYPE_CHESS) == NOT_STARTED) {
          npc_echo_of_medivh::npc_echo_of_medivhAI *medivhAI =
              (npc_echo_of_medivh::npc_echo_of_medivhAI *)GetMedivhAI(pInstance, _Creature);
          if (medivhAI) {
  ...
  struct chess_npcAI : public Scripted_NoMovementAI {
    ...
    void JustDied(Unit *Killer) {
      ...
      if (me->GetEntry() == NPC_KING_H || me->GetEntry() == NPC_KING_A) {
        ...
        Creature *summon = Killer->SummonCreature(NPC_VICTORY_CONTROLER, me->GetPositionX(), me->GetPositionY(),
                                                  me->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN, 60000);

        npc_echo_of_medivh::npc_echo_of_medivhAI *medivhAI =
            (npc_echo_of_medivh::npc_echo_of_medivhAI *)GetMedivhAI(pInstance, me);
        if (medivhAI)
    ...
    void JustRespawned() {
      ...
      if (pInstance->GetData(TYPE_CHESS) == IN_PROGRESS) {
        ...
        me->GetMap()->CreatureRelocation(me, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(),
                                         me->GetOrientation());
    ...
    Unit *FindNearestEnemyChessPiece() {
      ...
      std::list<Creature *> unitList =
          npcFaction == FACTION_ALLIANCE ? medivhAI->AllianceChessPieces : medivhAI->HordeChessPieces;
      for (std::list<Creature *>::iterator itr = unitList.begin(); itr != unitList.end(); itr++) {
        if ((!target || me->GetDistance(*itr) < distance) &&
            !(*itr)->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE)) {
          target = *itr;
      ...
      for (std::list<uint64>::iterator itr = medivhAI->ControlledCreatureGuid.begin();
           itr != medivhAI->ControlledCreatureGuid.end(); itr++) {
        Unit *unit = Unit::GetUnit(*me, *itr);
        if (unit) {
          if (medivhAI->GetPlayerControlledFaction() == npcFaction) {
            if ((!target || me->GetDistance(unit) < distance) &&
    ...
    Unit *FindFriendlyChessPieceForHeal(uint32 hpdiv) {
      npc_echo_of_medivh::npc_echo_of_medivhAI *medivhAI =
          (npc_echo_of_medivh::npc_echo_of_medivhAI *)GetMedivhAI(pInstance, me);
      std::list<Creature *> unitList =
          me->GetFaction() == FACTION_ALLIANCE ? medivhAI->AllianceChessPieces : medivhAI->HordeChessPieces;
      for (std::list<Creature *>::iterator itr = unitList.begin(); itr != unitList.end(); itr++) {
        if ((*itr)->IsAlive() && (*itr)->GetMaxHealth() - (*itr)->GetHealth() >= hpdiv &&
            (*itr)->GetFaction() == me->GetFaction())
          return (*itr);
      }
      return NULL;
    }
            ...
            if (proto->SchoolMask != SPELL_SCHOOL_MASK_NORMAL) {
              if (EffectRange == sSpellRangeStore.LookupEntry(proto->rangeIndex)) {
                if (proto->Effect[effIdx] != 0 && proto->rangeIndex != 13 &&
                    me->GetDistance(target) <= EffectRange->maxRange)
                  return true;
    ...
    void HandleCastHeal(Unit *who) {
      if (me->GetEntry() == NPC_BISHOP_A || me->GetEntry() == NPC_BISHOP_H) {
        ...
        if (who) {
          me->CastSpell(who, m_spells[0].m_spellEntry,
                        false); // heal is 3rd spell so 0 in our store (leftout move and face)
    ...
    Unit *FindChessFieldForMovement(Unit *target) {
      ...
      float SpellRange = sSpellRangeStore.LookupEntry(MoveSpell->rangeIndex)->maxRange -
                         sSpellRadiusStore.LookupEntry(MoveSpell->EffectRadiusIndex[0])->radiusHostile;
      Unit *victim = NULL;
      float distance = 100.0f;

      // check black fields first
      for (std::list<Unit *>::iterator itr = medivhAI->BlackFieldList.begin(); itr != medivhAI->BlackFieldList.end();
           itr++) {
        if ((*itr)->GetDistance(me) <= SpellRange && (*itr)->GetDistance(target) < distance &&
            !(*itr)->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_REMOVE_CLIENT_CONTROL)) {
          victim = (*itr);
      ...
      for (std::list<Unit *>::iterator itr = medivhAI->WhiteFieldList.begin(); itr != medivhAI->WhiteFieldList.end();
           itr++) {
        if ((*itr)->GetDistance(me) <= SpellRange && (*itr)->GetDistance(target) < distance &&
            !(*itr)->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_REMOVE_CLIENT_CONTROL)) {
          victim = (*itr);
          ...
          if (target)                                  // found target
          {
            if (IsWithinSpellRange(target) && !SpecialAI()) // do cast spell
            {
              ...
              if (me->GetDistance(target) < 3 && !me->ToCreature()->HasSpellCooldown(SPELL_CHANGE_FACING)) {
                me->CastSpell(target->GetPositionX(), target->GetPositionY(), target->GetPositionZ(),
                              SPELL_CHANGE_FACING, false); // target nearby enemy
            ...
            // move only if medivh controlled faction (player pieces don't move by AI)
              ...
              if (urand(0, 100) < 20) // 20% Chance to move
              {
                ...
                if (ChessField) {
                  if (!me->ToCreature()->HasSpellCooldown(me->m_spells[0]) &&
                      ChessField->GetDistance(target) < me->GetDistance(target)) {
                    me->CastSpell(ChessField->GetPositionX(), ChessField->GetPositionY(), ChessField->GetPositionZ(),
                                  me->m_spells[0], false);
                  }
                }
              }
  ...
  CreatureAI *GetAI(Creature *_Creature) const { return GetInstanceAI<chess_npcAI>(_Creature); }
};
...
public:
  ...
  struct Move_markerAI : public ScriptedAI {
    ...
    void UpdateAI(const uint32 diff) {
      ...
      if (check_timer < diff) {
        if (me->IsSummon() && (!CAST_SUM(me)->GetSummoner()->IsAlive() ||
                               CAST_SUM(me)->GetSummoner()->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE)))
          CAST_SUM(me)->UnSummon();
    ...
    void TriggerUnsummon() { unsummon = true; }
  ...
  CreatureAI *GetAI(Creature *_Creature) const { return GetInstanceAI<Move_markerAI>(_Creature); }
};
...
public:
  ...
  struct chess_victory_controlerAI : ScriptedAI {
    ...
    void LoadListForDespawn() {
      ...
      for (std::list<Creature *>::iterator itr = medivhAI->AllianceChessPieces.begin();
           itr != medivhAI->AllianceChessPieces.end(); itr++)
        pieceList.push_back((*itr));

      for (std::list<Creature *>::iterator itr = medivhAI->HordeChessPieces.begin();
           itr != medivhAI->HordeChessPieces.end(); itr++)
        pieceList.push_back((*itr));
  ...
  CreatureAI *GetAI(Creature *_Creature) const { return GetInstanceAI<chess_victory_controlerAI>(_Creature); }
};
