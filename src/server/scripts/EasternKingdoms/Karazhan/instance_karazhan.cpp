...
public:
  ...
  struct instance_karazhanAI : public ScriptedInstance {
    instance_karazhanAI(Map *pMap) : ScriptedInstance(pMap) { Initialize(); }
    ...
    void SetData(uint32 type, uint32 uiData) override {
      ...
      case TYPE_CHESS:
        ...
        else if (GameObject *door = instance->GetGameObject(GetData64(DATA_GO_GAME_EXIT_DOOR)))
    ...
    std::string GetSaveData() override {
      ...
      stream << "K Z "
             << " " << m_auiEncounter[0] << " " << m_auiEncounter[1] << " " << m_auiEncounter[2] << " "
    ...
    void Load(const char *in) override {
      ...
      stream >> dataHead1 >> dataHead2 >> m_auiEncounter[0] >> m_auiEncounter[1] >> m_auiEncounter[2] >>
  ...
  InstanceData *GetInstanceScript(InstanceMap *pMap) const override { return new instance_karazhanAI(pMap); }
};

void AddSC_instance_karazhan() { new instance_karazhan(); }
