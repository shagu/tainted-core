...
float Locations[4][3] = {{-10987.086f, -1884.002f, 1.352331f},
...
public:
  ...
  struct boss_moroesAI : public ScriptedAI {
    ...
    void KilledUnit(Unit * /*victim*/) { DoScriptText(RAND(SAY_KILL_1, SAY_KILL_2, SAY_KILL_3), me); }
    ...
    void SpawnAdds() {
      ...
      if (isAddlistEmpty()) {
        ...
        for (std::vector<uint32>::const_iterator itr = AddList.begin(); itr != AddList.end(); ++itr) {
          ...
          pCreature = me->SummonCreature(entry, Locations[i][0], Locations[i][1], POS_Z, Locations[i][2],
        ...
        for (uint8 i = 0; i < 4; ++i) {
          Creature *pCreature = me->SummonCreature(AddId[i], Locations[i][0], Locations[i][1], POS_Z, Locations[i][2],
                                                   TEMPSUMMON_CORPSE_TIMED_DESPAWN, 10000);
          if (pCreature)
            AddGUID[i] = pCreature->GetGUID();
        }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return GetInstanceAI<boss_moroesAI>(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return GetInstanceAI<boss_baroness_dorothea_millstipeAI>(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return GetInstanceAI<boss_baron_rafe_dreugerAI>(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return GetInstanceAI<boss_lady_catriona_von_indiAI>(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return GetInstanceAI<boss_lady_keira_berrybuckAI>(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return GetInstanceAI<boss_lord_robin_darisAI>(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return GetInstanceAI<boss_lord_crispin_ferenceAI>(pCreature); }
};
