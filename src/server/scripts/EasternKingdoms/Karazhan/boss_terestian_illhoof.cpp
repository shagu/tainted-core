...
#define SPELL_ENRAGE                                                                                                   \
  23537 // Increases the caster's attack speed by 50% and the Physical damage it deals by 219 to 281 for 10 min.
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new mob_fiendish_portalAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new mob_demon_chainAI(pCreature); }
};
...
public:
  ...
  struct boss_terestian_illhoofAI : public ScriptedAI {
    ...
    void JustSummoned(Creature *pSummoned) {
      if (pSummoned->GetEntry() == CREATURE_PORTAL) {
        ...
        /* DoScriptText(SAY_SUMMON2, me); */ /* Needs to be added later to randomize summon portal text: between SUMMON1
                                                AND SUMMON2 */
      }
    }
    ...
    void UpdateAI(const uint32 diff) {
      ...
      if (SummonKilrekTimer <= diff && SummonedKilrek == false) {
        Creature *Kilrek = me->SummonCreature(CREATURE_KILREK, -11237.955078, -1693.597412, 179.237823, 0,
                                              TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, 15000);
        if (Kilrek) {
      ...
      if (SummonTimer <= diff) {
        if (!SummonedPortals) {
          for (uint8 i = 0; i < 2; ++i) {
            Creature *Portal = me->SummonCreature(CREATURE_PORTAL, PortalLocations[i][0], PortalLocations[i][1],
                                                  PORTAL_Z, 0, TEMPSUMMON_CORPSE_DESPAWN, 0);
            if (Portal)
        ...
        Creature *Imp = me->SummonCreature(CREATURE_FIENDISHIMP, PortalLocations[random][0], PortalLocations[random][1],
                                           PORTAL_Z, 0, TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, 15000);
        if (Imp) {
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_terestian_illhoofAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new mob_fiendish_impAI(pCreature); }
};
...
public:
  ...
  struct mob_kilrekAI : public ScriptedAI {
    mob_kilrekAI(Creature *c) : ScriptedAI(c) { pInstance = (ScriptedInstance *)c->GetInstanceData(); }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new mob_kilrekAI(pCreature); }
};
