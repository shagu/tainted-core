...
static Dialogue OzDialogue[] = {{-1532103, 6000}, {-1532104, 18000}, {-1532105, 9000}, {-1532106, 15000}};

static Dialogue HoodDialogue[] = {{-1532107, 6000}, {-1532108, 10000}, {-1532109, 14000}, {-1532110, 15000}};

static Dialogue RAJDialogue[] = {{-1532111, 5000}, {-1532112, 7000}, {-1532113, 14000}, {-1532114, 14000}};
...
float GameObjects_RomeJulia[1][4] = {{183443, -10890.9f, -1744.06f, 90.476f}};

float GameObjects_Wolf[5][4] = {{183492, -10908.9f, -1762.55f, 90.478f},
...
enum eBerthold { SPELL_TELEPORT = 39567 };
...
#define SAY_DIALOG_ARCANAGOS_2                                                                                         \
  "Your dabbling in the arcane has gone too far, Medivh. You've attracted the attention of powers beyond your "        \
  "understanding. You must leave Karazhan at once!"
...
#define SAY_DIALOG_ARCANAGOS_4                                                                                         \
  "A dark power seeks to use you, Medivh! If you stay, dire days will follow. You must hurry, we don't have much "     \
  "time!"
#define SAY_DIALOG_MEDIVH_5                                                                                            \
  "I do not know what you speak of, dragon... but I will not be bullied by this display of insolence. I'll leave "     \
  "Karazhan when it suits me!"
...
enum chaoticMisc { SPELL_UNSTABLE_MAGIC = 29900 };

/////////////////
//*mana_feeder*//
/////////////////

enum feederMisc { SPELL_MANA_BITE = 29908 };
...
public:
  ...
  struct npc_barnesAI : public npc_escortAI {
    ...
    void WaypointReached(uint32 i) {
      ...
      case 2:
        ...
        if (Creature *pSpotlight =
                me->SummonCreature(CREATURE_SPOTLIGHT, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), 0.0f,
        ...
        case EVENT_OZ:
          for (int i = 0; i < 5; i++)
            me->SummonGameObject(GameObjects_OZ[i][0], GameObjects_OZ[i][1], GameObjects_OZ[i][2], GameObjects_OZ[i][3],
                                 4.63f, 0, 0, 0.73f, -0.68f, 60000);
          break;
        case EVENT_HOOD:
          for (int i = 0; i < 5; i++)
            me->SummonGameObject(GameObjects_Wolf[i][0], GameObjects_Wolf[i][1], GameObjects_Wolf[i][2],
                                 GameObjects_Wolf[i][3], 4.63f, 0, 0, 0.73f, -0.68f, 60000);
          break;
        case EVENT_RAJ:
          for (int i = 0; i < 1; i++)
            me->SummonGameObject(GameObjects_RomeJulia[i][0], GameObjects_RomeJulia[i][1], GameObjects_RomeJulia[i][2],
                                 GameObjects_RomeJulia[i][3], 4.63f, 0, 0, 0.73f, -0.68f, 60000);
          break;
        }
        break;
      case 5:
    ...
    void PrepareEncounter() {
      sLog.outDebug("OSCR: Barnes Opera Event - Introduction complete - preparing encounter %d", m_uiEventId);
      uint8 index = 0;
      uint8 count = 0;
      ...
      for (; index < count; ++index) {
        ...
        if (Creature *pCreature = me->SummonCreature(entry, PosX, SPAWN_Y, SPAWN_Z, SPAWN_O,
                                                     TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, HOUR * 2 * IN_MILLISECONDS)) {
    ...
    if (ScriptedInstance *pInstance = (ScriptedInstance *)pCreature->GetInstanceData()) {
      ...
      if (pInstance->GetData(TYPE_MOROES) == DONE && pInstance->GetData(TYPE_OPERA) != DONE) {
        ...
        if (npc_barnesAI *pBarnesAI = CAST_AI(npc_barnesAI, pCreature->AI())) {
          if (pBarnesAI->pInstance->GetData(TYPE_OPERA) != IN_PROGRESS ||
              pBarnesAI->pInstance->GetData(TYPE_OPERA) != DONE)
  ...
  bool OnGossipSelect(Player *pPlayer, Creature *pCreature, uint32 /*uiSender*/, uint32 uiAction) override {
    ...
    case GOSSIP_ACTION_INFO_DEF + 3:
      ...
      sLog.outString("OSCR: player (GUID %llu) manually set Opera event to EVENT_OZ", pPlayer->GetGUID());
      break;
    case GOSSIP_ACTION_INFO_DEF + 4:
      pPlayer->CLOSE_GOSSIP_MENU();
      pBarnesAI->m_uiEventId = EVENT_HOOD;
      sLog.outString("OSCR: player (GUID %llu) manually set Opera event to EVENT_HOOD", pPlayer->GetGUID());
      break;
    case GOSSIP_ACTION_INFO_DEF + 5:
      pPlayer->CLOSE_GOSSIP_MENU();
      pBarnesAI->m_uiEventId = EVENT_RAJ;
      sLog.outString("OSCR: player (GUID %llu) manually set Opera event to EVENT_RAJ", pPlayer->GetGUID());
      break;
    }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_barnesAI(pCreature); }
};
  ...
  bool OnGossipHello(Player *pPlayer, Creature *pCreature) override {
    if (ScriptedInstance *pInstance = (ScriptedInstance *)pCreature->GetInstanceData()) {
      ...
      if (pInstance->GetData(TYPE_ARAN) == DONE)
        pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_ITEM_TELEPORT, GOSSIP_SENDER_MAIN,
...
public:
  ...
  struct npc_image_of_medivhAI : public ScriptedAI {
    npc_image_of_medivhAI(Creature *c) : ScriptedAI(c) { pInstance = (ScriptedInstance *)c->GetInstanceData(); }
    ...
    void StartEvent() {
      Step = 1;
      EventStarted = true;
      Creature *Arcanagos = me->SummonCreature(MOB_ARCANAGOS, ArcanagosPos[0], ArcanagosPos[1], ArcanagosPos[2], 0,
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_image_of_medivhAI(pCreature); }
};
...
public:
  ...
  struct npc_concubineAI : public ScriptedAI {
    npc_concubineAI(Creature *c) : ScriptedAI(c) { pInstance = (ScriptedInstance *)c->GetInstanceData(); }
    ...
    void EnterCombat(Unit * /*who*/) { DoScriptText(RAND(SAY_CONCUBINE, SAY_CONCUBINE2, SAY_CONCUBINE3), me); }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_concubineAI(pCreature); }
};
...
public:
  ...
  struct npc_arcane_anomalyAI : public ScriptedAI {
    npc_arcane_anomalyAI(Creature *c) : ScriptedAI(c) { pInstance = (ScriptedInstance *)c->GetInstanceData(); }
    ...
    void JustDied(Unit * /*killer*/) { DoCast(SPELL_LOOSEMANA); }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_arcane_anomalyAI(pCreature); }
};
...
public:
  ...
  struct npc_chaotic_sentienceAI : public ScriptedAI {
    npc_chaotic_sentienceAI(Creature *c) : ScriptedAI(c) { pInstance = (ScriptedInstance *)c->GetInstanceData(); }
    ...
    void EnterCombat(Unit * /*who*/) { DoCast(SPELL_UNSTABLE_MAGIC); }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_chaotic_sentienceAI(pCreature); }
};
...
public:
  ...
  struct npc_mana_feederAI : public ScriptedAI {
    npc_mana_feederAI(Creature *c) : ScriptedAI(c) { pInstance = (ScriptedInstance *)c->GetInstanceData(); }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_mana_feederAI(pCreature); }
};
...
public:
  ...
  struct npc_arcane_protectorAI : public ScriptedAI {
    npc_arcane_protectorAI(Creature *c) : ScriptedAI(c) { pInstance = (ScriptedInstance *)c->GetInstanceData(); }
    ...
    void UpdateAI(const uint32 diff) {
      ...
      if (summonTimer <= diff) {
        me->SummonCreature(NPC_ASTRAL_SPARK, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), 0,
