...
public:
  ...
  struct boss_netherspiteAI : public ScriptedAI {
    ...
    void SummonPortals() {
      ...
      for (int i = 0; i < 3; ++i)
        if (Creature *portal = me->SummonCreature(PortalID[i], PortalCoord[pos[i]][0], PortalCoord[pos[i]][1],
                                                  PortalCoord[pos[i]][2], 0, TEMPSUMMON_TIMED_DESPAWN, 60000)) {
      ...
      for (int j = 0; j < 3; ++j) // j = color
        if (Creature *portal = Unit::GetCreature(*me, PortalGUID[j])) {
          ...
          if (Map *map = me->GetMap()) {
            ...
            for (Map::PlayerList::const_iterator i = players.begin(); i != players.end(); ++i) {
              ...
              if (p && p->IsAlive()                                                          // alive
                  ...
                  && !p->HasAura(PlayerBuff[(j + 2) % 3], 0) && IsBetween(me, p, portal))    // on the beam
                pTarget = p;
            }
          }
          // buff the target
          ...
          if (!current || pTarget != current) {
            ...
            if (Creature *beamer = portal->SummonCreature(PortalID[j], portal->GetPositionX(), portal->GetPositionY(),
                                                          portal->GetPositionZ(), portal->GetOrientation(),
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return GetInstanceAI<boss_netherspiteAI>(pCreature); }
};
