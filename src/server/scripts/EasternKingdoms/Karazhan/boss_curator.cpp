...
public:
  ...
  struct boss_curatorAI : public ScriptedAI {
    ...
    void UpdateAI(const uint32 diff) {
      ...
      case EVENT_SPELL_HATEFUL_BOLT:
        ...
        events.ScheduleEvent(EVENT_SPELL_HATEFUL_BOLT,
                             urand(5000, 7500) * (events.GetNextEventTime(EVENT_SPELL_BERSERK) == 0 ? 1 : 2));
        break;
      case EVENT_SPELL_ASTRAL_FLARE: {
        ...
        if (me->GetPower(POWER_MANA) < 10.0f) {
          ...
          Creature *AstralFlare =
              DoSpawnCreature(17096, rand() % 37, rand() % 37, 0, 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 5000);
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_curatorAI(pCreature); }
};

void AddSC_boss_curator() { new boss_curator(); }
