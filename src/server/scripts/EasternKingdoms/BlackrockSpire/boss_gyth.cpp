...
public:
  ...
  struct boss_gythAI : public ScriptedAI {
    ...
    void SummonCreatureWithRandomTarget(uint32 creatureId) {
      Unit *Summoned = me->SummonCreature(creatureId, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), 0,
    ...
    void UpdateAI(const uint32 diff) {
      ...
      if (bAggro) {
        ...
        if (!SummonedRend && HealthBelowPct(11) && me->GetHealth() > 0) {
          ...
          me->SummonCreature(10429, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), 0,
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_gythAI(pCreature); }
};

void AddSC_boss_gyth() { new boss_gyth(); }
