...
public:
  ...
  struct boss_overlord_wyrmthalakAI : public ScriptedAI {
    ...
    void UpdateAI(const uint32 diff) {
      ...
      if (!Summoned && HealthBelowPct(50)) {
        ...
        if (Creature *SummonedCreature =
                me->SummonCreature(9216, ADD_1X, ADD_1Y, ADD_1Z, ADD_1O, TEMPSUMMON_TIMED_DESPAWN, 300000))
          SummonedCreature->AI()->AttackStart(pTarget);
        if (Creature *SummonedCreature =
                me->SummonCreature(9268, ADD_2X, ADD_2Y, ADD_2Z, ADD_2O, TEMPSUMMON_TIMED_DESPAWN, 300000))
          SummonedCreature->AI()->AttackStart(pTarget);
        Summoned = true;
      }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_overlord_wyrmthalakAI(pCreature); }
};
