...
public:
  ...
  struct boss_eranikusAI : public ScriptedAI {
    boss_eranikusAI(Creature *creature) : ScriptedAI(creature) { Initialize(); }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_eranikusAI(pCreature); }
};

void AddSC_boss_eranikus() { new boss_eranikus(); }
