...
public:
  ...
  struct instance_sunken_templeAI : public ScriptedInstance {
    instance_sunken_templeAI(Map *pMap) : ScriptedInstance(pMap) { Initialize(); };
    ...
    void UseStatue(GameObject *pGo) {
      pGo->SummonGameObject(GO_ATALAI_LIGHT1, pGo->GetPositionX(), pGo->GetPositionY(), pGo->GetPositionZ(), 0, 0, 0, 0,
                            0, 0);
  ...
  InstanceData *GetInstanceScript(InstanceMap *pMap) const override { return new instance_sunken_templeAI(pMap); }
};
void AddSC_instance_sunken_temple() { new instance_sunken_temple(); }
