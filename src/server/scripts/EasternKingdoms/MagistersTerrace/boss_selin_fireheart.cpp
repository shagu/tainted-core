...
enum CrystalSpells {
  ...
  SPELL_MANA_RAGE =
      44320 // This spell triggers 44321, which changes scale and regens mana Requires an entry in spell_script_target
};
...
public:
  ...
  struct boss_selin_fireheartAI : public ScriptedAI {
    ...
    bool Heroic;
    uint64 CrystalGUID; // This will help us create a pointer to the crystal we are draining. We store GUIDs, never
                        // units in case unit is deleted/offline (offline if player of course).
      ...
      if (instance) {
        ...
        for (std::list<uint64>::const_iterator itr = Crystals.begin(); itr != Crystals.end(); ++itr) {
          ...
          if (unit) {
            ...
            // Only need to set unselectable flag. You can't attack unselectable units so non_attackable flag is not
            // necessary here.
      ...
      for (std::list<uint64>::const_iterator itr = Crystals.begin(); itr != Crystals.end(); ++itr) {
        ...
        if (pCrystal && pCrystal->IsAlive()) {
          ...
          if (!CrystalChosen || me->GetDistanceOrder(pCrystal, CrystalChosen, false)) {
            ...
            CrystalChosen = pCrystal; // Store a copy of pCrystal so we don't need to recreate a pointer to closest
                                      // crystal for the movement and yell.
          }
        }
      }
      if (CrystalChosen) {
    ...
    void KilledUnit(Unit * /*victim*/) { DoScriptText(RAND(SAY_KILL_1, SAY_KILL_2), me); }
  ...
  CreatureAI *GetAI(Creature *creature) const { return GetInstanceAI<boss_selin_fireheartAI>(creature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *creature) const { return GetInstanceAI<mob_fel_crystalAI>(creature); }
};
