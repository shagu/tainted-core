...
#define SAY_AGGRO                                                                                                      \
  -1585023 // This yell should be done when the room is cleared. For now, set it as a movelineofsight yell.
...
#define SPELL_POWER_FEEDBACK                                                                                           \
  44233 // Stuns him, making him take 50% more damage for 10 seconds. Cast after Gravity Lapse
...
public:
  ...
  struct boss_felblood_kaelthasAI : public ScriptedAI {
    ...
    void JustDied(Unit * /*killer*/) { pInstance->SetData(DATA_KAELTHAS_EVENT, DONE); }
  ...
  CreatureAI *GetAI(Creature *c) const { return GetInstanceAI<boss_felblood_kaelthasAI>(c); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *c) const { return new mob_arcane_sphereAI(c); }
};
...
public:
  ...
  struct mob_felkael_phoenixAI : public ScriptedAI {
    mob_felkael_phoenixAI(Creature *c) : ScriptedAI(c) { pInstance = (ScriptedInstance *)c->GetInstanceData(); }
  ...
  CreatureAI *GetAI(Creature *c) const { return GetInstanceAI<mob_felkael_phoenixAI>(c); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *c) const { return new mob_felkael_phoenix_eggAI(c); }
};
...
public:
  ...
  struct mob_felkael_flamestrikeAI : public ScriptedAI {
    mob_felkael_flamestrikeAI(Creature *c) : ScriptedAI(c) { Heroic = c->GetMap()->IsHeroic(); }
  ...
  CreatureAI *GetAI(Creature *c) const { return new mob_felkael_flamestrikeAI(c); }
};
