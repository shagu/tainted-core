...
SD%Complete: 45
SDComment: No Heroic support yet. Needs further testing. Several scripts for pets disabled, not seem to require any
special script. SDCategory: Magister's Terrace EndScriptData */
...
static Speech PlayerDeath[] = {
    {-1585017}, {-1585018}, {-1585019}, {-1585020}, {-1585021},
};

enum Texts { SAY_AGGRO = -1585012, SAY_DEATH = -1585022 };
...
float LackeyLocations[4][2] = {{123.77f, 17.6007f}, {131.731f, 15.0827f}, {121.563f, 15.6213f}, {129.988f, 17.2355f}};
...
enum eHealingPotion { SPELL_HEALING_POTION = 15503 };
...
public:
    ...
    void InitializeLackeys() {
      ...
      if (LackeyEntryList.empty()) {
        ...
        for (std::vector<uint32>::const_iterator itr = LackeyEntryList.begin(); itr != LackeyEntryList.end(); ++itr) {
          if (Creature *add = me->SummonCreature((*itr), LackeyLocations[j][0], LackeyLocations[j][1], fZLocation,
                                                 fOrientation, TEMPSUMMON_CORPSE_DESPAWN, 0))
  ...
  CreatureAI *GetAI(Creature *creature) const { return GetInstanceAI<boss_priestess_delrissaAI>(creature); }
};
...
public:
  ...
  struct boss_priestess_lackey_commonAI : public ScriptedAI {
    ...
    void Reset() {
      ...
      // in case she is not alive and Reset was for some reason called, respawn her (most likely party wipe after
      // killing her)
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_priestess_lackey_commonAI(pCreature); }
};
...
enum eKickDown { SPELL_KNOCKDOWN = 11428, SPELL_SNAP_KICK = 46182 };
...
public:
  ...
  struct boss_kagani_nightstrikeAI : public boss_priestess_lackey_common::boss_priestess_lackey_commonAI {
    ...
    boss_kagani_nightstrikeAI(Creature *creature)
        : boss_priestess_lackey_common::boss_priestess_lackey_commonAI(creature) {}
  ...
  CreatureAI *GetAI(Creature *creature) const { return GetInstanceAI<boss_kagani_nightstrikeAI>(creature); }
};
...
public:
  ...
  struct boss_ellris_duskhallowAI : public boss_priestess_lackey_common::boss_priestess_lackey_commonAI {
    ...
    void Aggro(Unit * /*pWho*/) { DoCast(me, SPELL_SUMMON_IMP); }
  ...
  CreatureAI *GetAI(Creature *creature) const { return GetInstanceAI<boss_ellris_duskhallowAI>(creature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *creature) const { return GetInstanceAI<boss_eramas_brightblazeAI>(creature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *creature) const { return GetInstanceAI<boss_yazzaiAI>(creature); }
};
...
public:
  ...
  struct boss_warlord_salarisAI : public boss_priestess_lackey_common::boss_priestess_lackey_commonAI {
    ...
    void EnterCombat(Unit * /*who*/) { DoCast(me, SPELL_BATTLE_SHOUT); }
  ...
  CreatureAI *GetAI(Creature *creature) const { return GetInstanceAI<boss_warlord_salarisAI>(creature); }
};
...
public:
  ...
  struct boss_garaxxasAI : public boss_priestess_lackey_common::boss_priestess_lackey_commonAI {
    ...
    void JustSummoned(Creature *pSummoned) { m_uiPetGUID = pSummoned->GetGUID(); }
  ...
  CreatureAI *GetAI(Creature *creature) const { return GetInstanceAI<boss_garaxxasAI>(creature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *creature) const { return GetInstanceAI<boss_apokoAI>(creature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *creature) const { return GetInstanceAI<boss_zelfanAI>(creature); }
};
