...
enum Texts { SAY_TRANSFORM = -1309000, SAY_DEATH = -1309001 };
...
public:
  ...
  struct boss_venoxisAI : public ScriptedAI {
    boss_venoxisAI(Creature *c) : ScriptedAI(c) { pInstance = (ScriptedInstance *)c->GetInstanceData(); }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_venoxisAI(pCreature); }
};

void AddSC_boss_venoxis() { new boss_venoxis(); }
