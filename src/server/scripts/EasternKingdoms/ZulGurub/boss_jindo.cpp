...
#define SPELL_POWERFULLHEALINGWARD                                                                                     \
  24309 // We will not use this spell. We will summon a totem by script cause the spell totems will not cast.
...
public:
  ...
  struct boss_jindoAI : public ScriptedAI {
    ...
    void UpdateAI(const uint32 diff) {
      ...
      if (HealingWard_Timer <= diff) {
        ...
        me->SummonCreature(14987, me->GetPositionX() + 3, me->GetPositionY() - 2, me->GetPositionZ(), 0,
      ...
      if (Delusions_Timer <= diff) {
        if (Unit *pTarget = SelectUnit(SELECT_TARGET_RANDOM, 0)) {
          ...
          Creature *Shade =
              me->SummonCreature(14986, pTarget->GetPositionX(), pTarget->GetPositionY(), pTarget->GetPositionZ(), 0,
      ...
      if (Teleport_Timer <= diff) {
        ...
        if (pTarget && pTarget->GetTypeId() == TYPEID_PLAYER) {
          ...
          Creature *Skeletons;
          Skeletons = me->SummonCreature(14826, pTarget->GetPositionX() + 2, pTarget->GetPositionY(),
                                         pTarget->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 15000);
          if (Skeletons)
          ...
          if (Skeletons)
            Skeletons->AI()->AttackStart(pTarget);
          Skeletons = me->SummonCreature(14826, pTarget->GetPositionX() + 4, pTarget->GetPositionY(),
                                         pTarget->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 15000);
          if (Skeletons)
            Skeletons->AI()->AttackStart(pTarget);
          Skeletons = me->SummonCreature(14826, pTarget->GetPositionX() - 4, pTarget->GetPositionY(),
          ...
          if (Skeletons)
            Skeletons->AI()->AttackStart(pTarget);
          Skeletons = me->SummonCreature(14826, pTarget->GetPositionX(), pTarget->GetPositionY() + 2,
                                         pTarget->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 15000);
          if (Skeletons)
            Skeletons->AI()->AttackStart(pTarget);
          Skeletons = me->SummonCreature(14826, pTarget->GetPositionX(), pTarget->GetPositionY() - 2,
          ...
          if (Skeletons)
            Skeletons->AI()->AttackStart(pTarget);
          Skeletons = me->SummonCreature(14826, pTarget->GetPositionX(), pTarget->GetPositionY() + 4,
                                         pTarget->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 15000);
          if (Skeletons)
            Skeletons->AI()->AttackStart(pTarget);
          Skeletons = me->SummonCreature(14826, pTarget->GetPositionX(), pTarget->GetPositionY() - 4,
          ...
          if (Skeletons)
            Skeletons->AI()->AttackStart(pTarget);
          Skeletons = me->SummonCreature(14826, pTarget->GetPositionX() + 3, pTarget->GetPositionY(),
                                         pTarget->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 15000);
          if (Skeletons)
            Skeletons->AI()->AttackStart(pTarget);
        }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_jindoAI(pCreature); }
};
...
public:
  ...
  struct mob_healing_wardAI : public ScriptedAI {
    mob_healing_wardAI(Creature *c) : ScriptedAI(c) { pInstance = (ScriptedInstance *)c->GetInstanceData(); }

    uint32 Heal_Timer;

    ScriptedInstance *pInstance;

    void Reset() { Heal_Timer = 2000; }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new mob_healing_wardAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new mob_shade_of_jindoAI(pCreature); }
};
