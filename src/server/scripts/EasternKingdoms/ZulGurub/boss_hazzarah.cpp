...
public:
    ...
    void UpdateAI(const uint32 diff) {
      ...
      if (Illusions_Timer <= diff) {
        ...
        for (uint8 i = 0; i < 3; ++i) {
          ...
          Creature *Illusion =
              me->SummonCreature(15163, pTarget->GetPositionX(), pTarget->GetPositionY(), pTarget->GetPositionZ(), 0,
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_hazzarahAI(pCreature); }
};

void AddSC_boss_hazzarah() { new boss_hazzarah(); }
