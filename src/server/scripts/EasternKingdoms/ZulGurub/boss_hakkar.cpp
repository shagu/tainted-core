...
public:
  ...
  struct boss_hakkarAI : public ScriptedAI {
    boss_hakkarAI(Creature *c) : ScriptedAI(c) { pInstance = (ScriptedInstance *)c->GetInstanceData(); }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_hakkarAI(pCreature); }
};

void AddSC_boss_hakkar() { new boss_hakkar(); }
