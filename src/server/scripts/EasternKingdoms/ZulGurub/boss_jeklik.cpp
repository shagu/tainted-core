...
enum Texts { SAY_AGGRO = -1309002, SAY_RAIN_FIRE = -1309003, SAY_DEATH = -1309004 };
...
enum Spells {
  ...
  SPELL_CHAIN_MIND_FLAY = 26044, // Correct spell unknown, however unable to find on WoWHead or WoWWiki but was added
                                 // in 1.12 so possibily they used this spell and it's correct?
...
public:
  ...
  struct boss_jeklikAI : public ScriptedAI {
    boss_jeklikAI(Creature *c) : ScriptedAI(c) { pInstance = (ScriptedInstance *)c->GetInstanceData(); }
      ...
      if (me->GetVictim() && me->IsAlive()) {
        if (HealthAbovePct(50)) {
          ...
          if (SpawnBats_Timer <= diff) {
            ...
            Creature *Bat = NULL;
            Bat = me->SummonCreature(11368, -12291.6220f, -1380.2640f, 144.8304f, 5.483f,
                                     TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 15000);
            if (pTarget && Bat)
              Bat->AI()->AttackStart(pTarget);

            Bat = me->SummonCreature(11368, -12289.6220f, -1380.2640f, 144.8304f, 5.483f,
                                     TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 15000);
            if (pTarget && Bat)
              Bat->AI()->AttackStart(pTarget);

            Bat = me->SummonCreature(11368, -12293.6220f, -1380.2640f, 144.8304f, 5.483f,
                                     TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 15000);
            if (pTarget && Bat)
              Bat->AI()->AttackStart(pTarget);
          ...
          if (PhaseTwo) {
            ...
            if (SpawnFlyingBats_Timer <= diff) {
              ...
              Creature *FlyingBat =
                  me->SummonCreature(14965, pTarget->GetPositionX(), pTarget->GetPositionY(),
                                     pTarget->GetPositionZ() + 15, 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 15000);
              if (FlyingBat)
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_jeklikAI(pCreature); }
};
...
public:
  ...
  struct mob_batriderAI : public ScriptedAI {
    mob_batriderAI(Creature *c) : ScriptedAI(c) { pInstance = (ScriptedInstance *)c->GetInstanceData(); }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new mob_batriderAI(pCreature); }
};
