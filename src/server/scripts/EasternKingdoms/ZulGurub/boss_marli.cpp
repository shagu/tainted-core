...
enum Texts { SAY_AGGRO = -1309005, SAY_TRANSFORM = -1309006, SAY_SPIDER_SPAWN = -1309007, SAY_DEATH = -1309008 };
...
public:
  ...
  struct boss_marliAI : public ScriptedAI {
    boss_marliAI(Creature *c) : ScriptedAI(c) { pInstance = (ScriptedInstance *)c->GetInstanceData(); }
    ...
    void UpdateAI(const uint32 diff) {
      ...
      if (me->GetVictim() && me->IsAlive()) {
        ...
        if (!Spawned && SpawnStartSpiders_Timer <= diff) {
          ...
          Spider = me->SummonCreature(15041, pTarget->GetPositionX(), pTarget->GetPositionY(), pTarget->GetPositionZ(),
                                      0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 15000);
          if (Spider)
        ...
        if (SpawnSpider_Timer <= diff) {
          ...
          Creature *Spider =
              me->SummonCreature(15041, pTarget->GetPositionX(), pTarget->GetPositionY(), pTarget->GetPositionZ(), 0,
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_marliAI(pCreature); }
};
...
public:
  ...
  struct mob_spawn_of_marliAI : public ScriptedAI {
    ...
    void EnterCombat(Unit * /*who*/) { Growth_Level = 1; }

    void JustDied(Unit * /*Killer*/) { me->SetObjectScale(1.0f); }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new mob_spawn_of_marliAI(pCreature); }
};
