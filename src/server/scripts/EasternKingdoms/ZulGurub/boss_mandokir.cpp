...
public:
  ...
  struct boss_mandokirAI : public ScriptedAI {
    boss_mandokirAI(Creature *c) : ScriptedAI(c) { pInstance = (ScriptedInstance *)c->GetInstanceData(); }
    ...
    void UpdateAI(const uint32 diff) {
      ...
      if (me->GetVictim() && me->IsAlive()) {
        if (!CombatStart) {
          ...
          me->SummonCreature(OHGAN, me->GetVictim()->GetPositionX(), me->GetVictim()->GetPositionY(),
                             me->GetVictim()->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 5000);
          CombatStart = true;
        }
        ...
        if (Gaze_Timer <= diff) // Every 20 seconds Mandokir will check this
        {
          if (GazeTarget) {
            ...
            if (pUnit && (targetX != pUnit->GetPositionX() || targetY != pUnit->GetPositionY() ||
                          targetZ != pUnit->GetPositionZ() || pUnit->IsInCombat())) {
        ...
        if (Gaze_Timer < 8000 && !someGazed) // 8 second(cast time + expire time) before the check for the gaze effect
                                             // Mandokir will cast gaze debuff on a random target
        {
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_mandokirAI(pCreature); }
};
...
public:
  ...
  struct mob_ohganAI : public ScriptedAI {
    mob_ohganAI(Creature *c) : ScriptedAI(c) { pInstance = (ScriptedInstance *)c->GetInstanceData(); }

    uint32 SunderArmor_Timer;
    ScriptedInstance *pInstance;

    void Reset() { SunderArmor_Timer = 5000; }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new mob_ohganAI(pCreature); }
};
