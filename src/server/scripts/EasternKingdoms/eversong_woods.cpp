  ...
  struct mobs_mana_tappedAI : public ScriptedAI {
    ...
    void SpellHit(Unit *caster, const SpellEntry *spell) {
      if (caster->GetTypeId() == TYPEID_PLAYER)
        if (CAST_PLR(caster)->GetQuestStatus(8346) == QUEST_STATUS_INCOMPLETE &&
            !CAST_PLR(caster)->GetReqKillOrCastCurrentCount(8346, me->GetEntry()) && spell->Id == 28734)
          CAST_PLR(caster)->CastedCreatureOrGO(15468, me->GetGUID(), spell->Id);
      return;
    }
  };
  CreatureAI *GetAI(Creature *pCreature) const { return new mobs_mana_tappedAI(pCreature); }
};
...
public:
  ...
  struct npc_prospector_anvilwardAI : public npc_escortAI {
    ...
    void JustDied(Unit * /*killer*/) { me->RestoreFaction(); }
  };
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_prospector_anvilwardAI(pCreature); }
};
...
static Locations SpawnPosition[] = {{5.3f, -11.8f, 0.361f, 4.2f},
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_secondTrialAI(pCreature); }
};
...
public:
  ...
  bool OnGossipHello(Player *pPlayer, Creature *pCreature) override {
    ...
    // Escort quests or any other event-driven quests. If player in party, all players that can accept this quest will
    // receive confirmation box to accept quest. !not sure if this really works!
      ...
      if (questPhase == 1) {
        ...
        for (uint8 i = 0; i < 4; ++i)
          if (Creature *pSummoned =
                  DoSpawnCreature(PaladinEntry[i], SpawnPosition[i].x, SpawnPosition[i].y, SpawnPosition[i].z,
                                  SpawnPosition[i].o, TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 180000))
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new master_kelerun_bloodmournAI(pCreature); }
};
    ...
    if (me->GetEntry() == CHAMPION_SUNSTRIKER) {
      if (Group *pGroup = CAST_PLR(Killer)->GetGroup()) {
        for (GroupReference *itr = pGroup->GetFirstMember(); itr != NULL; itr = itr->next()) {
          ...
          if (pGroupGuy && pGroupGuy->IsAtGroupRewardDistance(me) && !pGroupGuy->GetCorpse() &&
              pGroupGuy->GetQuestStatus(QUEST_SECOND_TRIAL) == QUEST_STATUS_INCOMPLETE)
...
enum eMirveda { QUEST_UNEXPECTED_RESULT = 8488, MOB_GHARZUL = 15958, MOB_ANGERSHADE = 15656 };
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_apprentice_mirvedaAI(pCreature); }
};
...
enum eCrystal { MOB_ENRAGED_WRAITH = 17086, EMOTE = -1000283, QUEST_POWERING_OUR_DEFENSES = 8490 };

struct Location {
  float x, y, z;
};

static Location SpawnLocations[] = {{8270.68f, -7188.53f, 139.619f}, {8284.27f, -7187.78f, 139.603f},
                                    {8297.43f, -7193.53f, 139.603f}, {8303.5f, -7201.96f, 139.577f},
                                    {8273.22f, -7241.82f, 139.382f}, {8254.89f, -7222.12f, 139.603f},
                                    {8278.51f, -7242.13f, 139.162f}, {8267.97f, -7239.17f, 139.517f}};
...
public:
  ...
  struct npc_infused_crystalAI : public Scripted_NoMovementAI {
    ...
    void UpdateAI(const uint32 diff) {
      ...
      if (WaveTimer < diff && !Completed && Progress) {
        ...
        uint32 ran2 = rand() % 8;
        uint32 ran3 = rand() % 8;
        me->SummonCreature(MOB_ENRAGED_WRAITH, SpawnLocations[ran1].x, SpawnLocations[ran1].y, SpawnLocations[ran1].z,
                           0, TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, 10000);
        me->SummonCreature(MOB_ENRAGED_WRAITH, SpawnLocations[ran2].x, SpawnLocations[ran2].y, SpawnLocations[ran2].z,
                           0, TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, 10000);
        me->SummonCreature(MOB_ENRAGED_WRAITH, SpawnLocations[ran3].x, SpawnLocations[ran3].y, SpawnLocations[ran3].z,
                           0, TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, 10000);
        WaveTimer = 30000;
      } else
        WaveTimer -= diff;
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_infused_crystalAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_infused_crystal::npc_infused_crystalAI(pCreature); }
};
