...
public:
  ...
  struct instance_stratholmeAI : public ScriptedInstance {
    ...
    bool StartSlaugtherSquare() {
      ...
      sLog.outDebug("OSCR: Instance Stratholme: Cannot open slaugther square yet.");
      return false;
    }

    // if withRestoreTime true, then newState will be ignored and GO should be restored to original state after 10
    // seconds
    ...
    void OnGameObjectCreate(GameObject *pGo, bool /*add*/) {
      ...
      case GO_PORT_GAUNTLET:
        ...
        if (GetData(TYPE_BARONESS) == IN_PROGRESS && GetData(TYPE_NERUB) == IN_PROGRESS &&
    ...
    void SetData(uint32 type, uint32 data) {
      ...
      case TYPE_BARON_RUN:
        ...
        case IN_PROGRESS:
          ...
          sLog.outDebug("OSCR: Instance Stratholme: Baron run in progress.");
          break;
        case FAIL:
        ...
        case DONE:
          ...
          if (Creature *pYsidaT = instance->GetCreature(ysidaTriggerGUID))
            pYsidaT->SummonCreature(C_YSIDA, pYsidaT->GetPositionX(), pYsidaT->GetPositionY(), pYsidaT->GetPositionZ(),
                                    pYsidaT->GetOrientation(), TEMPSUMMON_TIMED_DESPAWN, 1800000);
          BaronRun_Timer = 0;
          break;
        }
        break;
      ...
      case TYPE_RAMSTEIN:
        if (data == IN_PROGRESS) {
          ...
          if (!count) {
            ...
            if (Creature *pBaron = instance->GetCreature(baronGUID))
              pBaron->SummonCreature(C_RAMSTEIN, 4032.84f, -3390.24f, 119.73f, 4.71f, TEMPSUMMON_TIMED_OR_DEAD_DESPAWN,
                                     1800000);
            sLog.outDebug("OSCR: Instance Stratholme: Ramstein spawned.");
          } else
            sLog.outDebug("OSCR: Instance Stratholme: %u Abomnation left to kill.", count);
        }
        ...
        if (data == DONE) {
          ...
          sLog.outDebug("OSCR: Instance Stratholme: Slaugther event will continue in 5 minutes.");
        }
        Encounter[4] = data;
        break;
      case TYPE_BARON:
    ...
    uint32 GetData(uint32 type) {
      switch (type) {
      case TYPE_SH_QUEST:
        if (IsSilverHandDead[0] && IsSilverHandDead[1] && IsSilverHandDead[2] && IsSilverHandDead[3] &&
            IsSilverHandDead[4])
          return 1;
        return 0;
      case TYPE_BARON_RUN:
    ...
    void Update(uint32 diff) {
      if (BaronRun_Timer) {
        if (BaronRun_Timer <= diff) {
          ...
          BaronRun_Timer = 0;
          sLog.outDebug("OSCR: Instance Stratholme: Baron run event reached end. Event has state %u.",
      ...
      if (SlaugtherSquare_Timer) {
        if (SlaugtherSquare_Timer <= diff) {
          if (Creature *pBaron = instance->GetCreature(baronGUID)) {
            for (uint8 i = 0; i < 4; ++i)
              pBaron->SummonCreature(C_BLACK_GUARD, 4032.84f, -3390.24f, 119.73f, 4.71f,
                                     TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 1800000);

            HandleGameObject(ziggurat4GUID, true);
            HandleGameObject(ziggurat5GUID, true);
            sLog.outDebug("OSCR: Instance Stratholme: Black guard sentries spawned. Opening gates to baron.");
          }
          SlaugtherSquare_Timer = 0;
        } else
  ...
  InstanceData *GetInstanceScript(InstanceMap *pMap) const override { return new instance_stratholmeAI(pMap); }
};
