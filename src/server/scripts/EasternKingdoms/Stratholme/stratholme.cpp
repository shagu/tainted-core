...
enum eGhostlyCitizenSpells { SPELL_HAUNTING_PHANTOM = 16336, SPELL_SLAP = 6754 };
    ...
    if (Group *pGroup = player->GetGroup()) {
      for (GroupReference *itr = pGroup->GetFirstMember(); itr != NULL; itr = itr->next()) {
        ...
        if (pGroupie->GetQuestStatus(QUEST_DEAD_MAN_PLEA) == QUEST_STATUS_INCOMPLETE &&
            !pGroupie->HasAura(SPELL_BARON_ULTIMATUM, 0) && pGroupie->GetMap() == pGo->GetMap())
          pGroupie->CastSpell(pGroupie, SPELL_BARON_ULTIMATUM, true);
      }
    } else if (player->GetQuestStatus(QUEST_DEAD_MAN_PLEA) == QUEST_STATUS_INCOMPLETE &&
               !player->HasAura(SPELL_BARON_ULTIMATUM, 0) && player->GetMap() == pGo->GetMap())
...
public:
  ...
  struct mob_freed_soulAI : public ScriptedAI {
    ...
    void Reset() { DoScriptText(RAND(SAY_ZAPPED0, SAY_ZAPPED1, SAY_ZAPPED2, SAY_ZAPPED3), me); }

    void EnterCombat(Unit * /*who*/) {}
  };

  CreatureAI *GetAI(Creature *pCreature) const { return new mob_freed_soulAI(pCreature); }
};
...
public:
  ...
  struct mob_restless_soulAI : public ScriptedAI {
    ...
    void SpellHit(Unit *caster, const SpellEntry *spell) {
      if (caster->GetTypeId() == TYPEID_PLAYER) {
        if (!Tagged && spell->Id == SPELL_EGAN_BLASTER &&
            CAST_PLR(caster)->GetQuestStatus(QUEST_RESTLESS_SOUL) == QUEST_STATUS_INCOMPLETE) {
          Tagged = true;
    ...
    void JustSummoned(Creature *summoned) { summoned->CastSpell(summoned, SPELL_SOUL_FREED, false); }

    void JustDied(Unit * /*Killer*/) {
      if (Tagged)
        me->SummonCreature(ENTRY_FREED, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(),
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new mob_restless_soulAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new mobs_spectral_ghostly_citizenAI(pCreature); }
};
