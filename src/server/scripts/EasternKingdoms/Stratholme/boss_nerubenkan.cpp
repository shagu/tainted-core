...
public:
  ...
  struct boss_nerubenkanAI : public ScriptedAI {
    boss_nerubenkanAI(Creature *c) : ScriptedAI(c) { pInstance = (ScriptedInstance *)me->GetInstanceData(); }
    ...
    void RaiseUndeadScarab(Unit *pVictim) {
      if (Creature *pUndeadScarab =
              DoSpawnCreature(10876, irand(-9, 9), irand(-9, 9), 0, 0, TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, 180000))
        if (pUndeadScarab->AI())
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_nerubenkanAI(pCreature); }
};
