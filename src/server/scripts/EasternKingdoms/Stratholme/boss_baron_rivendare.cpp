...
#define SAY_0                                                                                                          \
  "Intruders! More pawns of the Argent Dawn, no doubt. I already count one of their number among my prisoners. "       \
  "Withdraw from my domain before she is executed!"
#define SAY_1                                                                                                          \
  "You're still here? Your foolishness is amusing! The Argent Dawn wench needn't suffer in vain. Leave at once and "   \
  "she shall be spared!"
#define SAY_2                                                                                                          \
  "I shall take great pleasure in taking this poor wretch's life! It's not too late, she needn't suffer in vain. "     \
...
public:
  ...
  struct boss_baron_rivendareAI : public ScriptedAI {
    boss_baron_rivendareAI(Creature *c) : ScriptedAI(c) { pInstance = (ScriptedInstance *)me->GetInstanceData(); }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return GetInstanceAI<boss_baron_rivendareAI>(pCreature); }
};

void AddSC_boss_baron_rivendare() { new boss_baron_rivendare(); }
