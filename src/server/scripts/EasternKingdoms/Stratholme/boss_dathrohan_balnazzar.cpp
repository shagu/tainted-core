...
public:
  ...
  struct boss_dathrohan_balnazzarAI : public ScriptedAI {
    ...
    void JustDied(Unit * /*Victim*/) {
      ...
      for (uint8 i = 0; i < uiCount; ++i)
        me->SummonCreature(NPC_ZOMBIE, m_aSummonPoint[i].m_fX, m_aSummonPoint[i].m_fY, m_aSummonPoint[i].m_fZ,
                           m_aSummonPoint[i].m_fOrient, TEMPSUMMON_TIMED_DESPAWN, HOUR * IN_MILLISECONDS);
    }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_dathrohan_balnazzarAI(pCreature); }
};

void AddSC_boss_dathrohan_balnazzar() { new boss_dathrohan_balnazzar(); }
