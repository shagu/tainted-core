...
float DarkFiends[8][4] = {{1819.9f, 609.80f, 69.74f, 1.94f},  {1829.39f, 617.89f, 69.73f, 2.61f},
                          {1801.98f, 633.62f, 69.74f, 5.71f}, {1830.88f, 629.99f, 69.73f, 3.52f},
                          {1800.38f, 621.41f, 69.74f, 0.22f}, {1808.3f, 612.45f, 69.73f, 1.02f},
                          {1823.9f, 639.69f, 69.74f, 4.12f},  {1811.85f, 640.46f, 69.73f, 4.97f}};

float Humanoides[6][5] = {
    {CREATURE_FURY_MAGE, 1780.16f, 666.83f, 71.19f, 5.21f}, {CREATURE_FURY_MAGE, 1847.93f, 600.30f, 71.30f, 2.57f},
    {CREATURE_BERSERKER, 1779.97f, 660.64f, 71.19f, 5.28f}, {CREATURE_BERSERKER, 1786.2f, 661.01f, 71.19f, 4.51f},
    {CREATURE_BERSERKER, 1845.17f, 602.63f, 71.28f, 2.43f}, {CREATURE_BERSERKER, 1842.91f, 599.93f, 71.23f, 2.44f}};
...
public:
        ...
        if (Timer[i] <= diff) {
          ...
          case TIMER_DARKNESS:
            if (!DarkFiend) {
              ...
              DarkFiend = false;
              for (uint8 i = 0; i < 8; ++i)
                me->SummonCreature(CREATURE_DARK_FIENDS, DarkFiends[i][0], DarkFiends[i][1], DarkFiends[i][2],
                                   DarkFiends[i][3], TEMPSUMMON_CORPSE_DESPAWN, 0);
              Timer[TIMER_DARKNESS] = 42000;
            }
            break;
          case TIMER_HUMANOIDES:
            for (uint8 i = 0; i < 6; ++i)
              me->SummonCreature(Humanoides[i][0], Humanoides[i][1], Humanoides[i][2], Humanoides[i][3],
                                 Humanoides[i][4], TEMPSUMMON_CORPSE_DESPAWN, 0);
            // SummonAdds();
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_muruAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_entropiusAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_muru_portalAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_dark_fiendAI(pCreature); }
};
...
public:
  ...
  struct npc_void_sentinelAI : public ScriptedAI {
    npc_void_sentinelAI(Creature *c) : ScriptedAI(c) { pInstance = (ScriptedInstance *)c->GetInstanceData(); }
    ...
    void JustDied(Unit * /*killer*/) {
      for (uint8 i = 0; i < 6; ++i)
        me->CastSpell(me->GetPositionX() + ((2 * rand() % 1000) / 1000.0f),
                      me->GetPositionY() + ((2 * rand() % 1000) / 1000.0f), me->GetPositionZ(), SPELL_SUMMON_VOID_SPAWN,
                      false);
    }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_void_sentinelAI(pCreature); }
};
...
public:
  ...
  struct npc_blackholeAI : public ScriptedAI {
    npc_blackholeAI(Creature *c) : ScriptedAI(c) { pInstance = (ScriptedInstance *)c->GetInstanceData(); }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_blackholeAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_sentinal_summonerAI(pCreature); }
};
