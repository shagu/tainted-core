...
enum Spells {
  ...
  SPELL_FIRE_BLOOM = 45641, // Places a debuff on 5 raid members, which causes them to deal 2k Fire damage to nearby
                            // allies and selves. MIGHT NOT WORK
  ...
  SPELL_ARMAGEDDON_SUMMON_TRIGGER =
      45921,                       // Summons the triggers that cast the spells on himself need random target select
...
enum Phase {
  ...
  PHASE_SACRIFICE = 5,  // At 25%, Anveena sacrifices herself into the Sunwell; at this point he becomes enraged and has
                        // *significally* shorter cooldowns.
};
...
public:
  ...
  struct boss_kalecgos_kjAI : public ScriptedAI {
    boss_kalecgos_kjAI(Creature *c) : ScriptedAI(c) { pInstance = (ScriptedInstance *)c->GetInstanceData(); }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_kalecgos_kjAI(pCreature); }
};
...
public:
  ...
  bool OnGossipHello(Player *pPlayer, GameObject *pGo) override {
    if (pGo->GetUInt32Value(GAMEOBJECT_FACTION) == 35) {
      ...
      pPlayer->SummonCreature(CREATURE_POWER_OF_THE_BLUE_DRAGONFLIGHT, pPlayer->GetPositionX(), pPlayer->GetPositionY(),
                              pPlayer->GetPositionZ(), 0.0f, TEMPSUMMON_TIMED_DESPAWN, 121000);
...
public:
  ...
  struct mob_kiljaeden_controllerAI : public Scripted_NoMovementAI {
    ...
    void JustSummoned(Creature *summoned) {
      ...
      case CREATURE_PROPHET:
        summoned->CastSpell(summoned, SPELL_TELEPORT_VISUAL, true);
        summoned->GetMotionMaster()->MovePoint(0, aOutroLocations[5].m_fX, aOutroLocations[5].m_fY,
                                               aOutroLocations[5].m_fZ);
        break;
    ...
    void UpdateAI(const uint32 diff) {
      ...
      if (uiRandomSayTimer < diff) {
        if (pInstance && pInstance->GetData(DATA_MURU_EVENT) != DONE &&
            pInstance->GetData(DATA_KILJAEDEN_EVENT) == NOT_STARTED)
          DoScriptText(
              RAND(SAY_KJ_OFFCOMBAT1, SAY_KJ_OFFCOMBAT2, SAY_KJ_OFFCOMBAT3, SAY_KJ_OFFCOMBAT4, SAY_KJ_OFFCOMBAT5), me);
        uiRandomSayTimer = 30000;
      } else
      ...
      if (!bSummonedDeceivers) {
        for (uint8 i = 0; i < 3; ++i) {
          if (Creature *hand =
                  me->SummonCreature(CREATURE_HAND_OF_THE_DECEIVER, DeceiverLocations[i][0], DeceiverLocations[i][1],
                                     FLOOR_Z, DeceiverLocations[i][2], TEMPSUMMON_DEAD_DESPAWN, 0))
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new mob_kiljaeden_controllerAI(pCreature); }
};
...
public:
  ...
  struct boss_kiljaedenAI : public Scripted_NoMovementAI {
    ...
    void KilledUnit(Unit * /*victim*/) { DoScriptText(RAND(SAY_KJ_SLAY1, SAY_KJ_SLAY2), me); }
        ...
        if (Timer[t] <= diff && !TimerIsDeactivated[t]) {
          ...
          case TIMER_SPEECH:
            ...
            if (Speeches[speechCount].timer < SpeechTimer) {
              SpeechTimer = 0;
              if (pInstance)
                if (Creature *pSpeechCreature =
                        Unit::GetCreature(*me, pInstance->GetData64(Speeches[speechCount].pCreature)))
          ...
          case TIMER_SUMMON_SHILEDORB:
            for (uint8 i = 1; i < Phase; ++i) {
              ...
              me->SummonCreature(CREATURE_SHIELD_ORB, sx, sy, SHIELD_ORB_Z, 0, TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN,
                                 45000);
            }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_kiljaedenAI(pCreature); }
};
...
public:
  ...
  struct mob_hand_of_the_deceiverAI : public ScriptedAI {
    mob_hand_of_the_deceiverAI(Creature *c) : ScriptedAI(c) { pInstance = (ScriptedInstance *)c->GetInstanceData(); }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new mob_hand_of_the_deceiverAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new mob_felfire_portalAI(pCreature); }
};
...
public:
  ...
  struct mob_volatile_felfire_fiendAI : public ScriptedAI {
    mob_volatile_felfire_fiendAI(Creature *c) : ScriptedAI(c) { pInstance = (ScriptedInstance *)c->GetInstanceData(); }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new mob_volatile_felfire_fiendAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new mob_armageddonAI(pCreature); }
};
...
public:
  ...
  struct mob_shield_orbAI : public ScriptedAI {
    mob_shield_orbAI(Creature *c) : ScriptedAI(c) { pInstance = (ScriptedInstance *)c->GetInstanceData(); }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new mob_shield_orbAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new mob_sinster_reflectionAI(pCreature); }
};
