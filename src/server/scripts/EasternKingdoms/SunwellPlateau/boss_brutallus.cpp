...
public:
  ...
  struct npc_madrigosaAI : public ScriptedAI {
    npc_madrigosaAI(Creature *c) : ScriptedAI(c) { pInstance = ((ScriptedInstance *)c->GetInstanceData()); }

    ScriptedInstance *pInstance;

    uint32 IntroPhase;

    void Reset() { IntroPhase = 0; }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_madrigosaAI(pCreature); }
};
...
public:
  ...
  struct boss_brutallusAI : public ScriptedAI {
    boss_brutallusAI(Creature *c) : ScriptedAI(c) {
      ...
      if (pInstance && pInstance->GetData(DATA_BRUTALLUS_EVENT) == DONE &&
          pInstance->GetData(DATA_FELMYST_EVENT) != DONE) {
        float x, y, z;
    ...
    void DoIntro() {
      ...
      case 5:
        ...
        break;
      case 6:
        Madrigosa->GetMotionMaster()->MovePoint(0, Madrigosa->GetPositionX(), Madrigosa->GetPositionY() + 2,
                                                Madrigosa->GetPositionZ() + 8);
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_brutallusAI(pCreature); }
};
...
public:
  ...
  struct trigger_death_cloudAI : public ScriptedAI {
    trigger_death_cloudAI(Creature *c) : ScriptedAI(c) { pInstance = ((ScriptedInstance *)c->GetInstanceData()); }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new trigger_death_cloudAI(pCreature); }
};
