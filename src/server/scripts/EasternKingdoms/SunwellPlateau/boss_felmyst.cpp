...
Position lefts[3] = {
    {1446.540039f, 702.570007f, 52.083302f}, // south
    {1469.939941f, 704.239014f, 52.083302f}, // middle
    {1494.760010f, 705.000000f, 52.083302f},
}; // north

Position rights[3] = {
    {1441.640015f, 520.520020f, 52.083302f}, // south
    {1467.219971f, 516.318970f, 52.083302f}, // middle
    {1492.819946f, 515.668030f, 52.083302f},
}; // north

Position prepareLandingLoc[2] = {{1482.709961f, 649.406006f, 21.081100f}, {1491.119995f, 553.672974f, 24.921900f}};

enum EventTrail { EVENT_DEAD = 0 };
...
public:
  ...
  struct boss_felmystAI : public ScriptedAI {
    ...
    void KilledUnit(Unit * /*victim*/) { DoScriptText(RAND(YELL_KILL1, YELL_KILL2), me); }

    void JustRespawned() { DoScriptText(YELL_BIRTH, me); }
    ...
    void JustDied(Unit * /*Killer*/) {
      ...
      if (pInstance) {
        ...
        me->SummonCreature(25319, me->GetPositionX(), me->GetPositionY() + 10, me->GetPositionZ() + 20,
                           me->GetOrientation(), TEMPSUMMON_TIMED_DESPAWN, 10000);
    ...
    void HandleFlightSequence() {
      ...
      case 2:
        ...
        if (!direction)
          me->GetMotionMaster()->MovePoint(1, flightMobRight.GetPositionX(), flightMobRight.GetPositionY(),
                                           flightMobRight.GetPositionZ(), false);
        else
          me->GetMotionMaster()->MovePoint(1, flightMobLeft.GetPositionX(), flightMobLeft.GetPositionY(),
                                           flightMobLeft.GetPositionZ(), false);
      ...
      case 4:
        ...
        if (!direction)
          me->GetMotionMaster()->MovePoint(1, rights[chosenLane].GetPositionX(), rights[chosenLane].GetPositionY(),
                                           rights[chosenLane].GetPositionZ(), false);
        else
          me->GetMotionMaster()->MovePoint(1, lefts[chosenLane].GetPositionX(), lefts[chosenLane].GetPositionY(),
                                           lefts[chosenLane].GetPositionZ(), false);
      ...
      case 7:
        if (!direction)
          me->GetMotionMaster()->MovePoint(2, lefts[chosenLane].GetPositionX(), lefts[chosenLane].GetPositionY(),
                                           lefts[chosenLane].GetPositionZ(), false);
        else
          me->GetMotionMaster()->MovePoint(2, rights[chosenLane].GetPositionX(), rights[chosenLane].GetPositionY(),
                                           rights[chosenLane].GetPositionZ(), false);
      ...
      case 11:
        if (!origin)
          me->GetMotionMaster()->MovePoint(3, prepareLandingLoc[0].GetPositionX(), prepareLandingLoc[0].GetPositionY(),
                                           prepareLandingLoc[0].GetPositionZ(), false);
        else
          me->GetMotionMaster()->MovePoint(3, prepareLandingLoc[1].GetPositionX(), prepareLandingLoc[1].GetPositionY(),
                                           prepareLandingLoc[1].GetPositionZ(), false);
    ...
    void UpdateAI(const uint32 diff) {
      ...
      case EVENT_FOG_CORRUPTION:
        ...
        if (pInstance) {
          switch (chosenLane) {
          case 0:
            pInstance->SetData((direction ? DATA_ACTIVATE_SOUTH_TO_LEFT : DATA_ACTIVATE_SOUTH_TO_RIGHT),
                               (uint32)me->GetPositionY());
            break;
          case 1:
            pInstance->SetData((direction ? DATA_ACTIVATE_CENTER_TO_LEFT : DATA_ACTIVATE_CENTER_TO_RIGHT),
                               (uint32)me->GetPositionY());
            break;
          case 2:
            pInstance->SetData((direction ? DATA_ACTIVATE_NORTH_TO_LEFT : DATA_ACTIVATE_NORTH_TO_RIGHT),
                               (uint32)me->GetPositionY());
            break;
          }
        }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_felmystAI(pCreature); }
};
...
public:
  ...
  struct mob_felmyst_vaporAI : public ScriptedAI {
    mob_felmyst_vaporAI(Creature *c) : ScriptedAI(c) { me->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE); }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new mob_felmyst_vaporAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new mob_felmyst_trailAI(pCreature); }
};
...
public:
  ...
  struct mob_unyielding_deadAI : public ScriptedAI {
    mob_unyielding_deadAI(Creature *c) : ScriptedAI(c) { pInstance = ((ScriptedInstance *)c->GetInstanceData()); }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new mob_unyielding_deadAI(pCreature); }
};
