...
public:
  ...
  struct boss_kalecgosAI : public ScriptedAI {
    ...
    void UpdateAI(const uint32 diff) {
      if (TalkTimer) {
        ...
        if (SpectralBlastTimer <= diff) {
          ...
          for (ThreatContainer::StorageType::const_iterator itr = m_threatlist.begin(); itr != m_threatlist.end();
               ++itr) {
            Unit *target = (*itr)->getTarget();
            if (target && target->GetTypeId() == TYPEID_PLAYER && target->GetGUID() != me->GetVictim()->GetGUID() &&
                target->GetPositionZ() > me->GetPositionZ() - 5 && !target->HasAura(AURA_SPECTRAL_EXHAUSTION)) {
    ...
    void KilledUnit(Unit * /*victim*/) { DoScriptText(RAND(SAY_EVIL_SLAY1, SAY_EVIL_SLAY2), me); }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_kalecgosAI(pCreature); }
};
...
public:
  ...
  struct boss_sathrovarrAI : public ScriptedAI {
    ...
    void EnterCombat(Unit * /*who*/) {
      if (Creature *Kalec = me->SummonCreature(MOB_KALEC, me->GetPositionX() + 10, me->GetPositionY() + 5,
                                               me->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 0)) {
    ...
    void JustDied(Unit * /*killer*/) {
      ...
      me->GetMap()->CreatureRelocation(me, me->GetPositionX(), me->GetPositionY(), DRAGON_REALM_Z,
                                       me->GetOrientation());
      TeleportAllPlayersBack();
      if (Creature *Kalecgos = Unit::GetCreature(*me, KalecgosGUID)) {
        CAST_AI(boss_kalecgos::boss_kalecgosAI, Kalecgos->AI())->TalkTimer = 1;
    ...
    void TeleportAllPlayersBack() {
      ...
      for (Map::PlayerList::const_iterator itr = playerList.begin(); itr != playerList.end(); ++itr) {
        ...
        if (player->IsInDist(&homePos, 50.0f) && player->GetPositionZ() <= DEMON_REALM_Z + 10.f) {
          ...
          player->TeleportTo(me->GetMap()->GetId(), player->GetPositionX(), player->GetPositionY(), DRAGON_REALM_Z + 5,
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_sathrovarrAI(pCreature); }
};
...
public:
  ...
  struct boss_kalecAI : public ScriptedAI {
    ...
    boss_kalecAI(Creature *c) : ScriptedAI(c) { pInstance = (ScriptedInstance *)c->GetInstanceData(); }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_kalecAI(pCreature); }
};
...
public:
  ...
  bool OnGossipHello(Player *pPlayer, GameObject *pGo) override {
    ...
    if (pPlayer->HasAura(AURA_SPECTRAL_EXHAUSTION, 0) ||
        (MAX_PLAYERS_IN_SPECTRAL_REALM && SpectralPlayers >= MAX_PLAYERS_IN_SPECTRAL_REALM))
