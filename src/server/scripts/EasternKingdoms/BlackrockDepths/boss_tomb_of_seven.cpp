...
public:
  ...
  bool OnGossipHello(Player *pPlayer, Creature *pCreature) override {
    if (pPlayer->GetQuestRewardStatus(QUEST_SPECTRAL_CHALICE) == 1 &&
        pPlayer->GetSkillValue(SKILL_MINING) >= DATA_SKILLPOINT_MIN && !pPlayer->HasSpell(SPELL_SMELT_DARK_IRON))
...
public:
  ...
  struct boss_doomrelAI : public ScriptedAI {
    boss_doomrelAI(Creature *c) : ScriptedAI(c) { pInstance = (ScriptedInstance *)c->GetInstanceData(); }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_doomrelAI(pCreature); }
