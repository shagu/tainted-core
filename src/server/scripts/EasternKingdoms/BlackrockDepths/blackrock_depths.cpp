...
enum JailBreak { QUEST_JAIL_BREAK = 4322 };
...
#define SAY_REGINALD_WINDSOR_13_1                                                                                      \
  "Open it. We need to hurry up. I can smell those Dark Irons coming a mile away and I can tell you one thing, "       \
  "they're COMING!"
...
#define SAY_REGINALD_WINDSOR_20_2                                                                                      \
  "Meet me at Maxwell's encampment. We'll go over the next stages of the plan there and figure out a way to decode "   \
  "my tablets without the decryption ring."
...
enum RocknotSays { SAY_GOT_BEER = -1230000 };
...
public:
  ...
  struct npc_marshal_reginald_windsorAI : public npc_escortAI {
    ...
    void EnterCombat(Unit * /*who*/) {
      switch (urand(0, 2)) {
      case 0:
        me->Say(SAY_WINDSOR_AGGRO1, LANG_UNIVERSAL, 0);
        break;
      case 1:
        me->Say(SAY_WINDSOR_AGGRO2, LANG_UNIVERSAL, 0);
        break;
      case 2:
    ...
    void JustDied(Unit * /*killer*/) { pInstance->SetData(DATA_QUEST_JAIL_BREAK, ENCOUNTER_STATE_FAILED); }
...
public:
  ...
  struct npc_grimstoneAI : public npc_escortAI {
    ...
    void SummonRingMob() {
      if (Creature *tmp = me->SummonCreature(RingMob[MobSpawnId], 608.960f, -235.322f, -53.907f, 1.857f,
    ...
    //@todo move them to center
    void SummonRingBoss() {
      if (Creature *tmp = me->SummonCreature(RingBoss[rand() % 6], 644.300f, -175.989f, -53.739f, 3.418f,
                                             TEMPSUMMON_DEAD_DESPAWN, 0))
    ...
    void HandleGameObject(uint32 id, bool open) { pInstance->HandleGameObject(pInstance->GetData64(id), open); }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return GetInstanceAI<npc_grimstoneAI>(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new mob_phalanxAI(pCreature); }
};
...
public:
  ...
  bool OnGossipHello(Player *pPlayer, Creature *pCreature) override {
    ...
    if (pPlayer->GetQuestRewardStatus(QUEST_A_BINDING_CONTRACT) != 1 &&
      ...
      pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_ITEM_GET_CONTRACT, GOSSIP_SENDER_MAIN,
...
public:
  ...
  struct npc_marshal_windsorAI : public npc_escortAI {
    ...
    void WaypointReached(uint32 waypointId) {
      ...
      case 19:
        me->SetVisible(false);
        me->SummonCreature(MOB_ENTRY_REGINALD_WINDSOR, 403.61f, -51.71f, -63.92f, 3.600434f, TEMPSUMMON_DEAD_DESPAWN,
                           0);
    ...
    void UpdateAI(const uint32 diff) {
      ...
      if ((pInstance->GetData(DATA_QUEST_JAIL_BREAK) == ENCOUNTER_STATE_IN_PROGRESS ||
           pInstance->GetData(DATA_QUEST_JAIL_BREAK) == ENCOUNTER_STATE_FAILED ||
           pInstance->GetData(DATA_QUEST_JAIL_BREAK) == ENCOUNTER_STATE_ENDED) &&
          pInstance->GetData(DATA_SUPPLY_ROOM) == ENCOUNTER_STATE_ENDED) {
        me->SetVisible(false);
...
public:
  ...
  bool OnGossipHello(Player *player, Creature *creature) override {
    if (player->GetQuestStatus(QUEST_JAIL_BREAK) == QUEST_STATUS_INCOMPLETE &&
        creature->FindNearestCreature(9023, 20.0f, true)) {
...
public:
  ...
  bool OnGossipHello(Player *player, Creature *creature) override {
    if (player->GetQuestStatus(QUEST_JAIL_BREAK) == QUEST_STATUS_INCOMPLETE &&
        creature->FindNearestCreature(9682, 20.0f, true)) {
      player->ADD_GOSSIP_ITEM(0, GOSSIP_DUGHAL, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
      player->SEND_GOSSIP_MENU(2846, creature->GetGUID());
    }
    return true;
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_crestAI(pCreature); }
};
...
public:
  ...
  struct npc_shillAI : public ScriptedAI {
    ...
    void EnterCombat(Unit * /*who*/) { pInstance->SetData(DATA_SHILL, ENCOUNTER_STATE_IN_PROGRESS); }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_shillAI(pCreature); }
};
...
public:
  ...
  struct npc_ograbisiAI : public ScriptedAI {
    ...
    void EnterCombat(Unit * /*who*/) { pInstance->SetData(DATA_OGRABISI, ENCOUNTER_STATE_IN_PROGRESS); }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_ograbisiAI(pCreature); }
};
...
public:
  ...
  struct npc_jazAI : public ScriptedAI {
    ...
    void EnterCombat(Unit * /*who*/) { pInstance->SetData(DATA_JAZ, ENCOUNTER_STATE_IN_PROGRESS); }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_jazAI(pCreature); }
};
...
public:
  ...
  struct npc_rocknotAI : public npc_escortAI {
    npc_rocknotAI(Creature *c) : npc_escortAI(c) { pInstance = (ScriptedInstance *)c->GetInstanceData(); }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return GetInstanceAI<npc_rocknotAI>(pCreature); }
