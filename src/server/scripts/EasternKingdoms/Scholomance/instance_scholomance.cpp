...
public:
  ...
  struct instance_scholomanceAI : public ScriptedInstance {
    instance_scholomanceAI(Map *pMap) : ScriptedInstance(pMap) { Initialize(); };

    // Lord Alexei Barov, Doctor Theolen Krastinov, The Ravenian, Lorekeeper Polkelt, Instructor Malicia and the Lady
    // Illucia Barov.
    bool IsBossDied[6];
  ...
  InstanceData *GetInstanceScript(InstanceMap *pMap) const override { return new instance_scholomanceAI(pMap); }
};
