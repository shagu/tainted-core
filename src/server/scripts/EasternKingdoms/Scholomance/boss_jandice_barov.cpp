...
public:
  ...
  struct boss_jandicebarovAI : public ScriptedAI {
    ...
    void SummonIllusions(Unit *victim) {
      if (Creature *Illusion =
              DoSpawnCreature(11439, irand(-9, 9), irand(-9, 9), 0, 0, TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, 60000))
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_jandicebarovAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new mob_illusionofjandicebarovAI(pCreature); }
};
