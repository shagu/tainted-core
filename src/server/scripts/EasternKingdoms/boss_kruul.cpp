...
public:
  ...
  struct boss_kruulAI : public ScriptedAI {
    ...
    void SummonHounds(Unit *pVictim) {
      if (Creature *Hound =
              DoSpawnCreature(19207, irand(-9, 9), irand(-9, 9), 0, 0, TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, 300000))
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_kruulAI(pCreature); }
};

void AddSC_boss_kruul() { new boss_kruul(); }
