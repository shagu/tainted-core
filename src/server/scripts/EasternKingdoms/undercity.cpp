...
public:
    ...
    void JustSummoned(Creature *summoned) {
      if (summoned->GetEntry() == ENTRY_HIGHBORNE_BUNNY) {
        if (Unit *pTarget = Unit::GetUnit(*summoned, targetGUID)) {
          ...
          init.Launch();
          pTarget->GetMap()->CreatureRelocation(me, pTarget->GetPositionX(), pTarget->GetPositionY(),
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_lady_sylvanas_windrunnerAI(pCreature); }
  ...
  bool OnQuestReward(Player * /*pPlayer*/, Creature *pCreature, const Quest *_Quest, uint32 /*slot*/) override {
    if (_Quest->GetQuestId() == 9180) {
      ...
      for (uint8 i = 0; i < 4; ++i)
        pCreature->SummonCreature(ENTRY_HIGHBORNE_LAMENTER, HighborneLoc[i][0], HighborneLoc[i][1], HIGHBORNE_LOC_Y,
...
public:
    ...
    void UpdateAI(const uint32 diff) {
      if (EventMove) {
        if (EventMove_Timer <= diff) {
          ...
          if (duration != 5000)
            sLog.outError("undercity : Travel time and duration from spline don't match (travel time : 5000, duration "
                          ": %i). Object (TypeId: %u, Entry: %u, GUID: %u)",
                          duration, me->GetTypeId(), me->GetEntry(), me->GetGUIDLow());
          me->GetMap()->CreatureRelocation(me, me->GetPositionX(), me->GetPositionY(), HIGHBORNE_LOC_Y_NEW,
                                           me->GetOrientation());
          EventMove = false;
        } else
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_highborne_lamenterAI(pCreature); }
};
