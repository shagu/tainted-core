...
public:
  ...
  struct instance_uldamanAI : public ScriptedInstance {
    instance_uldamanAI(Map *pMap) : ScriptedInstance(pMap) { Initialize(); };
    ...
    uint64 GetData64(uint32 identifier) {
      if (identifier == 0)
        return whoWokeArchaedasGUID;
      if (identifier == 1)
        return vaultWalker[0]; // VaultWalker1
      if (identifier == 2)
        return vaultWalker[1]; // VaultWalker2
      if (identifier == 3)
        return vaultWalker[2]; // VaultWalker3
      if (identifier == 4)
        return vaultWalker[3]; // VaultWalker4

      if (identifier == 5)
        return earthenGuardian[0];
      if (identifier == 6)
        return earthenGuardian[1];
      if (identifier == 7)
        return earthenGuardian[2];
      if (identifier == 8)
        return earthenGuardian[3];
      if (identifier == 9)
        return earthenGuardian[4];
      if (identifier == 10)
        return earthenGuardian[5];

      return 0;
    } // end GetData64

    std::string GetSaveData() { return str_data.c_str(); }
  ...
  InstanceData *GetInstanceScript(InstanceMap *pMap) const override { return new instance_uldamanAI(pMap); }
};
void AddSC_instance_uldaman() { new instance_uldaman(); }
