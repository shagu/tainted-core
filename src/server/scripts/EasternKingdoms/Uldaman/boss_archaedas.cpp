...
public:
  ...
  struct boss_archaedasAI : public ScriptedAI {
    boss_archaedasAI(Creature *c) : ScriptedAI(c) { pInstance = (ScriptedInstance *)me->GetInstanceData(); }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return GetInstanceAI<boss_archaedasAI>(pCreature); }
};
...
public:
  ...
  struct mob_archaedas_minionsAI : public ScriptedAI {
    mob_archaedas_minionsAI(Creature *c) : ScriptedAI(c) { pInstance = (ScriptedInstance *)me->GetInstanceData(); }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return GetInstanceAI<mob_archaedas_minionsAI>(pCreature); }
};
...
public:
  ...
  bool OnGossipHello(Player *player, GameObject *go) override {
    ...
    for (uint32 loop = 0; loop < 5; loop++) {
      if (altarOfArchaedasCount[loop] == player->GetGUID())
        alreadyUsed = true;
    }
    if (!alreadyUsed)
...
public:
  ...
  struct mob_stonekeepersAI : public ScriptedAI {
    mob_stonekeepersAI(Creature *c) : ScriptedAI(c) { pInstance = (ScriptedInstance *)me->GetInstanceData(); }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return GetInstanceAI<mob_stonekeepersAI>(pCreature); }
};

/* ScriptData
SDName: go_altar_of_the_keepers
SD%Complete: 100
SDComment: Need 3 people to activate to open the altar.  One by one the StoneKeepers will activate.  After all four are
dead than the door will open. SDCategory: Uldaman EndScriptData */
