...
enum Yells { YELL_AGGRO = -1810019, YELL_EVADE = -1810020 };
...
public:
    ...
    void UpdateAI(const uint32 diff) {
      ...
      if (uiResetTimer <= diff) {
        if (Creature *pBalinda = Unit::GetCreature(*me, uiBalindaGUID))
          if (me->GetDistance2d(pBalinda->GetHomePosition().GetPositionX(),
  ...
  CreatureAI *GetAI(Creature *_Creature) const { return new mob_water_elementalAI(_Creature); }
};
...
public:
  ...
  struct boss_balindaAI : public ScriptedAI {
    ...
    void EnterCombat(Unit * /*who*/) { DoScriptText(YELL_AGGRO, me); }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_balindaAI(pCreature); }
};
