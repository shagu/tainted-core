...
public:
  ...
  struct boss_nothAI : public ScriptedAI {
    ...
    void UpdateAI(const uint32 diff) {
      ...
      if (Summon_Timer <= diff) {
        ...
        for (uint8 i = 0; i < 6; i++)
          me->SummonCreature(C_PLAGUED_WARRIOR, 2684.804f, -3502.517f, 261.313f, 0, TEMPSUMMON_TIMED_OR_DEAD_DESPAWN,
                             80000);
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_nothAI(pCreature); }
};
