...
public:
  ...
  struct boss_highlord_mograineAI : public ScriptedAI {
    ...
    void EnterCombat(Unit *) { InitialYell(); }
    ...
    void UpdateAI(const uint32 diff) {
      ...
      if (RighteousFire_Timer <= diff) {
        ...
        RighteousFire_Timer -= diff;

      DoMeleeAttackIfReady();
    }
  };

  CreatureAI *GetAI(Creature *pCreature) const { return new boss_highlord_mograineAI(pCreature); }
};

void AddSC_boss_highlord_mograine() { new boss_highlord_mograine(); }
