...
SD%Complete: 100 Door Closes when reset mograin resses back up after 1st death whitemane resets to orginal spawn point,
whitemane does not inturupt spell casting. SDComment: Edited by Lee SDCategory: Scarlet Monastery EndScriptData */
...
public:
  ...
  struct boss_scarlet_commander_mograineAI : public ScriptedAI {
    ...
    void JustReachedHome() { instance->SetData(TYPE_MOGRAINE_AND_WHITE_EVENT, NOT_STARTED); }

    void JustDied() { instance->SetData(DATA_CHAPEL_DOOR, FAIL); }
    ...
    void KilledUnit(Unit * /*victim*/) { DoScriptText(SAY_MO_KILL, me); }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_scarlet_commander_mograineAI(pCreature); }
};
...
public:
  ...
  struct boss_high_inquisitor_whitemaneAI : public ScriptedAI {
    ...
    void EnterCombat(Unit * /*who*/) { DoScriptText(SAY_WH_INTRO, me); }

    void KilledUnit(Unit * /*victim*/) { DoScriptText(SAY_WH_KILL, me); }
    ...
    void UpdateAI(const uint32 diff) {
      ...
      if (_bCanResurrect) {
        if (Creature *mograine = ObjectAccessor::GetCreature(*me, instance->GetData64(DATA_MOGRAINE)))
          me->GetMotionMaster()->MoveChase(mograine, 1.0f,
                                           mograine->GetAngle(mograine->GetPositionX(), mograine->GetPositionY()));
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_high_inquisitor_whitemaneAI(pCreature); }
};
