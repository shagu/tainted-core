...
public:
  ...
  struct boss_interrogator_vishasAI : public ScriptedAI {
    boss_interrogator_vishasAI(Creature *c) : ScriptedAI(c) { pInstance = (ScriptedInstance *)me->GetInstanceData(); }
    ...
    void Reset() { ShadowWordPain_Timer = 5000; }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_interrogator_vishasAI(pCreature); }
};

void AddSC_boss_interrogator_vishas() { new boss_interrogator_vishas(); }
