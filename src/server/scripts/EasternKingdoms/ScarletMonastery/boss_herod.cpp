...
public:
  ...
  struct boss_herodAI : public ScriptedAI {
    ...
    void JustDied(Unit * /*killer*/) {
      for (uint8 i = 0; i < 20; ++i)
        me->SummonCreature(ENTRY_SCARLET_TRAINEE, 1939.18f, -431.58f, 17.09f, 6.22f, TEMPSUMMON_TIMED_OR_DEAD_DESPAWN,
                           600000 + i);
    }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_herodAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new mob_scarlet_traineeAI(pCreature); }
};
