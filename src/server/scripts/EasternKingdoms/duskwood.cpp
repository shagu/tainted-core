...
public:
  ...
  struct boss_twilight_corrupterAI : public ScriptedAI {
    ...
    void EnterCombat(Unit * /*who*/) { DoScriptText(CORRUPTER_YELL_CANT_STOP, me); }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_twilight_corrupterAI(pCreature); }
};
...
public:
  ...
  bool OnTrigger(Player *pPlayer, const AreaTriggerEntry * /*at*/) override {
    if (pPlayer->HasQuestForItem(21149)) {
      ...
      if (Creature *pCorrupter = pPlayer->SummonCreature(NPC_TWILINGHT_CORRUPTER, -10636.9f, -389.254f, 102.626f, 0,
                                                         TEMPSUMMON_TIMED_DESPAWN, CORRUPTER_DESPAWN_TIMER)) {
        pCorrupter->SetVisible(false);
        DoScriptText(CORRUPTER_YELL_COME, pCorrupter, pPlayer);
      }

      if (Creature *TCorrupter = pPlayer->SummonCreature(NPC_TWILINGHT_CORRUPTER, -10328.16f, -489.57f, 49.95f, 0,
                                                         TEMPSUMMON_MANUAL_DESPAWN, 60000)) {
...
public:
  ...
  bool OnGossipHello(Player *pPlayer, GameObject *pGO) override {
    ...
    if (pPlayer->GetQuestStatus(QUEST_BRIDE_OF_THE_EMBER) == QUEST_STATUS_INCOMPLETE) {
      pGO->SummonCreature(NPC_ELIZA, pGO->GetPosition(), TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT,
