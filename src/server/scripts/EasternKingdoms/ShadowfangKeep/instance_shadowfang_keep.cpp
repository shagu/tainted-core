...
public:
  ...
  struct instance_shadowfang_keepAI : public ScriptedInstance {
    instance_shadowfang_keepAI(Map *pMap) : ScriptedInstance(pMap) { Initialize(); };
    ...
    std::string GetSaveData() { return str_data; }
    ...
    void Update(uint32 uiDiff) {
      ...
      if (uiPhase) {
        if (uiTimer <= uiDiff) {
          ...
          case 1:
            pSummon =
                pArchmage->SummonCreature(pArchmage->GetEntry(), SpawnLocation[4], TEMPSUMMON_TIMED_DESPAWN, 10000);
  ...
  InstanceData *GetInstanceScript(InstanceMap *pMap) const override { return new instance_shadowfang_keepAI(pMap); }
};
void AddSC_instance_shadowfang_keep() { new instance_shadowfang_keep(); }
