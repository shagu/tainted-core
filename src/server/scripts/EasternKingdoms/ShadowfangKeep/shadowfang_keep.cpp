...
SD%Complete: 75
SDComment: npc_shadowfang_prisoner using escortAI for movement to door. Might need additional code in case being
attacked. Add proper texts/say(). SDCategory: Shadowfang Keep EndScriptData */
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_shadowfang_prisonerAI(pCreature); }
...
public:
  ...
  struct npc_arugal_voidwalkerAI : public ScriptedAI {
    ...
    void Reset() { uiDarkOffering = urand(290, 10); }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_arugal_voidwalkerAI(pCreature); }
};
