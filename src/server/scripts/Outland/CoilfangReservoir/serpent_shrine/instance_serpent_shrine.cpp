...
SD%Complete: 100
SDComment: Instance Data Scripts and functions to acquire mobs and set encounter status for use in various Serpent
Shrine Scripts SDCategory: Coilfang Resevoir, Serpent Shrine Cavern EndScriptData */
...
public:
  ...
  struct instance_serpentshrine_cavernAI : public ScriptedInstance {
    instance_serpentshrine_cavernAI(Map *pMap) : ScriptedInstance(pMap) { Initialize(); };
    ...
    void Update(uint32 diff) {
      ...
      if (LurkerSubEvent == LURKER_FISHING) {
        if (FishingTimer <= diff) {
          ...
          SetData(DATA_STRANGE_POOL,
                  IN_PROGRESS); // just fished, signal Lurker script to emerge and start fight, we use IN_PROGRESS so it
                                // won't get saved and lurker will be alway invis at start if server restarted
        } else
          FishingTimer -= diff;
      }
      // Water checks
          ...
          if (Player *pPlayer = i->GetSource()) {
            if (pPlayer->IsAlive() && /*i->GetSource()->GetPositionZ() <= -21.434931f*/ pPlayer->IsInWater()) {
              if (Water == WATERSTATE_SCALDING) {
                ...
                if (DoSpawnFrenzy) {
                  if (Creature *frenzy =
                          pPlayer->SummonCreature(MOB_COILFANG_FRENZY, pPlayer->GetPositionX(), pPlayer->GetPositionY(),
                                                  pPlayer->GetPositionZ(), pPlayer->GetOrientation(),
    ...
    uint32 GetData(uint32 type) {
      ...
      case DATA_CANSTARTPHASE3:
        if (ShieldGeneratorDeactivated[0] && ShieldGeneratorDeactivated[1] && ShieldGeneratorDeactivated[2] &&
            ShieldGeneratorDeactivated[3])
          return 1;
        break;
      case DATA_STRANGE_POOL:
        return StrangePool;
