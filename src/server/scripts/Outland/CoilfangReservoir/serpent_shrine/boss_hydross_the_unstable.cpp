...
SD%Complete: 90
SDComment: Some details and adjustments left to do, probably nothing major. Spawns may be spawned in different
way/location. SDCategory: Coilfang Resevoir, Serpent Shrine Cavern EndScriptData */
...
public:
  ...
  struct boss_hydross_the_unstableAI : public ScriptedAI {
    ...
    void SummonBeams() {
      Creature *beamer =
          me->SummonCreature(ENTRY_BEAM_DUMMY, -258.333f, -356.34f, 22.0499f, 5.90835f, TEMPSUMMON_CORPSE_DESPAWN, 0);
      if (beamer) {
      ...
      beamer =
          me->SummonCreature(ENTRY_BEAM_DUMMY, -219.918f, -371.308f, 22.0042f, 2.73072f, TEMPSUMMON_CORPSE_DESPAWN, 0);
      if (beamer) {
        beamer->CastSpell(me, SPELL_BLUE_BEAM, true);
        beamer->SetUInt32Value(UNIT_FIELD_DISPLAYID, 11686); // invisible
        beamer->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE);
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return GetInstanceAI<boss_hydross_the_unstableAI>(pCreature); }
};

void AddSC_boss_hydross_the_unstable() { new boss_hydross_the_unstable(); }