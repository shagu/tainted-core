...
public:
          ...
          if (pMap->IsDungeon()) {
            ...
            for (Map::PlayerList::const_iterator i = PlayerList.begin(); i != PlayerList.end(); ++i) {
              if (i->GetSource() && i->GetSource()->IsAlive() &&
                  me->HasInArc((double)diff / 20000 * (double)M_PI * 2, i->GetSource()) &&
                  me->IsWithinDist(i->GetSource(), SPOUT_DIST) && !i->GetSource()->IsInWater())
      ...
      } else // submerged
      {
        ...
        if (!Spawned) {
          ...
          for (uint8 i = 0; i < 9; ++i) {
            Creature *Summoned;
            if (i < 6)
              Summoned = me->SummonCreature(MOB_COILFANG_AMBUSHER, AddPos[i][0], AddPos[i][1], AddPos[i][2], 0,
                                            TEMPSUMMON_CORPSE_DESPAWN, 0);
            else
              Summoned = me->SummonCreature(MOB_COILFANG_GUARDIAN, AddPos[i][0], AddPos[i][1], AddPos[i][2], 0,
                                            TEMPSUMMON_CORPSE_DESPAWN, 0);
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return GetInstanceAI<boss_the_lurker_belowAI>(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new mob_coilfang_ambusherAI(pCreature); }
};
