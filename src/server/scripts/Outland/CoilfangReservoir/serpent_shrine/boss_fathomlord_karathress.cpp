...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return GetInstanceAI<boss_fathomlord_karathressAI>(pCreature); }
};
...
public:
  ...
  struct boss_fathomguard_sharkkisAI : public ScriptedAI {
    boss_fathomguard_sharkkisAI(Creature *c) : ScriptedAI(c) { pInstance = (ScriptedInstance *)c->GetInstanceData(); }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return GetInstanceAI<boss_fathomguard_sharkkisAI>(pCreature); }
};
...
public:
  ...
  struct boss_fathomguard_tidalvessAI : public ScriptedAI {
    boss_fathomguard_tidalvessAI(Creature *c) : ScriptedAI(c) { pInstance = (ScriptedInstance *)c->GetInstanceData(); }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return GetInstanceAI<boss_fathomguard_tidalvessAI>(pCreature); }
};
...
public:
  ...
  struct boss_fathomguard_caribdisAI : public ScriptedAI {
    boss_fathomguard_caribdisAI(Creature *c) : ScriptedAI(c) { pInstance = (ScriptedInstance *)c->GetInstanceData(); }
    ...
    void UpdateAI(const uint32 diff) {
      ...
      if (Cyclone_Timer <= diff) {
        ...
        Cyclone_Timer = 30000 + rand() % 10000;
        Creature *Cyclone = me->SummonCreature(CREATURE_CYCLONE, me->GetPositionX(), me->GetPositionY(),
                                               me->GetPositionZ(), float(rand() % 5), TEMPSUMMON_TIMED_DESPAWN, 15000);
        if (Cyclone) {
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return GetInstanceAI<boss_fathomguard_caribdisAI>(pCreature); }
};
