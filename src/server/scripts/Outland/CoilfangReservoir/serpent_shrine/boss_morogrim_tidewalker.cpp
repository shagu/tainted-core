...
float MurlocCords[10][5] = {{21920, 424.36f, -715.4f, -7.14f, 0.124f},   {21920, 425.13f, -719.3f, -7.14f, 0.124f},
                            {21920, 425.05f, -724.23f, -7.14f, 0.124f},  {21920, 424.91f, -728.68f, -7.14f, 0.124f},
                            {21920, 424.84f, -732.18f, -7.14f, 0.124f},  {21920, 321.05f, -734.2f, -13.15f, 0.124f},
                            {21920, 321.05f, -729.4f, -13.15f, 0.124f},  {21920, 321.05f, -724.03f, -13.15f, 0.124f},
                            {21920, 321.05f, -718.73f, -13.15f, 0.124f}, {21920, 321.05f, -714.24f, -13.15f, 0.124f}};
...
public:
  ...
  struct boss_morogrim_tidewalkerAI : public ScriptedAI {
    boss_morogrim_tidewalkerAI(Creature *c) : ScriptedAI(c) { pInstance = (ScriptedInstance *)c->GetInstanceData(); }
    ...
    void UpdateAI(const uint32 diff) {
      ...
      if (Earthquake_Timer <= diff) {
        if (!Earthquake) {
          ...
          for (uint8 i = 0; i < 10; ++i) {
            ...
            Creature *Murloc =
                me->SummonCreature(MurlocCords[i][0], MurlocCords[i][1], MurlocCords[i][2], MurlocCords[i][3],
                                   MurlocCords[i][4], TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 10000);
            if (pTarget && Murloc)
      ...
      if (!Phase2) {
        ...
        if (WateryGlobules_Timer <= diff) {
          ...
          set<int>::const_iterator itr;
          for (uint8 g = 0; g < 4;
               g++) // one unit can't cast more than one spell per update, so some players have to cast for us XD
          {
            counter = 0;
            do {
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return GetInstanceAI<boss_morogrim_tidewalkerAI>(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new mob_water_globuleAI(pCreature); }
};
