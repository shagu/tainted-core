...
float ElementPos[8][4] = {{8.3f, -835.3f, 21.9f, 5},   {53.4f, -835.3f, 21.9f, 4.5f}, {96, -861.9f, 21.8f, 4},
                          {96, -986.4f, 21.4f, 2.5f},  {54.4f, -1010.6f, 22, 1.8f},   {9.8f, -1012, 21.7f, 1.4f},
                          {-35, -987.6f, 21.5f, 0.8f}, {-58.9f, -901.6f, 21.5f, 6}};

float ElementWPPos[8][3] = {{71.700752f, -883.905884f, 41.097168f},  {45.039848f, -868.022827f, 41.097015f},
                            {14.585141f, -867.894470f, 41.097061f},  {-25.415508f, -906.737732f, 41.097061f},
                            {-11.801594f, -963.405884f, 41.097067f}, {14.556657f, -979.051514f, 41.097137f},
                            {43.466549f, -979.406677f, 41.097027f},  {69.945908f, -964.663940f, 41.097054f}};

float SporebatWPPos[8][3] = {{31.6f, -896.3f, 59.1f}, {9.1f, -913.9f, 56}, {5.2f, -934.4f, 52.4f},
                             {20.7f, -946.9f, 49.7f}, {41, -941.9f, 51},   {47.7f, -927.3f, 55},
                             {42.2f, -912.4f, 51.7f}, {27, -905.9f, 50}};

float CoilfangElitePos[3][4] = {{28.84f, -923.28f, 42.9f, 6},
                                {31.183281f, -953.502625f, 41.523602f, 1.640957f},
                                {58.895180f, -923.124268f, 41.545307f, 3.152848f}};

float CoilfangStriderPos[3][4] = {{66.427010f, -948.778503f, 41.262245f, 2.584220f},
                                  {7.513962f, -959.538208f, 41.300422f, 1.034629f},
                                  {-12.843201f, -907.798401f, 41.239620f, 6.087094f}};

float ShieldGeneratorChannelPos[4][4] = {{49.6262f, -902.181f, 43.0975f, 3.95683f},
...
public:
  VashjSurgeAura(SpellEntry *spell, uint32 eff, int32 *bp, Unit *pTarget, Unit *caster)
      : Aura(spell, eff, bp, pTarget, caster, NULL) {}
};
...
public:
  ...
  struct boss_lady_vashjAI : public ScriptedAI {
    boss_lady_vashjAI(Creature *c) : ScriptedAI(c) {
      ...
      c->SetFlag(UNIT_FIELD_FLAGS,
                 UNIT_FLAG_NON_ATTACKABLE); // set it only once on Creature create (no need do intro if wiped)
    }
    ...
    void MoveInLineOfSight(Unit *who) {
      ...
      if (who->isTargetableForAttack() && who->isInAccessiblePlaceFor(me) && me->IsHostileTo(who)) {
        ...
        if (me->IsWithinDistInMap(who, attackRadius) && me->GetDistanceZ(who) <= CREATURE_Z_ATTACK_RANGE &&
    ...
    void CastShootOrMultishot() {
      ...
      case 0:
        // Shoot
        // Used in Phases 1 and 3 after Entangle or while having nobody in melee range. A shot that hits her target for
        // 4097-5543 Physical damage.
        DoCastVictim(SPELL_SHOOT);
        break;
      case 1:
        // Multishot
        // Used in Phases 1 and 3 after Entangle or while having nobody in melee range. A shot that hits 1 person and 4
        // people around him for 6475-7525 physical damage.
    ...
    void UpdateAI(const uint32 diff) {
      ...
      if (Phase == 1 || Phase == 3) {
        ...
        if (ShockBlast_Timer <= diff) {
          // Shock Burst
          // Randomly used in Phases 1 and 3 on Vashj's target, it's a Shock spell doing 8325-9675 nature damage and
          // stunning the target for 5 seconds, during which she will not attack her target but switch to the next person
          // on the aggro list.
        ...
        if (StaticCharge_Timer <= diff) {
          ...
          // Nature damage to the target and everybody in about 5 yards around it, every 1 seconds for 30 seconds. It can
          // be removed by Cloak of Shadows, Iceblock, Divine Shield, etc, but not by Cleanse or Dispel Magic.
          Unit *pTarget = NULL;
        ...
        if (Entangle_Timer <= diff) {
          if (!Entangle) {
            // Entangle
            // Used in Phases 1 and 3, it casts Entangling Roots on everybody in a 15 yard radius of Vashj, immobilzing
            // them for 10 seconds and dealing 500 damage every 2 seconds. It's not a magic effect so it cannot be
            // dispelled, but is removed by various buffs such as Cloak of Shadows or Blessing of Freedom.
        ...
        if (Phase == 1) {
          ...
          if (HealthBelowPct(70)) {
            // Phase 2 begins when Vashj hits 70%. She will run to the middle of her platform and surround herself in a
            // shield making her invulerable.
            Phase = 2;
            ...
            Creature *pCreature;
            for (uint8 i = 0; i < 4; ++i) {
              pCreature = me->SummonCreature(SHIED_GENERATOR_CHANNEL, ShieldGeneratorChannelPos[i][0],
                                             ShieldGeneratorChannelPos[i][1], ShieldGeneratorChannelPos[i][2],
                                             ShieldGeneratorChannelPos[i][3], TEMPSUMMON_CORPSE_DESPAWN, 0);
              if (pCreature)
        ...
        // Phase 3
          ...
          if (SummonSporebat_Timer <= diff) {
            ...
            Sporebat = me->SummonCreature(TOXIC_SPOREBAT, SPOREBAT_X, SPOREBAT_Y, SPOREBAT_Z, SPOREBAT_O,
      ...
      // Phase 2
        ...
        if (ForkedLightning_Timer <= diff) {
          // Forked Lightning
          // Used constantly in Phase 2, it shoots out completely randomly targeted bolts of lightning which hit
          // everybody in a roughtly 60 degree cone in front of Vashj for 2313-2687 nature damage.
          Unit *pTarget = NULL;
          pTarget = SelectUnit(SELECT_TARGET_RANDOM, 0);
        ...
        if (EnchantedElemental_Timer <= diff) {
          me->SummonCreature(ENCHANTED_ELEMENTAL, ElementPos[EnchantedElemental_Pos][0],
                             ElementPos[EnchantedElemental_Pos][1], ElementPos[EnchantedElemental_Pos][2],
                             ElementPos[EnchantedElemental_Pos][3], TEMPSUMMON_CORPSE_DESPAWN, 0);
        ...
        if (TaintedElemental_Timer <= diff) {
          uint32 pos = rand() % 8;
          me->SummonCreature(TAINTED_ELEMENTAL, ElementPos[pos][0], ElementPos[pos][1], ElementPos[pos][2],
                             ElementPos[pos][3], TEMPSUMMON_DEAD_DESPAWN, 0);
        ...
        if (CoilfangElite_Timer <= diff) {
          ...
          CoilfangElite = me->SummonCreature(COILFANG_ELITE, CoilfangElitePos[pos][0], CoilfangElitePos[pos][1],
                                             CoilfangElitePos[pos][2], CoilfangElitePos[pos][3],
        ...
        if (CoilfangStrider_Timer <= diff) {
          ...
          CoilfangStrider = me->SummonCreature(COILFANG_STRIDER, CoilfangStriderPos[pos][0], CoilfangStriderPos[pos][1],
                                               CoilfangStriderPos[pos][2], CoilfangStriderPos[pos][3],
                                               TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 5000);
          if (CoilfangStrider) {
            Unit *pTarget = NULL;
            pTarget = SelectUnit(SELECT_TARGET_RANDOM, 0);
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return GetInstanceAI<boss_lady_vashjAI>(pCreature); }
};
...
public:
  ...
  struct mob_enchanted_elementalAI : public ScriptedAI {
    mob_enchanted_elementalAI(Creature *c) : ScriptedAI(c) { pInstance = (ScriptedInstance *)c->GetInstanceData(); }
    ...
    void UpdateAI(const uint32 diff) {
      ...
      if (move <= diff) {
        ...
        if (Vashj) {
          if (!Vashj->IsInCombat() || CAST_AI(boss_lady_vashj::boss_lady_vashjAI, Vashj->AI())->Phase != 2 ||
              Vashj->isDead()) {
            // call Unsummon()
            me->Kill(me);
          }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new mob_enchanted_elementalAI(pCreature); }
};
...
public:
  ...
  struct mob_tainted_elementalAI : public ScriptedAI {
    mob_tainted_elementalAI(Creature *c) : ScriptedAI(c) { pInstance = (ScriptedInstance *)c->GetInstanceData(); }
    ...
    void EnterCombat(Unit *who) { me->AddThreat(who, 0.1f); }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new mob_tainted_elementalAI(pCreature); }
};
...
public:
  ...
  struct mob_toxic_sporebatAI : public ScriptedAI {
    ...
    void UpdateAI(const uint32 diff) {
      ...
      if (movement_timer <= diff) {
        ...
        me->GetMotionMaster()->MovePoint(1, SporebatWPPos[rndpos][0], SporebatWPPos[rndpos][1],
                                         SporebatWPPos[rndpos][2]);
        movement_timer = 6000;
      } else
        movement_timer -= diff;
      ...
      if (bolt_timer <= diff) {
        ...
        pTarget = SelectUnit(SELECT_TARGET_RANDOM, 0);
        if (pTarget) {
          Creature *trig = me->SummonCreature(TOXIC_SPORES_TRIGGER, pTarget->GetPositionX(), pTarget->GetPositionY(),
                                              pTarget->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN, 30000);
          if (trig) {
            trig->SetFaction(14);
      ...
      if (Check_Timer <= diff) {
        if (pInstance) {
          ...
          if (!Vashj || (Vashj && !Vashj->IsAlive()) ||
              (Vashj && CAST_AI(boss_lady_vashj::boss_lady_vashjAI, CAST_CRE(Vashj)->AI())->Phase != 3)) {
            // remove
            me->setDeathState(DEAD);
            me->RemoveCorpse();
            me->SetFaction(35);
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new mob_toxic_sporebatAI(pCreature); }
};
...
public:
  ...
  struct mob_coilfang_striderAI : public ScriptedAI {
    mob_coilfang_striderAI(Creature *c) : ScriptedAI(c) { Reset(); }

    uint32 Blast_Timer;

    void Reset() { Blast_Timer = 8000; }

    void EnterCombat(Unit *) { DoCast(me, SPELL_PANIC, true); }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new mob_coilfang_striderAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new mob_shield_generator_channelAI(pCreature); }
};
  ...
  bool OnUse(Player *pPlayer, Item * /*_Item*/, SpellCastTargets const &targets) override {
    ...
    if (Vashj && CAST_AI(boss_lady_vashj::boss_lady_vashjAI, Vashj->AI())->Phase == 2) {
      if (targets.getGOTarget() && targets.getGOTarget()->GetTypeId() == TYPEID_GAMEOBJECT) {
        ...
        Unit *Channel = NULL;
        Channel = Unit::GetCreature(
            *Vashj,
            CAST_AI(boss_lady_vashj::boss_lady_vashjAI, Vashj->AI())->ShieldGeneratorChannel[channel_identifier]);
        if (Channel) {
          // call Unsummon()
