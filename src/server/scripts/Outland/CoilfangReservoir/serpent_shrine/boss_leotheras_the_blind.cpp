...
public:
  InsidiousAura(SpellEntry *spell, uint32 eff, int32 *bp, Unit *pTarget, Unit *caster)
...
public:
  ...
  struct mob_inner_demonAI : public ScriptedAI {
    mob_inner_demonAI(Creature *c) : ScriptedAI(c) { victimGUID = 0; }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new mob_inner_demonAI(pCreature); }
};
...
public:
  ...
  struct boss_leotheras_the_blindAI : public ScriptedAI {
      ...
      for (uint8 i = 0; i < 5; ++i) {
        if (InnderDemon[i] > 0) {
          ...
          if (pUnit && pUnit->IsAlive()) {
            Unit *pUnit_pTarget =
                Unit::GetUnit(*pUnit, CAST_AI(mob_inner_demon::mob_inner_demonAI, pUnit->AI())->victimGUID);
    ...
    void UpdateAI(const uint32 diff) {
      ...
      if (me->HasAura(SPELL_WHIRLWIND, 0)) {
        if (Whirlwind_Timer <= diff) {
          ...
          if (newTarget) {
            ...
            me->GetMotionMaster()->MovePoint(0, newTarget->GetPositionX(), newTarget->GetPositionY(),
                                             newTarget->GetPositionZ());
          }
      ...
      if (NeedThreatReset && !me->HasAura(SPELL_WHIRLWIND, 0)) {
        // when changing forms seting timers (or when ending whirlwind - to avoid adding new variable i use
        // Whirlwind_Timer to countdown 2s while whirlwinding)
        if (DemonForm)
          InnerDemons_Timer = 30000;
        else
          Whirlwind_Timer = 15000;
      ...
      if (!DemonForm) {
        ...
        if (ChaosBlast_Timer <= diff) {
          ...
          if (me->GetDistance(me->GetVictim()) < 30) {
            ...
            int damage = 100;
            me->CastCustomSpell(me->GetVictim(), SPELL_CHAOS_BLAST, &damage, NULL, NULL, false, NULL, NULL,
                                me->GetGUID());
          }
        ...
        if (InnerDemons_Timer <= diff) {
          ...
          for (ThreatContainer::StorageType::const_iterator itr = ThreatList.begin(); itr != ThreatList.end(); ++itr) {
            ...
            if (tempTarget && tempTarget->GetTypeId() == TYPEID_PLAYER &&
                tempTarget->GetGUID() != me->GetVictim()->GetGUID() && TargetList.size() < 5)
          ...
          for (std::vector<Unit *>::const_iterator itr = TargetList.begin(); itr != TargetList.end(); ++itr) {
            if ((*itr) && (*itr)->IsAlive()) {
              Creature *demon =
                  me->SummonCreature(INNER_DEMON_ID, (*itr)->GetPositionX() + 10, (*itr)->GetPositionY() + 10,
                                     (*itr)->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 5000);
              if (demon) {
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return GetInstanceAI<boss_leotheras_the_blindAI>(pCreature); }
};
...
public:
  ...
  struct boss_leotheras_the_blind_demonformAI : public ScriptedAI {
    ...
    void StartEvent() { DoScriptText(SAY_FREE, me); }
    ...
    void EnterCombat(Unit * /*who*/) { StartEvent(); }
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return GetInstanceAI<mob_greyheart_spellbinderAI>(pCreature); }
};
