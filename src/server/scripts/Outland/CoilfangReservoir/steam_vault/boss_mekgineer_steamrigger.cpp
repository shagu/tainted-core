...
SD%Complete: 60
SDComment: Mechanics' interrrupt heal doesn't work very well, also a proper movement needs to be implemented -> summon
further away and move towards pTarget to repair. SDCategory: Coilfang Resevoir, The Steamvault EndScriptData */
...
public:
  ...
  struct boss_mekgineer_steamriggerAI : public ScriptedAI {
    ...
    void KilledUnit(Unit *victim) { DoScriptText(RAND(SAY_SLAY_1, SAY_SLAY_2, SAY_SLAY_3), me); }
    ...
    void SummonMechanichs() {
      ...
      me->SummonCreature(NPC_STREAMRIGGER_MECHANIC, me->GetPositionX() + 15.0f, me->GetPositionY() + 15.0f,
                         me->GetPositionZ(), 0.0f, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 30000);
      me->SummonCreature(NPC_STREAMRIGGER_MECHANIC, me->GetPositionX() - 15.0f, me->GetPositionY() + 15.0f,
                         me->GetPositionZ(), 0.0f, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 30000);
      me->SummonCreature(NPC_STREAMRIGGER_MECHANIC, me->GetPositionX() - 15.0f, me->GetPositionY() - 15.0f,
                         me->GetPositionZ(), 0.0f, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 30000);
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return GetInstanceAI<boss_mekgineer_steamriggerAI>(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return GetInstanceAI<mob_steamrigger_mechanicAI>(pCreature); }
};
