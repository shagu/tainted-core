...
public:
  ...
  struct boss_hungarfenAI : public ScriptedAI {
    boss_hungarfenAI(Creature *c) : ScriptedAI(c) { HeroicMode = me->GetMap()->IsHeroic(); }
    ...
    void UpdateAI(const uint32 diff) {
      ...
      if (Mushroom_Timer <= diff) {
        if (Unit *pTarget = SelectUnit(SELECT_TARGET_RANDOM, 0))
          me->SummonCreature(17990, pTarget->GetPositionX() + float(rand() % 8),
                             pTarget->GetPositionY() + float(rand() % 8), pTarget->GetPositionZ(), float(rand() % 5),
                             TEMPSUMMON_TIMED_DESPAWN, 22000);
        else
          me->SummonCreature(17990, me->GetPositionX() + float(rand() % 8), me->GetPositionY() + float(rand() % 8),
                             me->GetPositionZ(), float(rand() % 5), TEMPSUMMON_TIMED_DESPAWN, 22000);
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_hungarfenAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new mob_underbog_mushroomAI(pCreature); }
};
