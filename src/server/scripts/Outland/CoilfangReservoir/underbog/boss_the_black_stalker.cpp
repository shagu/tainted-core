...
public:
  ...
  struct boss_the_black_stalkerAI : public ScriptedAI {
    boss_the_black_stalkerAI(Creature *c) : ScriptedAI(c) { HeroicMode = me->GetMap()->IsHeroic(); }
    ...
    void JustDied(Unit * /*who*/) {
      for (std::list<uint64>::iterator i = Striders.begin(); i != Striders.end(); ++i)
        if (Creature *strider = Unit::GetCreature(*me, *i)) {
          ...
          strider->DealDamage(strider, strider->GetMaxHealth(), NULL, DIRECT_DAMAGE, SPELL_SCHOOL_MASK_NORMAL, NULL,
                              false);
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_the_black_stalkerAI(pCreature); }
};

void AddSC_boss_the_black_stalker() { new boss_the_black_stalker(); }
