...
SD%Complete: 98
SDComment: Quest support: 10519, 10583, 10601, 10814, 10804, 10854, 10458, 10481, 10480, 11082, 10781, 10451. Vendor
Drake Dealer Hurlunk, 10563, 10596. SDCategory: Shadowmoon Valley EndScriptData */
...
enum CinematicCreature { LORD_ILLIDAN = 1, TORLOTH = 0 };
...
static TorlothCinematic TorlothAnim[] = {{SAY_TORLOTH_DIALOGUE1, TORLOTH, 2000},
...
enum { SPELL_CLEAVE = 15284, SPELL_SHADOWFURY = 39082, SPELL_SPELL_REFLECTION = 33961 };
...
#define GOSSIP_SGCR3                                                                                                   \
  "Very well. I will return to the Black Temple and notify Lord Illidan of your unwillingness to carry out his "       \
  "wishes. I suggest you make arrangements with your subordinates and let them know that you will soon be leaving "    \
  "this world."
#define GOSSIP_SGCR4                                                                                                   \
  "Do I need to go into all the gory details? I think we are both well aware of what Lord Illidan does with those "    \
  "that would oppose his word. Now, I must be going! Farewell, Ruusk! Forever..."
...
enum sunfurywarlock { SPELL_BEAM_BANISH = 39123 };
...
#define SAY_TIMER_START                                                                                                \
  "I've rigged a few control consoles down the slope. Get to one quickly and take control of the fel reaver! You'll "  \
...
#define KARSIUS_SAY_TEXT_1                                                                                             \
  "Your pathetic attempt to escape will be short lived, Gorefiend. Let the $s go and submit! Even with your armor "    \
  "and weapons, you cannot defeat the ancients!"
#define KARSIUS_SAY_TEXT_2 "What... have you done..."
#define TERON_WHISPER                                                                                                  \
...
#define ICARIUS_SAY_2                                                                                                  \
  "Zarath, I am perfectly capable of making it up this road to Eclipse Point. If we do not deliver the missive, Lord " \
  "Illidan will have both of our heads! You are dismissed!"
...
#define SPIRIT_FIRE_1                                                                                                  \
  "Yes... Well enough for the elements that are here but the cipher is known to another... The spirits of fire in "    \
  "turmoil... If this force is not stopped, the world where there these mortals came from will cease."
...
public:
  ...
  struct mob_mature_netherwing_drakeAI : public ScriptedAI {
    mob_mature_netherwing_drakeAI(Creature *c) : ScriptedAI(c) { PlayerGUID = 0; }
    ...
    void SpellHit(Unit *caster, const SpellEntry *spell) {
      ...
      if (caster->GetTypeId() == TYPEID_PLAYER && spell->Id == SPELL_PLACE_CARCASS &&
          !me->HasAura(SPELL_JUST_EATEN, 0) && !PlayerGUID) {
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new mob_mature_netherwing_drakeAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new mob_enslaved_netherwing_drakeAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new mob_dragonmaw_peonAI(pCreature); }
};
...
public:
  ...
  struct npc_karynakuAI : public ScriptedAI {
    ...
    void Reset() { ZuluStart = false; }

    bool ZuluStart;

    void JustSummoned(Creature *summoned) { summoned->CastSpell(summoned, 42510, true); }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_karynakuAI(pCreature); }
  ...
  bool OnQuestAccept(Player *player, Creature *creature, Quest const *quest) override {
    ...
    if (quest->GetQuestId() == 10870) // Ally of the Netherwing
    {
      ...
      sLog.outError("OSCR: Player %s started quest 10870 which has disabled taxi node, need to be fixed in core",
                    player->GetName());
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_oronok_tornheartAI(pCreature); }
...
public:
  ...
  struct npc_overlord_morghorAI : public ScriptedAI {
    ...
    void StartEvent() {
      ...
      Unit *Illidan =
          me->SummonCreature(C_ILLIDAN, -5107.83f, 602.584f, 85.2393f, 4.92598f, TEMPSUMMON_CORPSE_DESPAWN, 0);
      if (Illidan) {
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_overlord_morghorAI(pCreature); }
...
public:
  ...
  struct npc_earthmender_wildaAI : public npc_escortAI {
    ...
    void SummonAssassin() {
      ...
      Unit *CoilskarAssassin = me->SummonCreature(NPC_COILSKAR_ASSASSIN, me->GetPositionX(), me->GetPositionY(),
                                                  me->GetPositionZ(), me->GetOrientation(), TEMPSUMMON_DEAD_DESPAWN, 0);
      if (CoilskarAssassin) {
        switch (rand() % 2) {
        case 0:
        ...
        sLog.outError("OSCR ERROR: Coilskar Assassin couldn't be summmoned");
    }
    ...
    void UpdateAI(const uint32 diff) { npc_escortAI::UpdateAI(diff); }
  };

  CreatureAI *GetAI(Creature *pCreature) const { return new npc_earthmender_wildaAI(pCreature); }
...
public:
  ...
  struct mob_torlothAI : public ScriptedAI {
    ...
    void EnterEvadeMode() { me->ForcedDespawn(); }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new mob_torlothAI(pCreature); }
};
...
public:
  ...
  struct npc_lord_illidan_stormrageAI : public Scripted_NoMovementAI {
    ...
    void SummonWave() {
      ...
      for (uint8 i = 0; i < uiCount; ++i) {
        ...
        if (Creature *pSpawn = me->SummonCreature(WavesInfo[m_uiWaveCount].uiCreatureId, fLocX, fLocY, fLocZ, fOrient,
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_lord_illidan_stormrageAI(pCreature); }
};
  ...
  bool OnQuestAccept(Player *pPlayer, GameObject * /*pGo*/, Quest const *pQuest) override {
    if (pQuest->GetQuestId() == QUEST_BATTLE_OF_THE_CRIMSON_WATCH)
      if (Creature *pLordIllidan = GetClosestCreatureWithEntry(pPlayer, NPC_LORD_ILLIDAN, 50.0))
        if (npc_lord_illidan_stormrage::npc_lord_illidan_stormrageAI *pIllidanAI =
                dynamic_cast<npc_lord_illidan_stormrage::npc_lord_illidan_stormrageAI *>(pLordIllidan->AI()))
...
public:
    ...
    void JustDied(Unit * /*killer*/) {
      ...
      if (Summoned) {
        ...
        if (totemOspirits) {
          ...
          Summoned->GetMotionMaster()->MovePoint(0, totemOspirits->GetPositionX(), totemOspirits->GetPositionY(),
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_enraged_spiritAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *creature) const { return new npc_azalothAI(creature); }
};
...
public:
  ...
  struct npc_sunfurywarlockAI : public ScriptedAI {
    ...
    void Reset() { CastBeam = false; }

    npc_sunfurywarlockAI(Creature *c) : ScriptedAI(c) {}

    void EnterCombat(Unit *who) { me->InterruptNonMeleeSpells(true); }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_sunfurywarlockAI(pCreature); }
};
      ...
      if (!UpdateVictim()) {
        if (Creature *summoner = me->FindNearestCreature(61030, 100.0f, true)) {
          if (infernal_summon_timer <= diff) {
            ...
            infernal_summon_timer -= diff;
        }
      }
    }
  };

  CreatureAI *GetAI(Creature *pCreature) const { return new legion_infernal_summon_triggerAI(pCreature); }
};
...
public:
  ...
  struct legion_infernal_summoner_triggerAI : public ScriptedAI {
    ...
    void SpellHit(Unit *Hitter, const SpellEntry *Spellkind) {
      if (Spellkind->Id == 33814 && !spellHit) {
        me->SummonCreature(21419, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), 0,
                           TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 180000);
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new legion_infernal_summoner_triggerAI(pCreature); }
};
...
public:
  ...
  struct legion_hold_device_triggerAI : public ScriptedAI {
    ...
    void JustSummoned(Creature *summoned) { summoned->CastSpell(summoned, 41232, true); }

    void MoveInLineOfSight(Unit *pWho) {
      if (Player *plWho = pWho->GetCharmerOrOwnerPlayerOrPlayerItself()) {
        if (plWho->GetQuestStatus(QUEST_LEGION_HOLD2) == QUEST_STATUS_INCOMPLETE ||
            plWho->GetQuestStatus(QUEST_LEGION_HOLD1) == QUEST_STATUS_INCOMPLETE && plWho->GetDistance(me) < 15.0f &&
                plWho->HasItemCount(30638, 1, false) && plWho->HasAura(SPELL_BOX)) {
          switch (me->GetEntry()) {
          case 61031:
            if (!LegionEventStart) {
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new legion_hold_device_triggerAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_deathbringer_jovaanAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_shadowcouncil_warlockAI(pCreature); }
};
...
public:
  ...
  struct npc_zuluhed_the_whackedAI : public ScriptedAI {
    ...
    void MoveInLineOfSight(Unit *pWho) {
      if (Player *plWho = pWho->GetCharmerOrOwnerPlayerOrPlayerItself()) {
        if (plWho->GetQuestStatus(QUEST_ZULUHED_THE_WHACKED) == QUEST_STATUS_INCOMPLETE &&
            plWho->GetDistance(me) < 35.0f) {
          switch (me->GetEntry()) {
          case 11980:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_zuluhed_the_whackedAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_dragonmaw_archerAI(pCreature); }
};
...
public:
  ...
  struct npc_karsius_the_ancient_watcherAI : public ScriptedAI {
    ...
    void MoveInLineOfSight(Unit *pWho) {
      if (Player *plWho = pWho->GetCharmerOrOwnerPlayerOrPlayerItself()) {
        if (plWho->GetQuestStatus(QUEST_TERON_GOREFIEND_I_AM_ALLIANCE) == QUEST_STATUS_INCOMPLETE ||
            plWho->GetQuestStatus(QUEST_TERON_GOREFIEND_I_AM_HORDE) == QUEST_STATUS_INCOMPLETE &&
                plWho->GetDistance(me) < 35.0f) {
          switch (me->GetEntry()) {
          case 21877:
            playerGUID = plWho->GetGUID();
    ...
    void JustDied(Unit * /*killer*/) {
      ...
      me->SummonCreature(21876, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN,
                         15000);
    }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_karsius_the_ancient_watcherAI(pCreature); }
};
...
public:
  ...
  struct teron_triggerAI : public ScriptedAI {
    ...
    void MoveInLineOfSight(Unit *pWho) {
      if (Player *plWho = pWho->GetCharmerOrOwnerPlayerOrPlayerItself()) {
        if (plWho->GetQuestStatus(QUEST_TERON_GOREFIEND_I_AM_ALLIANCE) == QUEST_STATUS_COMPLETE ||
            plWho->GetQuestStatus(QUEST_TERON_GOREFIEND_I_AM_HORDE) == QUEST_STATUS_COMPLETE &&
                plWho->GetDistance(me) < 65.0f) {
          switch (me->GetEntry()) {
          case 61033:
            if (!teron) {
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new teron_triggerAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new teron_gorefiend_eventAI(pCreature); }
};
...
public:
  ...
  struct npc_artorAI : public ScriptedAI {
    ...
    void JustSummoned(Creature *summoned) { summoned->MonsterSay(ARTOR_SPAWN, LANG_UNIVERSAL, PlayerGUID); }

    void MoveInLineOfSight(Unit *pWho) {
      if (Player *plWho = pWho->GetCharmerOrOwnerPlayerOrPlayerItself()) {
        if (plWho->GetQuestStatus(QUEST_ARTOR_BOW) == QUEST_STATUS_INCOMPLETE ||
            plWho->GetQuestStatus(QUEST_CIPHER_OF_DAMNATION_ARTORS_CHARGE) == QUEST_STATUS_INCOMPLETE &&
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_artorAI(pCreature); }
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_tiny_triggerAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_painmistress_gabrisaAI(pCreature); }
};
...
public:
  ...
  struct npc_illidari_shocktrooperAI : public ScriptedAI {
    ...
    void JustSummoned(Creature *summoned) { summoned->MonsterSay(VENERATUS_SPAWN, LANG_UNIVERSAL, 0); }

    void MoveInLineOfSight(Unit *pWho) {
      if (Player *plWho = pWho->GetCharmerOrOwnerPlayerOrPlayerItself()) {
        if (plWho->GetQuestStatus(QUEST_CIPHER_OF_DAMNATION_ARTORS_CHARGE) == QUEST_STATUS_INCOMPLETE &&
            plWho->GetDistance(me) < 40.0f) {
          switch (me->GetEntry()) {
          case 19802:
    ...
    void JustDied(Unit * /*killer*/) {
      if (canSummon) {
        ...
        case 0:
          break;
        case 1:
          me->SummonCreature(20427, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), 0,
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_illidari_shocktrooperAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_veneratusAI(pCreature); }
};
...
public:
  ...
  struct npc_icarus_triggerAI : public ScriptedAI {
    ...
    void MoveInLineOfSight(Unit *pWho) {
      if (Player *plWho = pWho->GetCharmerOrOwnerPlayerOrPlayerItself()) {
        if (plWho->GetQuestStatus(QUEST_TO_CATCH_A_THISTLEHEAD) == QUEST_STATUS_INCOMPLETE &&
            plWho->GetDistance(me) < 30.0f) {
          switch (me->GetEntry()) {
          case 61035:
            PlayerGUID = plWho->GetGUID();
            break;
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_icarus_triggerAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_envoy_icariusAI(pCreature); }
};
...
public:
  ...
  struct npc_guldanAI : public ScriptedAI {
    ...
    void MoveInLineOfSight(Unit *pWho) {
      if (Player *plWho = pWho->GetCharmerOrOwnerPlayerOrPlayerItself()) {
        if (plWho->GetQuestStatus(QUEST_CIPHER_OF_DAMNATION) == QUEST_STATUS_INCOMPLETE &&
            plWho->GetDistance(me) < 10.0f) {
          switch (me->GetEntry()) {
          case 17008:
            playerGUID = plWho->GetGUID();
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_guldanAI(pCreature); }
};
...
public:
  ...
  struct npc_oronokAI : public ScriptedAI {
    ...
    uint32 NextStep(uint32 uiSteps) {
      ...
      case 6:
        ...
        me->SummonCreature(NPC_REDEEMED_SPIRIT_OF_AIR, -3588.31f, 1892.76f, 47.240f, 2.56f, TEMPSUMMON_TIMED_DESPAWN,
                           34000);
        me->SummonCreature(NPC_REDEEMED_SPIRIT_OF_EARTH, -3599.34f, 1887.27f, 47.240f, 1.82f, TEMPSUMMON_TIMED_DESPAWN,
                           34000);
        me->SummonCreature(NPC_REDEEMED_SPIRIT_OF_FIRE, -3606.63f, 1887.06f, 47.240f, 1.28f, TEMPSUMMON_TIMED_DESPAWN,
                           34000);
        me->SummonCreature(NPC_REDEEMED_SPIRIT_OF_WATER, -3592.95f, 1887.88f, 47.240f, 2.044f, TEMPSUMMON_TIMED_DESPAWN,
                           34000);
        return 4000;
      case 7:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_oronokAI(pCreature); }
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_cyrukh_the_firelordAI(pCreature); }
};
