...
SD%Complete: 100
SDComment: Instance Data Scripts and functions to acquire mobs and set encounter status for use in various Black Temple
Scripts SDCategory: Black Temple EndScriptData */
...
public:
  ...
  struct instance_black_templeAI : public ScriptedInstance {
    instance_black_templeAI(Map *pMap) : ScriptedInstance(pMap) { Initialize(); };
    ...
    void Load(const char *in) {
      ...
      loadStream >> Encounters[0] >> Encounters[1] >> Encounters[2] >> Encounters[3] >> Encounters[4] >>
          Encounters[5] >> Encounters[6] >> Encounters[7] >> Encounters[8];
  ...
  InstanceData *GetInstanceScript(InstanceMap *pMap) const override { return new instance_black_templeAI(pMap); }
};
void AddSC_instance_black_temple() { new instance_black_temple(); }
