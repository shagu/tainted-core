...
// Normal Form
#define SPELL_SHEAR                                                                                                    \
  37335 // 41032 is bugged, cannot be block/dodge/parry// Reduces Max. Health by 60% for 7 seconds. Can stack 19
        // times. 1.5 second cast
#define SPELL_FLAME_CRASH                                                                                              \
  40832                       // Summons an invis/unselect passive mob that has an aura of flame in a circle around him.
#define SPELL_DRAW_SOUL 40904 // 5k Shadow Damage in front of him. Heals Illidan for 100k health (script effect)
#define SPELL_PARASITIC_SHADOWFIEND                                                                                    \
  41917 // DoT of 3k Shadow every 2 seconds. Lasts 10 seconds. (Script effect: Summon 2 parasites once the debuff has
        // ticked off)
#define SPELL_PARASITIC_SHADOWFIEND2 41914 // Used by Parasitic
#define SPELL_SUMMON_PARASITICS                                                                                        \
  41915 // Summons 2 Parasitic Shadowfiends on the target. It's supposed to be cast as soon as the Parasitic Shadowfiend
        // debuff is gone, but the spells aren't linked :(
...
#define SPELL_SHADOW_BLAST                                                                                             \
  41078 // 8k - 11k Shadow Damage. Targets highest threat. Has a splash effect, damaging anyone in 20 yards of the
        // target.
#define SPELL_FLAME_BURST                                                                                              \
  41126 // Hurls fire at entire raid for ~3.5k damage every 10 seconds. Resistable. (Does not work: Script effect)
...
#define SPELL_CHARGE                                                                                                   \
  41581 // 40602 // Flames of Azzinoth charges whoever is too far from them. They enrage after this. For simplicity,
        // we'll use the same enrage as Illidan.
...
#define SPELL_CAGE_TRAP_SUMMON                                                                                         \
  40694 // Summons a Cage Trap GO (bugged) on the ground along with a Cage Trap Disturb Trigger mob (working)
#define SPELL_CAGE_TRAP_BEAM                                                                                           \
  40713 // 8 Triggers on the ground in an octagon cast spells like this on Illidan 'caging him'
...
static EventIllidan MaxTimer[] = {
    EVENT_NULL,   EVENT_DRAW_SOUL,     EVENT_MOVE_POINT,      EVENT_TRANSFORM_NORMAL,  EVENT_TRANSFORM_DEMON,
    EVENT_ENRAGE, EVENT_TALK_SEQUENCE, EVENT_FLIGHT_SEQUENCE, EVENT_TRANSFORM_SEQUENCE};
...
static Yells Conversation[] = {
    {11463,
     "Akama... your duplicity is hardly surprising. I should have slaughtered you and your malformed brethren long "
     "ago.",
     ILLIDAN_STORMRAGE, 8000, 0, true},
    {0, NULL, ILLIDAN_STORMRAGE, 5000, 396, true},
    {11389, "We've come to end your reign, Illidan. My people and all of Outland shall be free!", AKAMA, 7000, 25,
     true},
    ...
    {11491, "Their fury pales before mine, Illidan. We have some unsettled business between us.", MAIEV_SHADOWSONG,
     8000, 5, true},
    ...
    {11478,
     ...
     ILLIDAN_STORMRAGE, 30000, 65, true}, // Emote dead for now. Kill him later
...
static Yells RandomTaunts[] = {{11467, "I can feel your hatred.", ILLIDAN_STORMRAGE, 0, 0, false},
...
static Yells MaievTaunts[] = {{11493, "That is for Naisha!", MAIEV_SHADOWSONG, 0, 0, false},
...
static Locations HoverPosition[] = {{657, 340, 355}, {657, 275, 355}, {705, 275, 355}, {705, 340, 355}};

static Locations GlaivePosition[] = {{695.105f, 305.303f, 354.256f},
                                     {659.338f, 305.303f, 354.256f}, // the distance between two glaives is 36
                                     {700.105f, 305.303f, 354.256f},
                                     {664.338f, 305.303f, 354.256f}};

static Locations EyeBlast[] = {{677, 350, 354}, // start point, pass through glaive point
                               {677, 260, 354}};
...
static Locations AkamaWP[] = {
    ...
    {782.01f, 304.55f,
     319.76f} // Final location - back at the initial gates. This is where he will fight the minions! (12)
};
// 755.762, 304.0747, 312.1769 -- This is where Akama should be spawned
static Locations SpiritSpawns[] = {{755.5426f, 309.9156f, 312.2129f}, {755.5426f, 298.7923f, 312.0834f}};
...
static Animation DemonTransformation[] = {{SPELL_DEMON_TRANSFORM_1, 0, 1000, 0, 0, 6, true},
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new flame_of_azzinothAI(pCreature); }
};
...
public:
  ...
  struct boss_illidan_stormrageAI : public ScriptedAI {
    ...
    void SummonFlamesOfAzzinoth() {
      ...
      for (uint8 i = 0; i < 2; ++i) {
        if (GETUNIT(Glaive, GlaiveGUID[i])) {
          Creature *Flame = me->SummonCreature(FLAME_OF_AZZINOTH, GlaivePosition[i + 2].x, GlaivePosition[i + 2].y,
                                               GlaivePosition[i + 2].z, 0, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5000);
          if (Flame) {
            ...
            FlameGUID[i] =
                Flame->GetGUID(); // Record GUID in order to check if they're dead later on to move to the next phase
    ...
    void SummonMaiev() {
      ...
      if (!MaievGUID) // If Maiev cannot be summoned, reset the encounter and post some errors to the console.
      {
        ...
        sLog.outError("SD2 ERROR: Unable to summon Maiev Shadowsong (entry: 23197). Check your database to see if you "
                      "have the proper SQL for Maiev Shadowsong (entry: 23197)");
      }
    }
    ...
    void HandleFlightSequence() {
      ...
      {
        uint8 i = 1;
        Creature *Glaive = me->SummonCreature(BLADE_OF_AZZINOTH, GlaivePosition[i].x, GlaivePosition[i].y,
                                              GlaivePosition[i].z, 0, TEMPSUMMON_CORPSE_DESPAWN, 0);
        if (Glaive) {
      ...
      case 6: // fly to hover point
        me->GetMotionMaster()->MovePoint(0, HoverPosition[HoverPoint].x, HoverPosition[HoverPoint].y,
                                         HoverPosition[HoverPoint].z);
        Timer[EVENT_FLIGHT_SEQUENCE] = 0;
        break;
    ...
    void UpdateAI(const uint32 diff) {
      ...
      if (Phase == PHASE_NORMAL || Phase == PHASE_NORMAL_2 ||
          (Phase == PHASE_NORMAL_MAIEV && !me->HasAura(SPELL_CAGED, 0))) {
        switch (Event) {
          // PHASE_NORMAL
        case EVENT_BERSERK:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return GetInstanceAI<boss_illidan_stormrageAI>(pCreature); }
};
...
public:
  ...
  struct boss_maievAI : public ScriptedAI {
    ...
    void BlinkToPlayer() {
      if (GETCRE(Illidan, IllidanGUID)) {
        Unit *pTarget = CAST_AI(boss_illidan_stormrage::boss_illidan_stormrageAI, Illidan->AI())
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_maievAI(pCreature); }
};
...
public:
  ...
  struct npc_akama_illidanAI : public ScriptedAI {
    ...
    // Do not call reset in Akama's evade mode, as this will stop him from summoning minions after he kills the first
    // bit
    void EnterEvadeMode() {
      me->InterruptNonMeleeSpells(true);
      me->RemoveAllAuras();
    ...
    void BeginChannel() {
      ...
      for (uint8 i = 0; i < 2; ++i)
        if (Creature *Spirit =
                me->SummonCreature(i ? SPIRIT_OF_OLUM : SPIRIT_OF_UDALO, SpiritSpawns[i].x, SpiritSpawns[i].y,
                                   SpiritSpawns[i].z, 0, TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, 360000)) {
    ...
    void EnterPhase(PhaseAkama NextPhase) {
      ...
      case PHASE_WALK:
        if (Phase == PHASE_CHANNEL)
          ...
          if (GETCRE(Illidan, IllidanGUID))
            CAST_AI(boss_illidan_stormrage::boss_illidan_stormrageAI, Illidan->AI())
                ->DeleteFromThreatList(me->GetGUID());
          EnterEvadeMode();
          me->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE);
          ++WalkCount;
        }
    ...
    void UpdateAI(const uint32 diff) {
      ...
      if (Event) {
        ...
        case PHASE_FIGHT_MINIONS: {
          ...
          Creature *Elite = me->SummonCreature(ILLIDARI_ELITE, x + rand() % 10, y + rand() % 10, z, 0,
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_akama_illidanAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new blade_of_azzinothAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new cage_trap_triggerAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new shadow_demonAI(pCreature); }
};
...
public:
  ...
  struct mob_parasitic_shadowfiendAI : public ScriptedAI {
    mob_parasitic_shadowfiendAI(Creature *c) : ScriptedAI(c) { pInstance = (ScriptedInstance *)c->GetInstanceData(); }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new mob_parasitic_shadowfiendAI(pCreature); }
};
...
void boss_illidan_stormrage::boss_illidan_stormrageAI::HandleTalkSequence() {
  ...
  case 14:
    if (GETCRE(Maiev, MaievGUID)) {
      ...
      Maiev->AddThreat(me, 10000000.0f); // Have Maiev add a lot of threat on us so that players don't pull her off if
                                         // they damage her via AOE
