...
SD%Complete: 80
SDComment: Saber Lash missing, Fatal Attraction slightly incorrect; need to damage only if affected players are within
range of each other SDCategory: Black Temple EndScriptData */
...
static Locations TeleportPoint[] = {{959.996f, 212.576f, 193.843f}, {932.537f, 231.813f, 193.838f},
                                    {958.675f, 254.767f, 193.822f}, {946.955f, 201.316f, 192.535f},
                                    {944.294f, 149.676f, 197.551f}, {930.548f, 284.888f, 193.367f},
...
public:
  ...
  struct boss_mother_shahrazAI : public ScriptedAI {
    boss_mother_shahrazAI(Creature *c) : ScriptedAI(c) { pInstance = (ScriptedInstance *)c->GetInstanceData(); }
    ...
    void TeleportPlayers() {
      ...
      for (uint8 i = 0; i < 3; ++i) {
        ...
        if (pUnit && pUnit->IsAlive() && (pUnit->GetTypeId() == TYPEID_PLAYER) &&
            !pUnit->HasAura(SPELL_SABER_LASH_IMM, 0)) {
    ...
    void UpdateAI(const uint32 diff) {
      ...
      // Select 3 random targets (can select same target more than once), teleport to a random location then make them
      // cast explosions until they get away from each other.
      ...
      if (FatalAttractionExplodeTimer <= diff) {
        ...
        FatalAttractionExplodeTimer -= diff;
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return GetInstanceAI<boss_mother_shahrazAI>(pCreature); }
};

void AddSC_boss_mother_shahraz() { new boss_mother_shahraz(); }
