...
#define ERROR_INST_DATA                                                                                                \
  "SD2 ERROR: Instance Data for Black Temple not set properly; Illidari Council event will not function properly."
...
public:
  ...
  CreatureAI *GetAI(Creature *c) const { return new mob_blood_elf_council_voice_triggerAI(c); }
};
...
public:
  ...
  struct mob_illidari_councilAI : public ScriptedAI {
    ...
    void StartEvent(Unit *pTarget) {
      ...
      if (pTarget && pTarget->IsAlive()) {
        ...
        if (Creature *VoiceTrigger = (Unit::GetCreature(*me, pInstance->GetData64(DATA_BLOOD_ELF_COUNCIL_VOICE)))) {
          CAST_AI(mob_blood_elf_council_voice_trigger::mob_blood_elf_council_voice_triggerAI, VoiceTrigger->AI())
              ->LoadCouncilGUIDs();
          CAST_AI(mob_blood_elf_council_voice_trigger::mob_blood_elf_council_voice_triggerAI, VoiceTrigger->AI())
              ->EventStarted = true;
        }
      ...
      if (EndEventTimer) {
        if (EndEventTimer <= diff) {
          if (DeathCount > 3) {
            if (pInstance) {
              if (Creature *VoiceTrigger = (Unit::GetCreature(*me, pInstance->GetData64(DATA_BLOOD_ELF_COUNCIL_VOICE))))
                VoiceTrigger->DealDamage(VoiceTrigger, VoiceTrigger->GetHealth(), NULL, DIRECT_DAMAGE,
          ...
          if (pMember && pMember->IsAlive())
            pMember->DealDamage(pMember, pMember->GetHealth(), NULL, DIRECT_DAMAGE, SPELL_SCHOOL_MASK_NORMAL, NULL,
                                false);
          ++DeathCount;
          EndEventTimer = 1500;
        } else
              ...
              if (Creature *Member = (Unit::GetCreature((*me), Council[i]))) {
                ...
                if (Member->IsAlive() && !Member->GetVictim())
                  ++EvadeCheck; // If all members evade, we reset so that players can properly reset the event
                              ...
                              ->IsAlive()) // If even one member dies, kill the rest, set instance data, and kill self.
                {
                  EndEventTimer = 1000;
                  CheckTimer = 0;
                  return;
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return GetInstanceAI<mob_illidari_councilAI>(pCreature); }
};
...
struct boss_illidari_councilAI : public ScriptedAI {
  ...
  void EnterCombat(Unit *who) {
    ...
    // not in world, and thus have no GUID set in the instance data system. Putting it in aggro ensures that all the
    // creatures have been loaded and have their GUIDs set in the instance data system.
    if (!LoadedGUIDs)
      LoadGUIDs();
  }
...
public:
  ...
  struct boss_gathios_the_shattererAI : public boss_illidari_councilAI {
    ...
    void KilledUnit(Unit * /*victim*/) { DoScriptText(SAY_GATH_SLAY, me); }

    void JustDied(Unit * /*victim*/) { DoScriptText(SAY_GATH_DEATH, me); }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_gathios_the_shattererAI(pCreature); }
};
...
public:
  ...
  struct boss_lady_malandeAI : public boss_illidari_councilAI {
    ...
    void KilledUnit(Unit * /*victim*/) { DoScriptText(SAY_MALA_SLAY, me); }

    void JustDied(Unit * /*victim*/) { DoScriptText(SAY_MALA_DEATH, me); }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_lady_malandeAI(pCreature); }
};
...
public:
  ...
  struct boss_veras_darkshadowAI : public boss_illidari_councilAI {
    ...
    void KilledUnit(Unit * /*victim*/) { DoScriptText(SAY_VERA_SLAY, me); }

    void JustDied(Unit * /*victim*/) { DoScriptText(SAY_VERA_DEATH, me); }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_veras_darkshadowAI(pCreature); }
};
...
public:
  ...
  struct boss_high_nethermancer_zerevorAI : public boss_illidari_councilAI {
    ...
    void KilledUnit(Unit * /*victim*/) { DoScriptText(SAY_ZERE_SLAY, me); }

    void JustDied(Unit * /*victim*/) { DoScriptText(SAY_ZERE_DEATH, me); }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_high_nethermancer_zerevorAI(pCreature); }
};
