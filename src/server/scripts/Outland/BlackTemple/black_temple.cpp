...
SDName: Black_Temple
SD%Complete: 95
SDComment: Spirit of Olum: Player Teleporter to Seer Kanai Teleport after defeating Naj'entus and Supremus. @todo Find
proper gossip. SDCategory: Black Temple EndScriptData */
...
public:
  ...
  bool OnGossipHello(Player *player, Creature *creature) {
    ...
    if (pInstance && (pInstance->GetData(DATA_SUPREMUSEVENT) >= DONE) &&
        (pInstance->GetData(DATA_HIGHWARLORDNAJENTUSEVENT) >= DONE))
...
void AddSC_black_temple() { new npc_spirit_of_olum(); }
