...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new mob_doom_blossomAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new mob_shadowy_constructAI(pCreature); }
};
...
public:
  ...
  struct boss_teron_gorefiendAI : public ScriptedAI {
    boss_teron_gorefiendAI(Creature *c) : ScriptedAI(c) { pInstance = (ScriptedInstance *)c->GetInstanceData(); }
    ...
    void MindControlGhost() {
      ...
      if (Ghost && Ghost->IsAlive() && Ghost->HasAura(SPELL_SHADOW_OF_DEATH, 0)) {
        ...
        for (uint8 i = 0; i < 4; ++i) {
          ...
          Construct = me->SummonCreature(CREATURE_SHADOWY_CONSTRUCT, X, Y, Ghost->GetPositionZ(), 0,
    ...
    void UpdateAI(const uint32 diff) {
      ...
      if (SummonShadowsTimer <= diff) {
        ...
        for (uint8 i = 0; i < 2; ++i) {
          ...
          Shadow = me->SummonCreature(CREATURE_SHADOWY_CONSTRUCT, X, me->GetPositionY(), me->GetPositionZ(), 0,
      ...
      if (SummonDoomBlossomTimer <= diff) {
        if (Unit *pTarget = SelectUnit(SELECT_TARGET_RANDOM, 0)) {
          ...
          Creature *DoomBlossom =
              me->SummonCreature(CREATURE_DOOM_BLOSSOM, X, Y, Z, 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 20000);
          if (DoomBlossom) {
      ...
      /*if (ShadowOfDeathTimer <= diff)
      {
          ...
          if (pTarget && pTarget->IsAlive() && pTarget->GetTypeId() == TYPEID_PLAYER)
          {
              ...
              SummonShadowsTimer = 53000; // Make it VERY close but slightly less so that we can check if the aura is
      still on the player
          }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return GetInstanceAI<boss_teron_gorefiendAI>(pCreature); }
};
