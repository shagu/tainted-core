...
public:
  ...
  struct boss_gurtogg_bloodboilAI : public ScriptedAI {
    boss_gurtogg_bloodboilAI(Creature *c) : ScriptedAI(c) { pInstance = (ScriptedInstance *)c->GetInstanceData(); }
    ...
    // Note: This seems like a very complicated fix. The fix needs to be handled by the core, as implementation of
    // limited-target AoE spells are still not limited.
    void CastBloodboil() {
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return GetInstanceAI<boss_gurtogg_bloodboilAI>(pCreature); }
};

void AddSC_boss_gurtogg_bloodboil() { new boss_gurtogg_bloodboil(); }
