...
public:
  ...
  struct boss_najentusAI : public ScriptedAI {
    boss_najentusAI(Creature *c) : ScriptedAI(c) { pInstance = (ScriptedInstance *)c->GetInstanceData(); }
    ...
    void KilledUnit(Unit * /*victim*/) { DoScriptText(rand() % 2 ? SAY_SLAY1 : SAY_SLAY2, me); }
      ...
      if (ImpalingSpineTimer <= diff) {
        if (!me->IsNonMeleeSpellCast(false)) {
          ...
          if (pTarget) {
            ...
            pTarget->SummonGameObject(GOBJECT_SPINE, pTarget->GetPositionX(), pTarget->GetPositionY(),
                                      pTarget->GetPositionZ(), me->GetOrientation(), 0, 0, 0, 0, 0);
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return GetInstanceAI<boss_najentusAI>(pCreature); }
};
