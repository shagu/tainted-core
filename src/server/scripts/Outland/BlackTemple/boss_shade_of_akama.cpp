...
*/
static Location SpawnLocations[] = {{498.652740f, 461.728119f, 0, 0}, {498.505003f, 339.619324f, 0, 0}};

static Location AkamaWP[] = {{482.352448f, 401.162720f, 0, 112.783928f}, {469.597443f, 402.264404f, 0, 118.537910f}};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new mob_ashtongue_channelerAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new mob_ashtongue_sorcererAI(pCreature); }
};
...
public:
  ...
  struct boss_shade_of_akamaAI : public ScriptedAI {
    ...
    void JustSummoned(Creature *summon) {
      if (summon->GetEntry() == CREATURE_DEFENDER || summon->GetEntry() == 23523 || summon->GetEntry() == 23318 ||
          summon->GetEntry() == 23524)
    ...
    void MoveInLineOfSight(Unit * /*who*/) {
      if (!GridSearcherSucceeded) {
        ...
        if (Channelers.size() >= 6) {
          ...
          sLog.outError("SD2 ERROR: No Channelers are stored in the list. This encounter will not work properly");
      }
    }
    ...
    void IncrementDeathCount(uint64 guid = 0) // If guid is set, will remove it from list of sorcerer
    {
      ...
      sLog.outDebug("OSCR: Increasing Death Count for Shade of Akama encounter");
      ++DeathCount;
      me->RemoveSingleAuraFromStack(SPELL_SHADE_SOUL_CHANNEL_2, 0);
      if (guid) {
        if (Sorcerers.empty())
          sLog.outError(
              "SD2 ERROR: Shade of Akama - attempt to remove guid %llu from Sorcerers list but list is already empty",
              guid);
        else
    ...
    void SummonCreature() {
      ...
      for (uint8 pos = 0; pos < 2; ++pos) {
        ...
        for (uint8 i = 0; i < 3; ++i) {
          Creature *Spawn =
              me->SummonCreature(spawnEntries[i], X, Y, Z_SPAWN, 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 5000);
          if (Spawn) {
            Spawn->SetWalk(false);
    ...
    void FindChannelers() {
      ...
      if (!ChannelerList.empty()) {
        for (std::list<Creature *>::iterator itr = ChannelerList.begin(); itr != ChannelerList.end(); ++itr) {
          ...
          sLog.outDebug("OSCR: Shade of Akama Grid Search found channeler %llu. Adding to list", (*itr)->GetGUID());
        }
      } else
        sLog.outError(
            "SD2 ERROR: Grid Search was unable to find any channelers. Shade of Akama encounter will be buggy");
    }
    ...
    void UpdateAI(const uint32 diff) {
      ...
      if (IsBanished) {
        ...
        if (DefenderTimer <= diff) {
          uint32 ran = rand() % 2;
          Creature *Defender = me->SummonCreature(CREATURE_DEFENDER, SpawnLocations[ran].x, SpawnLocations[ran].y,
                                                  Z_SPAWN, 0, TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, 25000);
          if (Defender) {
      ...
      } else // No longer banished, let's fight Akama now
      {
        if (ReduceHealthTimer <= diff) {
          if (AkamaGUID) {
            ...
            if (Akama && Akama->IsAlive()) {
              ...
              me->DealDamage(Akama, Akama->GetMaxHealth() / 10, NULL, DIRECT_DAMAGE, SPELL_SCHOOL_MASK_NORMAL, NULL,
                             false);
        ...
        if (HasKilledAkama) {
          if (!HasKilledAkamaAndReseting) // do not let players kill Shade if Akama is dead and Shade is waiting for
                                          // ResetTimer!! event would bug
          {
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return GetInstanceAI<boss_shade_of_akamaAI>(pCreature); }
};
...
public:
      ...
      if (SummonBrokenTimer && BrokenSummonIndex < 4) {
        if (SummonBrokenTimer <= diff) {
          for (uint8 i = 0; i < 4; ++i) {
            ...
            Creature *Broken =
                me->SummonCreature(CREATURE_BROKEN, x, y, z, o, TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, 360000);
            if (Broken) {
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_akama_shadeAI(pCreature); }
};
