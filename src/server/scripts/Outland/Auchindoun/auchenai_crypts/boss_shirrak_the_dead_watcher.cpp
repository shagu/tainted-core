...
public:
  ...
  struct boss_shirrak_the_dead_watcherAI : public ScriptedAI {
    boss_shirrak_the_dead_watcherAI(Creature *c) : ScriptedAI(c) { HeroicMode = me->GetMap()->IsHeroic(); }
    ...
    void JustSummoned(Creature *summon) { summon->CastSpell(summon, SPELL_FOCUS_FIRE_VISUAL, true); }
    ...
    void UpdateAI(const uint32 diff) {
      ...
      case EVENT_SPELL_FOCUS_FIRE:
        if (Unit *target = SelectTarget(SELECT_TARGET_RANDOM, 0, 60.0f, true)) {
          if (Creature *cr = me->SummonCreature(ENTRY_FOCUS_FIRE, target->GetPositionX(), target->GetPositionY(),
                                                target->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN, 7000))
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_shirrak_the_dead_watcherAI(pCreature); }
};
...
public:
  ...
  struct mob_focus_fireAI : public ScriptedAI {
    mob_focus_fireAI(Creature *c) : ScriptedAI(c) { HeroicMode = me->GetMap()->IsHeroic(); }

    bool HeroicMode;
    EventMap events;

    void EnterCombat(Unit *) { events.ScheduleEvent(EVENT_SPELL_FIRE_BLAST, 3000 + (rand() % 1000)); }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new mob_focus_fireAI(pCreature); }
};
