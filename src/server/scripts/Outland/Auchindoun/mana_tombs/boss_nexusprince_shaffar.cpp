...
SD%Complete: 80
SDComment: Need more tuning of spell timers, it should not be as linear fight as current. Also should possibly find a
better way to deal with his three initial beacons to make sure all aggro. SDCategory: Auchindoun, Mana Tombs
EndScriptData */
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_nexusprince_shaffarAI(pCreature); }
};
...
public:
  ...
  struct mob_ethereal_beaconAI : public ScriptedAI {
    mob_ethereal_beaconAI(Creature *c) : ScriptedAI(c) { HeroicMode = me->GetMap()->IsHeroic(); }
    ...
    void JustSummoned(Creature *summoned) { summoned->AI()->AttackStart(me->GetVictim()); }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new mob_ethereal_beaconAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new mob_ethereal_apprenticeAI(pCreature); }
};
