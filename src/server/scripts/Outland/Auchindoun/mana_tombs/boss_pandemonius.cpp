...
SD%Complete: 75
SDComment: Not known how void blast is done (amount of rapid cast seems to be related to players in party). All mobs
remaining in surrounding area should aggro when engaged. SDCategory: Auchindoun, Mana Tombs EndScriptData */
...
public:
  ...
  struct boss_pandemoniusAI : public ScriptedAI {
    boss_pandemoniusAI(Creature *c) : ScriptedAI(c) { HeroicMode = me->GetMap()->IsHeroic(); }

    bool HeroicMode;
    uint32 VoidBlast_Counter;
    EventMap events;

    void Reset() { VoidBlast_Counter = 0; }

    void JustDied(Unit *) { DoScriptText(SAY_DEATH, me); }

    void KilledUnit(Unit *) { DoScriptText(RAND(SAY_KILL_1, SAY_KILL_2), me); }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_pandemoniusAI(pCreature); }
};

void AddSC_boss_pandemonius() { new boss_pandemonius(); }
