...
public:
  ...
  struct boss_murmurAI : public Scripted_NoMovementAI {
    boss_murmurAI(Creature *c) : Scripted_NoMovementAI(c) { HeroicMode = me->GetMap()->IsHeroic(); }
    ...
    void SpellHitTarget(Unit *pTarget, const SpellEntry *spell) {
      if (pTarget && pTarget->IsAlive() && spell && spell->Id == SPELL_SONIC_BOOM_EFFECT)
        me->DealDamage(pTarget, (pTarget->GetHealth() * 90) / 100, NULL, SPELL_DIRECT_DAMAGE, SPELL_SCHOOL_MASK_NATURE,
                       spell);
    }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_murmurAI(pCreature); }
};

void AddSC_boss_murmur() { new boss_murmur(); }
