...
float VoidPortalCoords[5][3] = {{-283.5894f, -239.5718f, 12.7f},
...
public:
  EmpoweringShadowsAura(SpellEntry *spell, uint32 eff, int32 *bp, Unit *pTarget, Unit *caster)
...
public:
  ...
  struct mob_voidtravelerAI : public ScriptedAI {
    mob_voidtravelerAI(Creature *c) : ScriptedAI(c) { HeroicMode = me->GetMap()->IsHeroic(); }
    ...
    void UpdateAI(const uint32 diff) {
      ...
      if (move <= diff) {
        if (sacrificed) {
          SpellEntry *spell = (SpellEntry *)GetSpellStore()->LookupEntry(HeroicMode ? H_SPELL_EMPOWERING_SHADOWS
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new mob_voidtravelerAI(pCreature); }
};
...
public:
  ...
  struct boss_grandmaster_vorpilAI : public ScriptedAI {
    ...
    void summonPortals() {
      if (!sumportals) {
        for (int i = 0; i < 5; i++) {
          ...
          Portal = me->SummonCreature(MOB_VOID_PORTAL, VoidPortalCoords[i][0], VoidPortalCoords[i][1],
                                      VoidPortalCoords[i][2], 0, TEMPSUMMON_CORPSE_DESPAWN, 3000000);
          if (Portal) {
    ...
    void spawnVoidTraveler() {
      int pos = rand() % 5;
      me->SummonCreature(MOB_VOID_TRAVELER, VoidPortalCoords[pos][0], VoidPortalCoords[pos][1],
                         VoidPortalCoords[pos][2], 0, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5000);
      if (!HelpYell) {
        ...
        for (Map::PlayerList::const_iterator i = PlayerList.begin(); i != PlayerList.end(); ++i)
          if (Player *i_pl = i->GetSource())
            if (i_pl->IsAlive() && !i_pl->HasAura(SPELL_BANISH, 0))
              i_pl->TeleportTo(me->GetMapId(), VorpilPosition[0], VorpilPosition[1], VorpilPosition[2], 0,
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return GetInstanceAI<boss_grandmaster_vorpilAI>(pCreature); }
};
