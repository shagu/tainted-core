...
public:
  ...
  struct boss_anzuAI : public ScriptedAI {
    ...
    void SummonBroods() {
      ...
      for (int i = 0; i < 5; ++i) {
        Creature *brood = me->SummonCreature(NPC_BROOD, me->GetPositionX() + 20 * cos((float)i),
                                             me->GetPositionY() + 20 * sin((float)i), me->GetPositionZ() + 25.0f, 0.0f,
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return GetInstanceAI<boss_anzuAI>(pCreature); }
};

void AddSC_boss_anzu() { new boss_anzu(); }
