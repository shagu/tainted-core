...
static Location SpawnLocation[] = {{-158.226898f, 158.690933f, 0.009380f, 1.212f}};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_lakkaAI(pCreature); }
};
...
public:
  ...
  struct instance_sethekk_hallsAI : public ScriptedInstance {
    instance_sethekk_hallsAI(Map *map) : ScriptedInstance(map) { Initialize(); };
    ...
    void OnPlayerEnter(Player *pPlayer) {
      if ((CAST_PLR(pPlayer)->GetQuestStatus(QUEST_BROTHER) == QUEST_STATUS_INCOMPLETE) && !SummonLakka) {
        pPlayer->SummonCreature(NPC_LAKKA, SpawnLocation[0].fLocX, SpawnLocation[0].fLocY, SpawnLocation[0].fLocZ,
                                SpawnLocation[0].fOrient, TEMPSUMMON_DEAD_DESPAWN, 0);
        SummonLakka = true;
      }
    }
  ...
  InstanceData *GetInstanceScript(InstanceMap *pMap) const override { return new instance_sethekk_hallsAI(pMap); }
};
/*#####
## go_lakka_cage
#####*/
