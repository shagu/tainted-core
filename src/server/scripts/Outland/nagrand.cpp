...
SDName: Nagrand
SD%Complete: 99
SDComment: Quest support: 9849, 9918, 9874, 9991, 10107, 10108, 10044, 10172, 10646, 10085, 10987, 9868, 9948, 9923,
9924, 9955, 9879. TextId's unknown for altruis_the_sufferer and greatmother_geyah (npc_text) SDCategory: Nagrand
EndScriptData */
...
#define GOSSIP_HGG2                                                                                                    \
  "Garrosh is beyond redemption, Greatmother. I fear that in helping the Mag'har, I have convinced Garrosh that he "   \
  "is unfit to lead."

#define GOSSIP_SGG1 "You raised all of the orcs here, Greatmother?"
#define GOSSIP_SGG2 "Do you believe that?"
#define GOSSIP_SGG3                                                                                                    \
  "What can be done? I have tried many different things. I have done my best to help the people of Nagrand. Each "     \
  "time I have approached Garrosh, he has dismissed me."
...
#define GOSSIP_SGG9                                                                                                    \
  "Greatmother, I never had the honor. Durotan died long before my time, but his heroics are known to all on my "      \
  "world. The orcs of Azeroth reside in a place known as Durotar, named after your son. And ... (You take a moment "   \
  "to breathe and think through what you are about to tell the Greatmother.)"
#define GOSSIP_SGG10                                                                                                   \
  "It is my Warchief, Greatmother. The leader of my people. From my world. He ... He is the son of Durotan. He is "    \
  "your grandchild."
...
#define GOSSIP_SLB5                                                                                                    \
  "My people ask that you pull back your Boulderfist ogres and cease all attacks on our territories. In return, we "   \
  "will also pull back our forces."
#define GOSSIP_SLB6                                                                                                    \
  "We will fight you until the end, then, Lantresor. We will not stand idly by as you pillage our towns and kill our " \
  "people."
...
static Move Z[] = {{-885.76f, 7717.75f, 35.24f}, {-882.96f, 7723.00f, 34.78f}, {-871.40f, 7724.87f, 33.36f},
                   {-873.16F, 7727.59f, 33.35f}, {-855.66f, 7732.36f, 33.42f}, {-855.44f, 7735.44f, 33.44f},
                   {-843.39f, 7726.59f, 34.50f}, {-840.20f, 7728.34f, 34.39f}, {-848.31f, 7714.37f, 34.42f},
                   {-845.44f, 7710.70f, 35.05f}, {-859.99f, 7713.96f, 35.94f}, {-859.70f, 7710.61f, 36.68f},
                   {-873.74f, 7720.35f, 33.98f}, {-875.16f, 7717.15f, 34.39f}};
...
public:
  ...
  struct mob_shattered_rumblerAI : public ScriptedAI {
    ...
    void SpellHit(Unit *Hitter, const SpellEntry *Spellkind) {
      if (Spellkind->Id == 32001 && !Spawn) {
        ...
        Hitter->SummonCreature(18181, x + (0.7f * (rand() % 30)), y + (rand() % 5), z, 0,
                               TEMPSUMMON_CORPSE_TIMED_DESPAWN, 60000);
        Hitter->SummonCreature(18181, x + (rand() % 5), y - (rand() % 5), z, 0, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 60000);
        Hitter->SummonCreature(18181, x - (rand() % 5), y + (0.5f * (rand() % 60)), z, 0,
                               TEMPSUMMON_CORPSE_TIMED_DESPAWN, 60000);
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new mob_shattered_rumblerAI(pCreature); }
};
...
public:
  ...
  struct mob_lumpAI : public ScriptedAI {
    mob_lumpAI(Creature *c) : ScriptedAI(c) { bReset = false; }
    ...
    void UpdateAI(const uint32 diff) {
      ...
      if (Spear_Throw_Timer <= diff) {
        ...
        Spear_Throw_Timer -= diff;

      DoMeleeAttackIfReady();
    }
  };

  CreatureAI *GetAI(Creature *pCreature) const { return new mob_lumpAI(pCreature); }
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new mob_sunspring_villagerAI(pCreature); }
};
...
public:
  ...
  struct npc_maghar_captiveAI : public npc_escortAI {
    ...
    void EnterCombat(Unit * /*pWho*/) { DoCast(me, SPELL_EARTHBIND_TOTEM, false); }
    ...
    void WaypointReached(uint32 uiPointId) {
      ...
      case 7:
        ...
        if (Creature *pTemp = me->SummonCreature(NPC_MURK_PUTRIFIER, m_afAmbushB[0], m_afAmbushB[1], m_afAmbushB[2],
                                                 0.0f, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 25000))
          DoScriptText(SAY_MAG_MORE_REPLY, pTemp);
        me->SummonCreature(NPC_MURK_PUTRIFIER, m_afAmbushB[0] - 2.5f, m_afAmbushB[1] - 2.5f, m_afAmbushB[2], 0.0f,
                           TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 25000);
        me->SummonCreature(NPC_MURK_SCAVENGER, m_afAmbushB[0] + 2.5f, m_afAmbushB[1] + 2.5f, m_afAmbushB[2], 0.0f,
                           TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 25000);
        me->SummonCreature(NPC_MURK_SCAVENGER, m_afAmbushB[0] + 2.5f, m_afAmbushB[1] - 2.5f, m_afAmbushB[2], 0.0f,
                           TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 25000);
        break;
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_maghar_captiveAI(pCreature); }
  ...
  bool OnQuestAccept(Player *pPlayer, Creature *pCreature, const Quest *pQuest) override {
    if (pQuest->GetQuestId() == QUEST_TOTEM_KARDASH_H) {
      if (npc_maghar_captiveAI *pEscortAI = dynamic_cast<npc_maghar_captiveAI *>(pCreature->AI())) {
        ...
        pCreature->SummonCreature(NPC_MURK_RAIDER, m_afAmbushA[0] + 2.5f, m_afAmbushA[1] - 2.5f, m_afAmbushA[2], 0.0f,
                                  TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 25000);
        pCreature->SummonCreature(NPC_MURK_PUTRIFIER, m_afAmbushA[0] - 2.5f, m_afAmbushA[1] + 2.5f, m_afAmbushA[2],
                                  0.0f, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 25000);
        pCreature->SummonCreature(NPC_MURK_BRUTE, m_afAmbushA[0], m_afAmbushA[1], m_afAmbushA[2], 0.0f,
                                  TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 25000);
      }
    }
    return true;
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_creditmarker_visit_with_ancestorsAI(pCreature); }
};
...
public:
  ...
  struct npc_maghar_prisonerAI : public npc_escortAI {
    ...
    void Reset() { uiPlayerGUID = 0; }

    void MoveInLineOfSight(Unit *pWho) {
      if (pWho->GetTypeId() == TYPEID_PLAYER && ((Player *)pWho)->GetReputationRank(941) >= REP_FRIENDLY &&
          me->IsWithinDistInMap(((Player *)pWho), 20)) {
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_maghar_prisonerAI(pCreature); }
};
...
public:
  ...
  struct mob_sparrowhawkAI : public ScriptedAI {
    ...
    void MoveInLineOfSight(Unit *who) {
      ...
      if (!PlayerGUID && who->GetTypeId() == TYPEID_PLAYER && me->IsWithinDistInMap(((Player *)who), 30) &&
          ((Player *)who)->GetQuestStatus(10987) == QUEST_STATUS_INCOMPLETE) {
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new mob_sparrowhawkAI(pCreature); }
};
...
public:
  ...
  bool OnGossipHello(Player *pPlayer, GameObject *pGo) override {
    ...
    if (Creature *pPrisoner = pGo->FindNearestCreature(NPC_MPRISONER, 5, true)) {
      ...
      DoScriptText(RAND(SAYT_MAG_PRISONER1, SAYT_MAG_PRISONER2, SAYT_MAG_PRISONER3, SAYT_MAG_PRISONER4), pPrisoner,
                   pPlayer);

      if (npc_maghar_prisoner::npc_maghar_prisonerAI *pEscortAI =
              CAST_AI(npc_maghar_prisoner::npc_maghar_prisonerAI, pPrisoner->AI()))
...
public:
  ...
  struct npc_corki1AI : public npc_escortAI {
    ...
    void MoveInLineOfSight(Unit *pWho) {
      if (pWho->GetTypeId() == TYPEID_PLAYER && ((Player *)pWho)->GetReputationRank(978) >= REP_FRIENDLY &&
          me->IsWithinDistInMap(((Player *)pWho), 20)) {
        if (uiPlayerGUID == pWho->GetGUID())
          return;
        else
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_corki1AI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_corki2AI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_corki3AI(pCreature); }
};
...
public:
  ...
  struct npc_kurenai_captiveAI : public npc_escortAI {
    ...
    void EnterCombat(Unit * /*pWho*/) { DoCast(me, MAGIC_EARTHBIND_TOTEM, false); }

    void WaypointReached(uint32 uiPointId) {
      switch (uiPointId) {
      case 7:
        DoScriptText(SAY_KUR_MORE, me);
        if (Creature *pTemp = me->SummonCreature(MURK_PUTRIFIER, m_afAmbushD[0], m_afAmbushD[1], m_afAmbushD[2], 0.0f,
                                                 TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 25000))
          DoScriptText(SAY_KUR_MORE_REPLY, pTemp);
        me->SummonCreature(MURK_PUTRIFIER, m_afAmbushD[0] - 2.5f, m_afAmbushD[1] - 2.5f, m_afAmbushD[2], 0.0f,
                           TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 25000);
        me->SummonCreature(MURK_BRUTE, m_afAmbushD[0] + 2.5f, m_afAmbushD[1] + 2.5f, m_afAmbushD[2], 0.0f,
                           TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 25000);
        me->SummonCreature(MURK_RAIDER, m_afAmbushD[0] + 2.5f, m_afAmbushD[1] - 2.5f, m_afAmbushD[2], 0.0f,
                           TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 25000);
        break;
      case 8:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_kurenai_captiveAI(pCreature); }
  ...
  bool OnQuestAccept(Player *pPlayer, Creature *pCreature, const Quest *pQuest) override {
    if (pQuest->GetQuestId() == QUEST_TOTEM_KARDASH_A) {
      if (npc_kurenai_captiveAI *pEscortAI = dynamic_cast<npc_kurenai_captiveAI *>(pCreature->AI())) {
        ...
        pCreature->SummonCreature(MURK_RAIDER, m_afAmbushC[0] + 2.5f, m_afAmbushC[1] - 2.5f, m_afAmbushC[2], 0.0f,
                                  TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 25000);
        pCreature->SummonCreature(MURK_PUTRIFIER, m_afAmbushC[0] - 2.5f, m_afAmbushC[1] + 2.5f, m_afAmbushC[2], 0.0f,
                                  TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 25000);
        pCreature->SummonCreature(MURK_BRUTE, m_afAmbushC[0], m_afAmbushC[1], m_afAmbushC[2], 0.0f,
                                  TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 25000);
      }
    }
    return true;
...
public:
  ...
  struct npc_warmaul_pyreAI : public ScriptedAI {
    ...
    void DoSummon() {
      ...
      if (Creature *Saboteur = GetSaboteur(2)) {
        ...
        me->SummonCreature(NPC_CORPSE, Saboteur->GetPositionX(), Saboteur->GetPositionY(), Saboteur->GetPositionZ(),
                           0.0f, TEMPSUMMON_TIMED_DESPAWN, Time);
      }
    ...
    uint32 NextStep(uint32 Steps) {
      ...
      case 9:
        if (Creature *Saboteur = GetSaboteur(1))
          DoScriptText(RAND(SAY_SABOTEUR5, SAY_SABOTEUR6, SAY_SABOTEUR7, SAY_SABOTEUR8, SAY_SABOTEUR9, SAY_SABOTEUR10,
                            SAY_SABOTEUR11),
                       Saboteur);
        if (Creature *Saboteur = GetSaboteur(2))
          DoScriptText(RAND(SAY_SABOTEUR5, SAY_SABOTEUR6, SAY_SABOTEUR7, SAY_SABOTEUR8, SAY_SABOTEUR9, SAY_SABOTEUR10,
  ...
  CreatureAI *GetAI(Creature *creature) const { return new npc_warmaul_pyreAI(creature); }
};
...
public:
    ...
    void SpellHit(Unit *caster, const SpellEntry *spell) {
      if (spell->Id == SPELL_HATE && me->GetEntry() == NPC_CANNON_HATE) {
        if (Creature *Target = GetClosestCreatureWithEntry(me, NPC_HATE_TARGET, 85.5f)) {
          ...
          me->SummonGameObject(OBJECT_LARGE_FIRE, Target->GetPositionX(), Target->GetPositionY(),
                               Target->GetPositionZ(), Target->GetOrientation(), 0, 0, 0, 0, 30);
        }
      }
  ...
  CreatureAI *GetAI(Creature *creature) const { return new npc_fel_cannon_hafAI(creature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *creature) const { return new npc_living_cycloneAI(creature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *creature) const { return new npc_enraged_crusherAI(creature); }
};
  ...
  struct npc_storm_ragerAI : public ScriptedAI {
    ...
    void UpdateAI(const uint32 diff) {
      ...
      if (calllightning_timer <= diff) {
        ...
        calllightning_timer -= diff;

      DoMeleeAttackIfReady();
    }
  };

  CreatureAI *GetAI(Creature *creature) const { return new npc_storm_ragerAI(creature); }
};
...
public:
  ...
  struct npc_lake_surgerAI : public ScriptedAI {
    ...
    void Reset() { me->ApplySpellImmune(0, IMMUNITY_SCHOOL, SPELL_SCHOOL_MASK_FROST, true); }
  ...
  CreatureAI *GetAI(Creature *creature) const { return new npc_lake_surgerAI(creature); }
};
  ...
  struct npc_lake_spiritAI : public ScriptedAI {
    ...
    void UpdateAI(const uint32 diff) {
      ...
      if (waterbolt_timer <= diff) {
        ...
        waterbolt_timer -= diff;

      DoMeleeAttackIfReady();
    }
  };

  CreatureAI *GetAI(Creature *creature) const { return new npc_lake_spiritAI(creature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *creature) const { return new npc_tortured_earth_spiritAI(creature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *creature) const { return new npc_crashing_wave_spiritAI(creature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *creature) const { return new npc_raging_fire_soulAI(creature); }
};
  ...
  struct npc_storming_wind_ripperAI : public ScriptedAI {
    ...
    void UpdateAI(const uint32 diff) {
      ...
      if (lightning_Shield_timer <= diff) {
        ...
        lightning_Shield_timer -= diff;

      if (light_bolt_timer <= diff) {
        DoCastVictim(SPELL_LIGHTBOLT);
        light_bolt_timer = 6500;
      } else
        light_bolt_timer -= diff;

      DoMeleeAttackIfReady();
    }
  };

  CreatureAI *GetAI(Creature *creature) const { return new npc_storming_wind_ripperAI(creature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *creature) const { return new npc_rumbling_earth_heartAI(creature); }
};
