...
public:
  ...
  struct boss_gatewatcher_iron_handAI : public ScriptedAI {
    boss_gatewatcher_iron_handAI(Creature *c) : ScriptedAI(c) { pInstance = (ScriptedInstance *)c->GetInstanceData(); }
    ...
    void JustDied(Unit * /*Killer*/) { DoScriptText(SAY_DEATH_1, me); }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return GetInstanceAI<boss_gatewatcher_iron_handAI>(pCreature); }
};

void AddSC_boss_gatewatcher_iron_hand() { new boss_gatewatcher_iron_hand(); }
