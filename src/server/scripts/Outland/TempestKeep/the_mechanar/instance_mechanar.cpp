...
public:
  ...
  struct instance_mechanarAI : public ScriptedInstance {
    instance_mechanarAI(Map *map) : ScriptedInstance(map) { Initialize(); };
          ...
          if (Player *player = GetPassagePlayer(250.0f)) {
            ...
            for (uint8 i = 0; i < 4; ++i) {
              ...
              if (Creature *creature = instance->SummonCreature(
                      i == 1 || i == 2 ? NPC_SUNSEEKER_ASTROMAGE : NPC_BLOODWARDER_CENTURION, pos))
            ...
            if (_passageEncounter == ENCOUNTER_PASSAGE_PHASE1) {
              ...
              for (uint8 i = 0; i < 3; ++i) {
                ...
                if (Creature *creature =
                        instance->SummonCreature(i == 1 ? NPC_SUNSEEKER_ENGINEER : NPC_BLOODWARDER_PHYSICIAN, pos))
                  DoSummonAction(creature, player);
              }
            }
            _passageEncounter++;
          ...
          if (Player *player = GetPassagePlayer(148.0f)) {
            if (_passageEncounter == ENCOUNTER_PASSAGE_PHASE3) {
              for (uint8 i = 0; i < 3; ++i) {
                ...
                if (Creature *creature =
                        instance->SummonCreature(i == 1 ? NPC_SUNSEEKER_ASTROMAGE : NPC_BLOODWARDER_PHYSICIAN, pos))
                  DoSummonAction(creature, player);
              }
              ...
              for (uint8 i = 0; i < 4; ++i) {
                ...
                if (Creature *creature = instance->SummonCreature(
                        i == 1 || i == 2 ? NPC_SUNSEEKER_ASTROMAGE : NPC_SUNSEEKER_ENGINEER, pos))
                  DoSummonAction(creature, player);
              }
  ...
  InstanceData *GetInstanceScript(InstanceMap *pMap) const override { return new instance_mechanarAI(pMap); }
};
