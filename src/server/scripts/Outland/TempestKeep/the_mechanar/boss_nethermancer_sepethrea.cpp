...
enum raginFlames { EVENT_SPELL_FIRE_TAIL = 1, EVENT_SPELL_INFERNO = 2 };
...
public:
  ...
  struct boss_nethermancer_sepethreaAI : public BossAI {
    ...
    void EnterCombat(Unit *who) {
      ...
      if (isHeroic) {
        ...
        me->SummonCreature(20481, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), me->GetOrientation(),
                           TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 300);
      } else
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return GetInstanceAI<boss_nethermancer_sepethreaAI>(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return GetInstanceAI<mob_ragin_flamesAI>(pCreature); }
};
