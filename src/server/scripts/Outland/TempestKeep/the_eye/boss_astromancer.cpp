...
public:
  ...
  struct boss_high_astromancer_solarianAI : public ScriptedAI {
    ...
    void UpdateAI(const uint32 diff) {
      ...
      if (Phase == 1) {
        ...
        if (MarkOfTheAstromancer_Timer <= diff) // A debuff that lasts for 5 seconds, cast several times each phase on a
                                                // random raid member, but not the main tank
        {
          Unit *pTarget = SelectTarget(SELECT_TARGET_RANDOM, 1, 100, true);
          if (pTarget)
            DoCast(pTarget, SPELL_MARK_OF_THE_ASTROMANCER);
          else
            DoCastVictim(SPELL_MARK_OF_THE_ASTROMANCER);
          MarkOfTheAstromancer_Timer = 15000;
        } else
        ...
        if (Phase1_Timer <= diff) {
          Phase = 2;
          Phase1_Timer = 50000;
          // After these 50 seconds she portals to the middle of the room and disappears, leaving 3 light portals
          // behind.
          ...
          for (int i = 0; i <= 2; i++) {
            Creature *Summoned = me->SummonCreature(ASTROMANCER_SOLARIAN_SPOTLIGHT, Portals[i][0], Portals[i][1],
                                                    Portals[i][2], CENTER_O, TEMPSUMMON_TIMED_DESPAWN,
                                                    Phase2_Timer + Phase3_Timer + AppearDelay_Timer + 1700);
            if (Summoned) {
        ...
        if (Phase3_Timer <= diff) {
          ...
          // 15 seconds later Solarian reappears out of one of the 3 portals. Simultaneously, 2 healers appear in the
          // two other portals.
          int i = rand() % 3;
          me->GetMotionMaster()->Clear();
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return GetInstanceAI<boss_high_astromancer_solarianAI>(pCreature); }
};
...
public:
  ...
  struct mob_solarium_priestAI : public ScriptedAI {
    mob_solarium_priestAI(Creature *c) : ScriptedAI(c) { pInstance = (ScriptedInstance *)c->GetInstanceData(); }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return GetInstanceAI<mob_solarium_priestAI>(pCreature); }
};
