...
#define SPELL_FLAME_BUFFET                                                                                             \
  34121 // Flame Buffet - every 1,5 secs in phase 1 if there is no victim in melee range and after Dive Bomb in phase 2
        // with same conditions
#define SPELL_FLAME_QUILLS                                                                                             \
  34229 // Randomly after changing position in phase after watching tonns of movies, set probability 20%
...
#define SPELL_DIVE_BOMB_VISUAL                                                                                         \
  35367 // Bosskillers says 30 sec cooldown, wowwiki says 30 sec colldown, DBM and BigWigs addons says ~47 sec
...
#define CREATURE_EMBER_OF_ALAR                                                                                         \
  19551 // Al'ar summons one Ember of Al'ar every position change in phase 1 and two after Dive Bomb. Also in phase 2
        // when Ember of Al'ar dies, boss loose 3% health.
...
static float waypoint[6][3] = {{340.15f, 58.65f, 17.71f},  {388.09f, 31.54f, 20.18f}, {388.18f, -32.85f, 20.18f},
                               {340.29f, -60.19f, 17.72f}, {332, 0.01f, 39}, // better not use the same xy coord
                               {331, 0.01f, -2.39f}};
...
public:
    ...
    void UpdateAI(const uint32 diff) {
      ...
      if (Phase1) {
        ...
        if (FlamePatch_Timer <= diff) {
          if (Unit *pTarget = SelectUnit(SELECT_TARGET_RANDOM, 0)) {
            Creature *Summoned =
                me->SummonCreature(CREATURE_FLAME_PATCH_ALAR, pTarget->GetPositionX(), pTarget->GetPositionY(),
                                   pTarget->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN, 120000);
            if (Summoned) {
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return GetInstanceAI<boss_alarAI>(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return GetInstanceAI<mob_ember_of_alarAI>(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new mob_flame_patch_alarAI(pCreature); }
};
