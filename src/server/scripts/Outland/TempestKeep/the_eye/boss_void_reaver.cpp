...
public:
  ...
  struct boss_void_reaverAI : public ScriptedAI {
    boss_void_reaverAI(Creature *c) : ScriptedAI(c) { pInstance = (ScriptedInstance *)c->GetInstanceData(); }
    ...
    void UpdateAI(const uint32 diff) {
      ...
      if (ArcaneOrb_Timer <= diff) {
        ...
        if (pTarget)
          me->CastSpell(pTarget->GetPositionX(), pTarget->GetPositionY(), pTarget->GetPositionZ(), SPELL_ARCANE_ORB,
                        false);
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return GetInstanceAI<boss_void_reaverAI>(pCreature); }
};

void AddSC_boss_void_reaver() { new boss_void_reaver(); }
