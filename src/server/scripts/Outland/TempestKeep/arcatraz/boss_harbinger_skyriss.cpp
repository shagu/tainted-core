...
SD%Complete: 45
SDComment: CombatAI not fully implemented. Timers will need adjustments. Need more docs on how event fully work. Reset
all event and force start over if fail at one point? SDCategory: Tempest Keep, The Arcatraz EndScriptData */
...
enum IllusionSpells { SPELL_MIND_REND_IMAGE = 36929, H_SPELL_MIND_REND_IMAGE = 39021 };
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return GetInstanceAI<boss_harbinger_skyrissAI>(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *c) const { return GetInstanceAI<boss_harbinger_skyriss_illusionAI>(c); }
};
