...
float treant_pos[6][3] = {{24.301233f, 427.221100f, -27.060635f},  {16.795492f, 359.678802f, -27.355425f},
                          {53.493484f, 345.381470f, -26.196192f},  {61.867096f, 439.362732f, -25.921030f},
...
public:
  ...
  struct mob_warp_splinter_treantAI : public ScriptedAI {
    mob_warp_splinter_treantAI(Creature *c) : ScriptedAI(c) { WarpGuid = 0; }

    uint64 WarpGuid;
    uint32 check_Timer;

    void Reset() { check_Timer = 0; }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new mob_warp_splinter_treantAI(pCreature); }
};
...
public:
  ...
  struct boss_warp_splinterAI : public ScriptedAI {
    ...
    void SummonTreants() {
      for (int i = 0; i < 6; ++i) {
        ...
        if (Creature *pTreant = me->SummonCreature(CREATURE_TREANT, treant_pos[i][0], treant_pos[i][1],
                                                   treant_pos[i][2], O, TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, 25000))
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_warp_splinterAI(pCreature); }
};
