...
SDName: Boss_Laj
SD%Complete: 90
SDComment: Immunities are wrong, must be adjusted to use resistance from creature_templates. Most spells require
database support. SDCategory: Tempest Keep, The Botanica EndScriptData */
...
public:
  ...
  struct boss_lajAI : public ScriptedAI {
    ...
    void DoTransform() {
      ...
      switch (_lastTransform = RAND(SPELL_DAMAGE_IMMUNE_SHADOW, SPELL_DAMAGE_IMMUNE_FIRE, SPELL_DAMAGE_IMMUNE_FROST,
                                    SPELL_DAMAGE_IMMUNE_NATURE, SPELL_DAMAGE_IMMUNE_ARCANE)) {
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_lajAI(pCreature); }
};

void AddSC_boss_laj() { new boss_laj(); }
