...
public:
      ...
      if (CanPullBack) {
        if (ShadowWhip_Timer <= diff) {
          if (Unit *temp = Unit::GetUnit(*me, playerGUID)) {
            // if unit dosen't have this flag, then no pulling back (script will attempt cast, even if orbital strike
            // was resisted)
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return GetInstanceAI<boss_omor_the_unscarredAI>(pCreature); }
};

void AddSC_boss_omor_the_unscarred() { new boss_omor_the_unscarred(); }
