...
const float VazrudenRing[2][3] = {{-1430, 1705, 112}, {-1377, 1760, 112}};
...
public:
  ...
  struct boss_nazanAI : public ScriptedAI {
    ...
    void SpellHitTarget(Unit *pTarget, const SpellEntry *entry) {
      if (pTarget && entry->Id == SPELL_FIREBALL)
        me->SummonCreature(ENTRY_LIQUID_FIRE, pTarget->GetPositionX(), pTarget->GetPositionY(), pTarget->GetPositionZ(),
                           pTarget->GetOrientation(), TEMPSUMMON_TIMED_DESPAWN, 30000);
    }
    ...
    void UpdateAI(const uint32 diff) {
      ...
      if (flight) // phase 1 - the flight
      {
        ...
        if (Fly_Timer <= diff ||
            !(Vazruden && Vazruden->IsAlive() && (Vazruden->GetHealth() * 5 > Vazruden->GetMaxHealth()))) {
          flight = false;
        ...
        if (Turn_Timer <= diff) {
          ...
          if (me->GetDistance(VazrudenRing[waypoint][0], VazrudenRing[waypoint][1], VazrudenRing[waypoint][2]) > 5)
            me->GetMotionMaster()->MovePoint(0, VazrudenRing[waypoint][0], VazrudenRing[waypoint][1],
                                             VazrudenRing[waypoint][2]);
          Turn_Timer = 10000;
        } else
          Turn_Timer -= diff;
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return GetInstanceAI<boss_nazanAI>(pCreature); }
};
...
public:
  ...
  struct boss_vazruden_the_heraldAI : public ScriptedAI {
    ...
    void SummonAdds() {
      if (!summoned) {
        Creature *Vazruden = me->SummonCreature(ENTRY_VAZRUDEN, VazrudenMiddle[0], VazrudenMiddle[1], VazrudenMiddle[2],
                                                0, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 6000000);
        if (Vazruden)
          VazrudenGUID = Vazruden->GetGUID();
        Creature *Nazan = me->SummonCreature(ENTRY_NAZAN, VazrudenMiddle[0], VazrudenMiddle[1], VazrudenMiddle[2], 0,
                                             TEMPSUMMON_CORPSE_TIMED_DESPAWN, 6000000);
        if (Nazan)
      ...
      default: // adds do the job now
        if (check <= diff) {
          ...
          if ((Nazan && Nazan->IsAlive()) || (Vazruden && Vazruden->IsAlive())) {
            ...
            me->SummonGameObject(ENTRY_REINFORCED_FEL_IRON_CHEST, VazrudenMiddle[0], VazrudenMiddle[1],
                                 VazrudenMiddle[2], 0, 0, 0, 0, 0, 0);
            me->SetLootRecipient(NULL);
            me->DealDamage(me, me->GetMaxHealth(), NULL, DIRECT_DAMAGE, SPELL_SCHOOL_MASK_NORMAL, NULL, false);
            if (pInstance)
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return GetInstanceAI<boss_vazruden_the_heraldAI>(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return GetInstanceAI<boss_vazrudenAI>(pCreature); }
};
...
public:
  ...
  struct mob_hellfire_sentryAI : public ScriptedAI {
    ...
    void Reset() { KidneyShot_Timer = 3000 + rand() % 4000; }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return GetInstanceAI<mob_hellfire_sentryAI>(pCreature); }
};
