...
public:
  ...
  struct instance_shattered_hallsAI : public ScriptedInstance {
    instance_shattered_hallsAI(Map *map) : ScriptedInstance(map) { Initialize(); };
    ...
    void OnPlayerEnter(Player *pPlayer) {
      ...
      if (uiTeam == ALLIANCE)
        pPlayer->SummonCreature(aSoldiersLocs[1].uiAllianceEntry, aSoldiersLocs[1].m_fX, aSoldiersLocs[1].m_fY,
                                aSoldiersLocs[1].m_fZ, aSoldiersLocs[1].m_fO, TEMPSUMMON_DEAD_DESPAWN, 0);
      else
        pPlayer->SummonCreature(aSoldiersLocs[0].uiHordeEntry, aSoldiersLocs[0].m_fX, aSoldiersLocs[0].m_fY,
                                aSoldiersLocs[0].m_fZ, aSoldiersLocs[0].m_fO, TEMPSUMMON_DEAD_DESPAWN, 0);
    }
      ...
      case TYPE_EXECUTION:
        if (data == IN_PROGRESS && !instance->GetCreature(executionerGUID)) {
          if (Player *pPlayer = GetPlayerInMap()) {
            for (uint8 i = 2; i < 5; ++i)
              /*Creature* pVictim = */ pPlayer->SummonCreature(
                  uiTeam == ALLIANCE ? aSoldiersLocs[i].uiAllianceEntry : aSoldiersLocs[i].uiHordeEntry,
                  aSoldiersLocs[i].m_fX, aSoldiersLocs[i].m_fY, aSoldiersLocs[i].m_fZ, aSoldiersLocs[i].m_fO,
                  TEMPSUMMON_DEAD_DESPAWN, 0);

            if (Creature *pExecutioner = pPlayer->SummonCreature(
                    NPC_EXECUTIONER, afExecutionerLoc[0], afExecutionerLoc[1], afExecutionerLoc[2], afExecutionerLoc[3],
                    TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 80 * MINUTE * IN_MILLISECONDS))
        ...
        if (uiExecutionTimer <= diff) {
          ...
          case 0:
            if (Creature *pSoldier = instance->GetCreature(uiTeam == ALLIANCE ? officeraGUID : officerhGUID))
              pSoldier->DealDamage(pSoldier, pSoldier->GetHealth(), NULL, DIRECT_DAMAGE, SPELL_SCHOOL_MASK_NORMAL, NULL,
                                   false);

            // DoScriptText(uiTeam == ALLIANCE ? SAY_KARGATH_EXECUTE_ALLY : SAY_KARGATH_EXECUTE_HORDE,
            // instance->GetCreature(kargathGUID));
  ...
  InstanceData *GetInstanceScript(InstanceMap *pMap) const override { return new instance_shattered_hallsAI(pMap); }
};
...
public:
  ...
  bool OnTrigger(Player *pPlayer, AreaTriggerEntry const * /*pAt*/) override {
    ...
    instance_shattered_halls::instance_shattered_hallsAI *pInstance =
        (instance_shattered_halls::instance_shattered_hallsAI *)pPlayer->GetInstanceData();
