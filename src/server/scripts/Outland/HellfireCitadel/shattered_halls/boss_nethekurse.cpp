...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return GetInstanceAI<boss_grand_warlock_nethekurseAI>(pCreature); }
};
...
public:
  ...
  struct mob_fel_orc_convertAI : public ScriptedAI {
    mob_fel_orc_convertAI(Creature *c) : ScriptedAI(c) { pInstance = (ScriptedInstance *)c->GetInstanceData(); }
    ...
    void EnterCombat(Unit * /*who*/) {
      if (pInstance) {
        if (pInstance->GetData64(DATA_NETHEKURSE)) {
          ...
          if (pKurse)
            ((boss_grand_warlock_nethekurse::boss_grand_warlock_nethekurseAI *)pKurse->AI())
                ->DoYellForPeonEnterCombat();
        }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return GetInstanceAI<mob_fel_orc_convertAI>(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new mob_lesser_shadow_fissureAI(pCreature); }
};
