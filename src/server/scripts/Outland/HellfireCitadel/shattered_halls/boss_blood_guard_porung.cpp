...
public:
    ...
    void UpdateAI(const uint32 diff) {
      if (!UpdateVictim()) {
        ...
        if (form4 == true && reinforcement_timer <= diff) {
          me->SummonCreature(NPC_ZEALOT, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), 0,
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return GetInstanceAI<boss_blood_guard_porungAI>(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_shattered_zealotAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_blood_guardAI(pCreature); }
};
