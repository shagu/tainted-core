...
const float ShadowmoonChannelers[5][4] = {{302.0f, -87.0f, -24.4f, 0.157f},
...
public:
  BurningNovaAura(SpellEntry *spell, uint32 eff, Unit *pTarget, Unit *caster)
      : Aura(spell, eff, NULL, pTarget, caster, NULL) {}
};
...
public:
  ...
  struct boss_kelidan_the_breakerAI : public ScriptedAI {
    ...
    void SummonChannelers() {
      for (uint8 i = 0; i < 5; ++i) {
        ...
        if (!channeler)
          channeler = me->SummonCreature(NPC_CHANNELER, ShadowmoonChannelers[i][0], ShadowmoonChannelers[i][1],
                                         ShadowmoonChannelers[i][2], ShadowmoonChannelers[i][3],
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_kelidan_the_breakerAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new mob_shadowmoon_channelerAI(pCreature); }
};
...
public:
  ...
  struct npc_kelridan_triggerAI : public ScriptedAI {
    ...
    npc_kelridan_triggerAI(Creature *c) : ScriptedAI(c) { pInstance = (ScriptedInstance *)c->GetInstanceData(); }

    void Reset() { buffTimer = 5000; }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_kelridan_triggerAI(pCreature); }
};
