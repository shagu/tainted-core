...
public:
  ...
  CreatureAI *GetAI(Creature *creature) const { return new mob_abyssalAI(creature); }
};
...
public:
  ...
  struct boss_magtheridonAI : public ScriptedAI {
    ...
    // function to interrupt channeling and debuff clicker with mind exh(used if second person clicks with same cube or
    // after dispeling/ending shadow grasp DoT)
    ...
    void KilledUnit(Unit * /*victim*/) { DoScriptText(SAY_PLAYER_KILLED, me); }
    ...
    void UpdateAI(const uint32 diff) {
      ...
      if (!Phase3 && me->GetHealth() * 10 < me->GetMaxHealth() * 3 && !me->IsNonMeleeSpellCast(false) // blast nova
  ...
  CreatureAI *GetAI(Creature *creature) const { return GetInstanceAI<boss_magtheridonAI>(creature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *creature) const { return GetInstanceAI<mob_hellfire_channelerAI>(creature); }
};
