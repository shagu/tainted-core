...
#define ISHANAH_SAY_2                                                                                                  \
  "I offer myself in exchange. You will have Ishanah the High Priestess of the Aldor, as your prisoner if you "        \
  "release Kaylaan from your dark grasp."
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_raliq_the_drunkAI(pCreature); }
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_kyleneAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_salsalabimAI(pCreature); }
...
public:
  ...
  bool OnGossipHello(Player *player, Creature *pCreature) override {
    if (pCreature->GetEntry() == 23484) {
      // Aldor vendor
      if (pCreature->IsVendor() && (player->GetReputationRank(932) == REP_EXALTED) &&
    ...
    if (pCreature->GetEntry() == 23483) {
      // Scryers vendor
      if (pCreature->IsVendor() && (player->GetReputationRank(934) == REP_EXALTED) &&
          (player->GetReputationRank(935) == REP_EXALTED) && (player->GetReputationRank(942) == REP_EXALTED)) {
        player->ADD_GOSSIP_ITEM(1, GOSSIP_TEXT_BROWSE_GOODS, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_TRADE);
        player->SEND_GOSSIP_MENU(11085, pCreature->GetGUID());
      } else
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_khadgars_servantAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_dirty_larryAI(pCreature); }
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_ishanahAI(pCreature); }
