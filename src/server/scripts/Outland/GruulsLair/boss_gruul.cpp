...
#define SPELL_GROUND_SLAM                                                                                              \
  33525 // AoE Ground Slam applying Ground Slam to everyone with a script effect (most likely the knock back, we can
        // code it to a set knockback)
...
public:
  ...
  struct boss_gruulAI : public ScriptedAI {
    boss_gruulAI(Creature *c) : ScriptedAI(c) { pInstance = (ScriptedInstance *)c->GetInstanceData(); }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return GetInstanceAI<boss_gruulAI>(pCreature); }
};

void AddSC_boss_gruul() { new boss_gruul(); }
