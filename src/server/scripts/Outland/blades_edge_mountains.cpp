...
SD%Complete: 90
SDComment: Quest support: 10556, 10609, 10682, 10980, 10512. Ogri'la->Skettis Flight. (npc_daranelle needs bit more work
before consider complete) SDCategory: Blade's Edge Mountains EndScriptData */
...
public:
        ...
        if (NihilSpeech_Phase) {
          if (NihilSpeech_Timer <= diff) {
            ...
            case 5:
              ...
              me->GetMotionMaster()->MovePoint(0, me->GetPositionX() + 100, me->GetPositionY(),
                                               me->GetPositionZ() + 100);
              ++NihilSpeech_Phase;
              break;
            case 6:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new mobs_nether_drakeAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_daranelleAI(pCreature); }
};
...
enum eBloodmaul { NPC_OGRE_BRUTE = 19995, NPC_QUEST_CREDIT = 21241, GO_KEG = 184315 };
...
public:
  ...
  struct npc_bloodmaul_brutebaneAI : public ScriptedAI {
    ...
    void Reset() { OgreGUID = 0; }

    void UpdateAI(const uint32 /*uiDiff*/) {}
  };

  CreatureAI *GetAI(Creature *pCreature) const { return new npc_bloodmaul_brutebaneAI(pCreature); }
};
...
public:
  ...
  struct npc_ogre_bruteAI : public ScriptedAI {
    ...
    void MoveInLineOfSight(Unit *pWho) {
      ...
      if (me->IsWithinDistInMap(pWho, 50.0f) && (pWho->GetTypeId() == TYPEID_PLAYER) &&
          pWho->ToPlayer()->GetQuestStatus(10512) == QUEST_STATUS_INCOMPLETE)
        PlayerGUID = pWho->GetGUID();

      if (me->IsWithinDistInMap(pWho, 50.0f) && (pWho->GetTypeId() == TYPEID_PLAYER) &&
          pWho->ToPlayer()->GetQuestStatus(10545) == QUEST_STATUS_INCOMPLETE)
        PlayerGUID = pWho->GetGUID();
    }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_ogre_bruteAI(pCreature); }
};
...
public:
  ...
  bool OnGossipHello(Player *player, GameObject *go) {
    if (player->GetQuestStatus(Q_THE_THUNDERSPIKE) == QUEST_STATUS_INCOMPLETE) {
      ...
      if (Creature *pGor = player->SummonCreature(GOR_GRIMGUT_ENTRY, dest.m_positionX, dest.m_positionY,
                                                  dest.m_positionZ, 0.0f, TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 60000))
        pGor->AI()->AttackStart(player);
    }
...
public:
  ...
  struct npc_light_orb_collectorAI : public ScriptedAI {
    npc_light_orb_collectorAI(Creature *c) : ScriptedAI(c) { Reset(); }
        ...
        if (Player *player = itr->GetSource())
          if (player->GetGUID() == playerGUID)
            if (me->IsWithinDistInMap(player, 15.0f))
              if (player->GetQuestStatus(QUEST_LIGHT_FANTASTIC) ||
                  player->GetQuestStatus(QUEST_GATHER_THE_ORBS) == QUEST_STATUS_INCOMPLETE)
  ...
  CreatureAI *GetAI(Creature *creature) const { return new npc_light_orb_collectorAI(creature); }
};
...
public:
  ...
  struct npc_anger_campAI : public ScriptedAI {
    ...
    void Activate() {
      ...
      if (Count == 5) {
        me->SummonCreature(NPC_DOOMCRYER, 2878.467f, 4817.107f, 282.800f, 0.329514f,
  ...
  CreatureAI *GetAI(Creature *creature) const { return new npc_anger_campAI(creature); }
};
...
public:
  ...
  bool OnGossipHello(Player *player, GameObject *go) {
    if (player->GetQuestStatus(QUEST_FIRED) == QUEST_STATUS_INCOMPLETE) {
      ...
      if (!trigger) {
        if (Creature *bunny = go->SummonCreature(NPC_INVISB, go->GetPositionX(), go->GetPositionY(), go->GetPositionZ(),
                                                 go->GetOrientation(), TEMPSUMMON_TIMED_DESPAWN, 181000)) {
...
public:
  ...
  struct npc_bloodmaul_direwolfAI : public ScriptedAI {
    ...
    void MoveInLineOfSight(Unit *pWho) {
      ...
      if (me->IsWithinDistInMap(pWho, 50.0f) && (pWho->GetTypeId() == TYPEID_PLAYER) &&
          pWho->ToPlayer()->GetQuestStatus(10506) == QUEST_STATUS_INCOMPLETE)
        PlayerGUID = pWho->GetGUID();
    }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_bloodmaul_direwolfAI(pCreature); }
};
...
public:
  ...
  struct npc_prophecy_triggerAI : public ScriptedAI {
    npc_prophecy_triggerAI(Creature *c) : ScriptedAI(c) { me->SetReactState(REACT_AGGRESSIVE); }

    void MoveInLineOfSight(Unit *pWho) {
      if (Player *plWho = pWho->GetCharmerOrOwnerPlayerOrPlayerItself()) {
        if (plWho->GetQuestStatus(10607) == QUEST_STATUS_INCOMPLETE && plWho->HasAura(37642, 0) &&
            plWho->GetDistance(me) < 20.0f) {
          switch (me->GetEntry()) {
          case 22798:
            me->Whisper(
                "From the darkest night shall rise again the raven, shall take flight in the shadows, shall reveal the "
                "nature of its kind. Prepare yourself for its coming, for the faithful shall be elevated to take "
                "flight with the raven, the rest be forgotten to walk upon the ground, clipped wings and shame.",
                plWho->GetGUID());
            break;
          case 22799:
            me->Whisper("Steel your minds and guard your thoughts. The dark wings will cloud and consume the minds of "
                        "the weak, a flock of thralls whose feet may never leave the ground.",
                        plWho->GetGUID());
            break;
          case 22800:
            me->Whisper("The old blood will flow once again with the coming of the raven, the return of the darkness "
                        "in the skies. Scarlet night, and the rise of the old.",
                        plWho->GetGUID());
            break;
          case 22801:
            me->Whisper("The raven was struck down once for flying too high, unready. The eons have prepared the Dark "
                        "Watcher for its ascent, to draw the dark cloak across the horizon.",
                        plWho->GetGUID());
            break;
          }
  ...
  CreatureAI *GetAI(Creature *_Creature) const { return new npc_prophecy_triggerAI(_Creature); }
};
...
public:
  ...
  bool OnGossipHello(Player *player, GameObject *go) {
    if (player->GetQuestStatus(10607) == QUEST_STATUS_INCOMPLETE && player->HasAura(37642, 0)) {
      ...
      } else if (Creature *prophecy = player->FindNearestCreature(22799, 5.0f, true)) {
        CAST_AI(npc_prophecy_trigger::npc_prophecy_triggerAI, prophecy->AI())->MoveInLineOfSight(player);
      } else if (Creature *prophecy = player->FindNearestCreature(22800, 5.0f, true)) {
        CAST_AI(npc_prophecy_trigger::npc_prophecy_triggerAI, prophecy->AI())->MoveInLineOfSight(player);
      } else if (Creature *prophecy = player->FindNearestCreature(22801, 5.0f, true)) {
        CAST_AI(npc_prophecy_trigger::npc_prophecy_triggerAI, prophecy->AI())->MoveInLineOfSight(player);
      }
    }
    return false;
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_grishna_arakkoaAI(pCreature); }
};
...
public:
  ...
  struct npc_koikoi_spiritAI : public ScriptedAI {
    ...
    void Reset() { ghostlystrike_timer = 3000; }
    ...
    void UpdateAI(const uint32 diff) {
      ...
      if (ghostlystrike_timer <= diff) {
        ...
        ghostlystrike_timer -= diff;

      DoMeleeAttackIfReady();
    }
  };

  CreatureAI *GetAI(Creature *pCreature) const { return new npc_koikoi_spiritAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_raven_leafbeardAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_wyrmkult_zealotAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_wyrmkult_acolyteAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_evergrove_ancientAI(pCreature); }
};
...
public:
  ...
  struct npc_baelmonAI : public ScriptedAI {
    ...
    void EnterCombat(Unit * /*who*/) { DoScriptText(SAY_BAELMON_AGGRO, me); }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_baelmonAI(pCreature); }
};
...
public:
  ...
  struct npc_groc_triggerAI : public ScriptedAI {
    npc_groc_triggerAI(Creature *c) : ScriptedAI(c) { me->SetReactState(REACT_AGGRESSIVE); }

    void Reset() { summoned = false; }

    bool summoned;

    void MoveInLineOfSight(Unit *pWho) {
      if (Player *plWho = pWho->GetCharmerOrOwnerPlayerOrPlayerItself()) {
        if (plWho->GetQuestStatus(10806) == QUEST_STATUS_INCOMPLETE && plWho->HasItemCount(31808, 1, false) &&
            plWho->GetDistance(me) < 5.0f) {
          switch (me->GetEntry()) {
          case 61002:
            me->SummonCreature(20555, 3713.08f, 5396.28f, -3.85f, 4.94f, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT,
                               120000),
                plWho->GetGUID();
            me->SummonCreature(22473, 3718.914f, 5357.06f, -8.11f, 1.82f, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT,
                               35000),
                plWho->GetGUID();
            summoned = true;
            me->DisappearAndDie();
        ...
        if (plWho->GetQuestStatus(10742) == QUEST_STATUS_INCOMPLETE && plWho->HasItemCount(31146, 1, false) &&
            plWho->GetDistance(me) < 5.0f) {
          switch (me->GetEntry()) {
          case 61002:
            me->SummonCreature(20555, 3713.08f, 5396.28f, -3.85f, 4.94f, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT,
                               120000),
                plWho->GetGUID();
            me->SummonCreature(22448, 3718.914f, 5357.06f, -8.11f, 1.82f, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT,
                               35000),
                plWho->GetGUID();
            summoned = true;
            me->DisappearAndDie();
  ...
  CreatureAI *GetAI(Creature *_Creature) const { return new npc_groc_triggerAI(_Creature); }
};
...
public:
  ...
  struct npc_gocAI : public ScriptedAI {
    ...
    void EnterCombat(Unit * /*who*/) { me->Say(SAY_GROC_CRUSH, LANG_UNIVERSAL, 0); }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_gocAI(pCreature); }
};
...
public:
  ...
  struct npc_sabelian_humanAI : public ScriptedAI {
    ...
    void UpdateAI(const uint32 diff) {
      ...
      if (summoned == false && transformed == true && summon_timer <= diff) {
        me->SummonCreature(22496, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), 0,
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_sabelian_humanAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_sabelian_dragonAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_rexxarAI(pCreature); }
};
...
public:
  ...
  struct npc_grimgutAI : public ScriptedAI {
    ...
    void EnterCombat(Unit * /*who*/) { DoScriptText(SAY_PICKUP_SPEAR, me); }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_grimgutAI(pCreature); }
};
...
public:
  ...
  struct npc_onspiritswings_triggerAI : public ScriptedAI {
    npc_onspiritswings_triggerAI(Creature *c) : ScriptedAI(c) { me->SetReactState(REACT_AGGRESSIVE); }

    void Reset() {}

    void MoveInLineOfSight(Unit *pWho) {
      if (Player *plWho = pWho->GetCharmerOrOwnerPlayerOrPlayerItself()) {
        if (plWho->GetQuestStatus(QUEST_ON_SPIRITS_WINGS) == QUEST_STATUS_INCOMPLETE &&
            plWho->HasItemCount(31128, 1, false) && plWho->GetDistance(me) < 22.0f) {
          switch (me->GetEntry()) {
          case 61004:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_onspiritswings_triggerAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_hufferAI(pCreature); }
};
...
public:
  ...
  bool OnGossipHello(Player *player, GameObject *go) {
    if (player->GetQuestStatus(QUEST_PICKING_UP_SOME_POWER_CONVERTERS) == QUEST_STATUS_INCOMPLETE) {
      go->SummonCreature(NPC_ENCASED_ELECTROMENTAL, go->GetPositionX(), go->GetPositionY(), go->GetPositionZ(), 0,
...
public:
  ...
  bool OnGossipHello(Player *player, GameObject *go) {
    if (player->GetQuestStatus(QUEST_WHAT_CAME_FIRST_THE_DRAKE_OR_THE_EGG) == QUEST_STATUS_INCOMPLETE) {
      go->SummonCreature(NPC_NETHER_WHELP, go->GetPositionX(), go->GetPositionY(), go->GetPositionZ(), 0,
...
public:
  ...
  struct npc_nexusprince_razaanAI : public ScriptedAI {
    ...
    void Reset() { energy_timer = 5000; }

    uint32 energy_timer;

    void EnterCombat(Unit * /*who*/) { me->Yell(YELL_AGGRO, LANG_UNIVERSAL, 0); }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_nexusprince_razaanAI(pCreature); }
};
...
public:
  ...
  struct npc_summon_circle_triggerAI : public ScriptedAI {
    ...
    void Activate() {
      ...
      if (Count == 5) {
        me->SummonCreature(22911, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), 0,
                           TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 60000);
        me->DisappearAndDie();
      }
    }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_summon_circle_triggerAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_vimgol_circle_triggerAI(pCreature); }
};
...
public:
  ...
  struct npc_vimgol_the_vileAI : public ScriptedAI {
    ...
    void EnterCombat(Unit * /*who*/) { me->Yell(YELL_VIMGOL_AGGRO, LANG_UNIVERSAL, 0); }
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_vimgol_the_vileAI(pCreature); }
};
...
public:
  ...
  struct the_soulgrinderAI : public ScriptedAI {
    ...
    void MoveInLineOfSight(Unit *pWho) {
      if (Player *plWho = pWho->GetCharmerOrOwnerPlayerOrPlayerItself()) {
        if (plWho->GetQuestStatus(QUEST_INTO_THE_SOULGRINDER) == QUEST_STATUS_INCOMPLETE &&
            plWho->GetDistance(me) < 5.0f) {
          switch (me->GetEntry()) {
          case 23019:
            if (!summoned) {
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new the_soulgrinderAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new helper_bunny_1AI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new helper_bunny_2AI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new helper_bunny_3AI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new skulloc_soulgrinderAI(pCreature); }
};
  ...
  bool OnGossipSelect(Player *player, GameObject *go, uint32 sender, uint32 action) {
    ...
    case GOSSIP_ACTION_INFO_DEF + 1:
      if (player->HasItemCount(ITEM_APEXIS_SHARD, 35)) {
        ...
        player->SummonCreature(NPC_APEXIS_GUARDIAN, go->GetPositionX(), go->GetPositionY(), go->GetPositionZ(), 0,
...
public:
  ...
  bool OnGossipSelect(Player *player, GameObject *go, uint32 sender, uint32 action) {
    ...
    case GOSSIP_ACTION_INFO_DEF + 1:
      if (player->HasItemCount(ITEM_APEXIS_SHARD, 35)) {
        player->DestroyItemCount(ITEM_APEXIS_SHARD, 35, true);
        player->SummonCreature(NPC_GALVANOTH, go->GetPositionX() - 5, go->GetPositionY() - 5, go->GetPositionZ(), 0,
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_aether_rayAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_felburn_triggerAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_legion_flak_cannonAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new npc_abyssal_flamebringerAI(pCreature); }
};
