...
SDName: GO_Scripts
SD%Complete: 100
SDComment: Quest support: 4285,4287,4288(crystal pylons), 4296, 6481,
 * 10990, 10991, 10992. Field_Repair_Bot->Teaches spell 22704. Barov_journal->Teaches spell 26089, 2936.
 * Soulwell
SDCategory: Game Objects
EndScriptData */

/* ContentData
go_cat_figurine (the "trap" version of GO, two different
 * exist)
...
enum eResoniteCask { NPC_GOGGEROC = 11920 };
...
enum eHives { QUEST_HIVE_IN_THE_TOWER = 9544, NPC_HIVE_AMBUSHER = 13301 };
...
public:
  ...
  bool OnGossipHello(Player *pPlayer, GameObject * /*pGO*/) override {
    if (pPlayer->HasSkill(SKILL_TAILORING) && pPlayer->GetBaseSkillValue(SKILL_TAILORING) >= 280 &&
...
public:
  ...
  bool OnGossipHello(Player *pPlayer, GameObject *pGO) override {
    ...
    if (BirdEntry)
      pPlayer->SummonCreature(BirdEntry, fX, fY, fZ, pGO->GetOrientation(), TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT,
                              60000);
...
public:
  ...
  bool OnGossipHello(Player *player, GameObject *go) override {
    ...
    if (Creature *creature =
            player->SummonCreature(NpcPrisonEntry[Random], go->GetPositionX(), go->GetPositionY(), go->GetPositionZ(),
                                   go->GetAngle(player), TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 30000)) {
      if (!creature->IsHostileTo(player)) {
        if (FactionTemplateEntry const *pFaction = creature->GetFactionTemplateEntry()) {
          ...
          if (Spell)
            ...
            sLog.outError(
                "go_ethereum_prison summoned Creature (entry %u) but faction (%u) are not expected by script.",
...
public:
  ...
  bool OnGossipHello(Player *player, GameObject *go) override {
    ...
    player->SummonCreature(NpcStasisEntry[Random], go->GetPositionX(), go->GetPositionY(), go->GetPositionZ(),
                           go->GetAngle(player), TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 30000);
...
public:
  ...
  bool OnGossipHello(Player *pPlayer, GameObject *pGO) override {
    if (pGO->GetGoType() == GAMEOBJECT_TYPE_GOOBER)
      pPlayer->SummonCreature(NPC_ARIKARA, -5008.338f, -2118.894f, 83.657f, 0.874f,
...
public:
  ...
  bool OnGossipHello(Player *pPlayer, GameObject * /*pGO*/) override {
    if (pPlayer->GetQuestStatus(5381) == QUEST_STATUS_INCOMPLETE)
      pPlayer->SummonCreature(11876, pPlayer->GetPositionX(), pPlayer->GetPositionY(), pPlayer->GetPositionZ(), 0,
...
public:
  ...
  bool OnGossipHello(Player *pPlayer, GameObject *pGO) override {
    ...
    pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_FEL_CRYSTALFORGE_ITEM_1, GOSSIP_SENDER_MAIN,
                             GOSSIP_ACTION_INFO_DEF);
    pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_FEL_CRYSTALFORGE_ITEM_5, GOSSIP_SENDER_MAIN,
  ...
  bool OnGossipSelect(Player *pPlayer, GameObject *pGO, uint32 /*uiSender*/, uint32 uiAction) override {
    ...
    case GOSSIP_ACTION_INFO_DEF:
      ...
      pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_FEL_CRYSTALFORGE_ITEM_RETURN, GOSSIP_SENDER_MAIN,
...
public:
  ...
  bool OnGossipHello(Player *pPlayer, GameObject *pGO) override {
    ...
    pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_BASHIR_CRYSTALFORGE_ITEM_1, GOSSIP_SENDER_MAIN,
                             GOSSIP_ACTION_INFO_DEF);
    pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_BASHIR_CRYSTALFORGE_ITEM_5, GOSSIP_SENDER_MAIN,
                             GOSSIP_ACTION_INFO_DEF + 1);
  ...
  bool OnGossipSelect(Player *pPlayer, GameObject *pGO, uint32 /*uiSender*/, uint32 uiAction) override {
    ...
    case GOSSIP_ACTION_INFO_DEF:
      ...
      pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_BASHIR_CRYSTALFORGE_ITEM_RETURN, GOSSIP_SENDER_MAIN,
                               GOSSIP_ACTION_INFO_DEF + 2);
...
public:
  ...
  bool OnGossipHello(Player *pPlayer, GameObject *pGO) override {
    if (pGO->GetGoType() == GAMEOBJECT_TYPE_GOOBER)
      pPlayer->SummonCreature(NPC_ZELEMAR, -369.746f, 166.759f, -21.50f, 5.235f, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT,
                              30000);
...
public:
  ...
  bool OnGossipHello(Player *pPlayer, GameObject *pGO) override {
    ...
    pGO->SummonCreature(NPC_HIVE_AMBUSHER, pGO->GetPositionX() + 1, pGO->GetPositionY(), pGO->GetPositionZ(),
                        pGO->GetAngle(pPlayer), TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 60000);
    pGO->SummonCreature(NPC_HIVE_AMBUSHER, pGO->GetPositionX(), pGO->GetPositionY() + 1, pGO->GetPositionZ(),
                        pGO->GetAngle(pPlayer), TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 60000);
    return true;
  }
};
