...
enum eCoilfangGOs { GO_COILFANG_WATERFALL = 184212 };
...
enum eRavenholdt { QUEST_MANOR_RAVENHOLDT = 6681, NPC_RAVENHOLDT = 13936 };
...
enum eLarkorwi { QUEST_SCENT_OF_LARKORWI = 4291, NPC_LARKORWI_MATE = 9683 };
...
public:
  ...
  bool OnTrigger(Player *player, AreaTriggerEntry const *trigger) {
    if (player->IsAlive() && !player->IsGameMaster() &&
        player->GetQuestStatus(QUEST_SCENT_OF_LARKORWI) == QUEST_STATUS_INCOMPLETE) {
      if (!GetClosestCreatureWithEntry(player, NPC_LARKORWI_MATE, 25.0f, false))
        player->SummonCreature(NPC_LARKORWI_MATE, trigger->x, trigger->y, trigger->z, 3.3f,
                               TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 2 * MINUTE * IN_MILLISECONDS);
    }
  ...
  bool OnTrigger(Player *player, AreaTriggerEntry const *trigger) {
    ...
    if (player->GetQuestStatus(QUEST_ANCIENT_LEAF) == QUEST_STATUS_COMPLETE) {
      ...
      for (uint8 i = 0; i < MAX_ANCIENTS; ++i)
        player->SummonCreature(afSpawnLocations[i].uiEntry, afSpawnLocations[i].fX, afSpawnLocations[i].fY,
                               afSpawnLocations[i].fZ, afSpawnLocations[i].fO, TEMPSUMMON_TIMED_DESPAWN,
...
enum { QUEST_SPECIAL_DELIVERY_TO_SHATTRATH = 10280 };
