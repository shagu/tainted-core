...
uint32 m_auiSpellSummonShade[] = {SPELL_SUMMONSHADE_1, SPELL_SUMMONSHADE_2, SPELL_SUMMONSHADE_3};
...
public:
  ...
  struct boss_taerarAI : public ScriptedAI {
    ...
    void EnterCombat(Unit * /*pWho*/) { DoScriptText(SAY_AGGRO, me); }
    ...
    void UpdateAI(const uint32 uiDiff) {
      ...
      // Summon 3 Shades at 75%, 50% and 25% (if bShades is true we already left in line 117, no need to check here
      // again)
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_taerarAI(pCreature); }
};
...
public:
  ...
  CreatureAI *GetAI(Creature *pCreature) const { return new boss_shadeoftaerarAI(pCreature); }
};
