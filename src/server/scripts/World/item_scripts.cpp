...
SDName: Item_Scripts
SD%Complete: 100
SDComment: Items for a range of different items. See content below
 * (in script)
SDCategory: Items
EndScriptData */
 ...
 * Hacklike implements chance to spawn item or creature
item_disciplinary_rod               Prevents
 * abuse
item_nether_wraith_beacon(i31742)   Summons creatures for quest Becoming a Spellfire Tailor
 * (q10832)
item_flying_machine(i34060,i34061)  Engineering crafted flying machines
item_gor_dreks_ointment(i30175)
 * Protecting Our Own(q10488)
item_muiseks_vessel                 Cast on creature, they must be dead(q
 * 3123,3124,3125,3126,3127)
item_only_for_flight                Items which should only useable while
 * flying
item_razorthorn_flayer_gland        Quest Discovering Your Roots (q11520) and Rediscovering Your Roots
 * (q11521). Prevents abuse
item_tame_beast_rods(many)          Prevent cast on any other creature than the intended
 * (for all tame beast quests)
item_soul_cannon(i32825)            Prevents abuse of this
 * item
item_sparrowhawk_net(i32321)        Quest To Catch A Sparrowhawk (q10987). Prevents abuse
item_voodoo_charm
 * Provide proper error message and target(q2561)
item_yehkinyas_bramble(i10699)      Allow cast spell on vale screecher
 * only and remove corpse if cast sucessful (q3520)
item_inoculating_crystal            Quest Inoculating. Prevent
 * abuse
EndContentData */
...
enum CrystalCores { ERRATIC_BEHAVIOUR = 11524, FURTHER_CONVERSIONS = 11525, ERRATIC_SENTRY = 24972 };
...
// Creature/Item are in fact created before spell are sucessfully casted, without any checks at all to ensure
...
public:
  ...
  bool OnUse(Player *player, Item *_Item, SpellCastTargets const &targets) override {
    if (targets.getUnitTarget() && targets.getUnitTarget()->GetTypeId() == TYPEID_UNIT &&
        ...
        (player->GetQuestStatus(ERRATIC_BEHAVIOUR) == QUEST_STATUS_INCOMPLETE ||
...
public:
  ...
  bool OnUse(Player *pPlayer, Item * /*pItem*/, SpellCastTargets const & /*targets*/) override {
    ...
    // targets.getGOTarget()->GetGOInfo()->type == GAMEOBJECT_TYPE_SPELL_FOCUS && targets.getGOTarget()->GetEntry() ==
    // 181616)
    //{
    if (pPlayer->GetQuestStatus(9452) == QUEST_STATUS_INCOMPLETE) {
      if (urand(0, 99) < 35) {
        Creature *Murloc =
            pPlayer->SummonCreature(17102, pPlayer->GetPositionX(), pPlayer->GetPositionY() + 20,
                                    pPlayer->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 10000);
        if (Murloc)
  ...
  bool OnUse(Player *player, Item *_Item, SpellCastTargets const &targets) override {
    ...
    if (itemSpell) {
      ...
      if (uTarget && uTarget->GetTypeId() == TYPEID_UNIT && uTarget->isDead() &&
          (uTarget->GetEntry() == cEntry || uTarget->GetEntry() == cEntry2 || uTarget->GetEntry() == cEntry3 ||
           uTarget->GetEntry() == cEntry4)) {
...
public:
  ...
  bool OnUse(Player *player, Item *_Item, SpellCastTargets const &targets) override {
    if (targets.getUnitTarget() && targets.getUnitTarget()->GetTypeId() == TYPEID_UNIT &&
        targets.getUnitTarget()->isDead() && targets.getUnitTarget()->GetEntry() == 7318)
      return false;
...
public:
  ...
  bool OnUse(Player *player, Item *_Item, SpellCastTargets const &targets) override {
    if (targets.getUnitTarget() && targets.getUnitTarget()->GetTypeId() == TYPEID_UNIT &&
        targets.getUnitTarget()->IsAlive() && targets.getUnitTarget()->GetEntry() == MOB_RIFT_SPAWN &&
        targets.getUnitTarget()->HasAura(SPELL_SELF_STUN_30SEC))
      return false;
