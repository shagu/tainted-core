...
int RASocket::process_command(const std::string& command)
{
    ...
    sLog.outError ("Got command: %s", command.c_str());
...
int RASocket::subnegotiate()
{
    ...
    if (n >= 1024)
    {
        sLog.outError ("RASocket::subnegotiate: allocated buffer 1024 bytes was too small for negotiation packet, size: %u", n);
        return -1;
    }
