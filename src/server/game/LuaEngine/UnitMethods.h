...
namespace LuaUnit
{
    ...
    {
        Eluna::Push(L, unit->IsQuestGiver());return 1;
    }
    ...
    {
        Eluna::Push(L, unit->IsNonMeleeSpellCast(false));
        return 1;
    }
    ...
    int GetAura(Eluna* /*E*/, lua_State* L, Unit* unit)
    {
        ...
        Eluna::Push(L, unit->GetAura(spellID, EFFECT_0));
        return 1;
    }
    ...
    {
        Eluna::Push(L, unit->IsInPvPCombat());
        return 1;
    }
    ...
    int SendChatMessageToPlayer(Eluna* /*E*/, lua_State* L, Unit* unit)
    {
        ...
        WorldPacket data;
        ChatHandler::FillMessageData(&data, NULL, type, lang, NULL, target->GET_GUID(), msg.c_str(), NULL);
    ...
    int AddAura(Eluna* /*E*/, lua_State* L, Unit* unit)
    {
        ...
        for (uint32 i = 0; i < MAX_SPELL_EFFECTS; ++i)
        {
            if (spellInfo->Effect[i] == SPELL_EFFECT_APPLY_AURA)
            {
                ...
                {
                    Aura* Aur = CreateAura(spellInfo, i, NULL, unit, unit);
                    bool add = unit->AddAura(Aur);
                    if (!result)
                        result = add;
                }
                else
                {
                    Aura* Aur = CreateAura(spellInfo, i, NULL, target, unit);
                    bool add = target->AddAura(Aur);
                    if (!result)
                        result = add;
                }
            }
    ...
    int DealDamage(Eluna* /*E*/, lua_State* L, Unit* unit)
    {
        ...
        if (!spell)
        {
            uint32 absorb = 0;
            uint32 resist = 0;
            target->CalcAbsorbResist(unit, schoolmask, SPELL_DIRECT_DAMAGE, damage, &absorb, &resist);
    ...
    int AddThreat(Eluna* /*E*/, lua_State* L, Unit* unit)
    {
        ...
        unit->AddThreat(victim, threat, (SpellSchoolMask)schoolMask, spell ? sSpellStore.LookupEntry(spell) : NULL);
        return 0;
    }
