...
uint8 Eluna::OnCanUseItem(const Player* pPlayer, uint32 itemEntry)
{
    START_HOOK_WITH_RETVAL(PLAYER_EVENT_ON_CAN_USE_ITEM, EQUIP_ERR_OK);
    uint8 result = EQUIP_ERR_OK;
    Push(pPlayer);
    Push(itemEntry);
    ...
    while (n > 0)
    {
        ...
        if (lua_isnumber(L, r))
            result = (uint8)CHECKVAL<uint32>(L, r);
...
void Eluna::OnBindToInstance(Player* pPlayer, uint8 difficulty, uint32 mapid, bool permanent)
{
